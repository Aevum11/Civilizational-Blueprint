...
def build_gui():
    ...
    class CDFApp:
        ...
        def __init__(self, master_root):
            ...
            ttk.Button(r1, text="Browse...", command=self.browse_in).pack(side="right")
        ...
        def browse_in(self):
            """Open file dialog for input file selection and auto-set output path."""
            file_path = filedialog.askopenfilename(filetypes=[("All", "*.*"), ("CDF", "*.cdf")])
            if file_path:
                self.in_var.set(file_path)
                self.out_var.set(file_path + '.cdf' if not file_path.endswith('.cdf') else file_path[:-4])
        ...
        def do_compress(self):
            """Launch CDF compression in a background thread."""
        ...
        def do_decompress(self):
            """Launch CDF decompression in a background thread."""
            inp, out = self.in_var.get(), self.out_var.get()
            if not inp or not out:
                messagebox.showerror("Error", "Select input and output files."); return
            self.set_btns('disabled')
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] != 'gui':
        ...
        if args.action == 'gui':
            ...
            compressor = CDFCompressor(log_fn=print, progress_fn=lambda pct, m: print(f"\r[{pct:5.1f}%] {m}", end='', flush=True))
