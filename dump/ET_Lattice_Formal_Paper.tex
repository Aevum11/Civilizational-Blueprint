...
Section~\ref{sec:primitives} introduces the three primitive categories, fixes their cardinalities, and derives the four manifold states. Section~\ref{sec:N12} gives three independent derivations of the forced value $N=12$. Section~\ref{sec:lattice} constructs the lattice on $(\R^{+},\times)$ and proves its convention-independence. Section~\ref{sec:sublattice} classifies the sublattice families. Section~\ref{sec:tower} constructs the LCM tower of refinements. Section~\ref{sec:complex} extends the lattice to $\C^{\times}$ via the imaginary axis. Section~\ref{sec:cascade} describes the cascade dynamics and stability. Section~\ref{sec:selfcons} proves the self-consistency identity. Section~\ref{sec:applications} confronts the lattice with empirical data in particle physics, music, geometry, biochemistry, and celestial mechanics. Section~\ref{sec:comparison} places the lattice alongside Fourier analysis and Riemannian geometry. Section~\ref{sec:predictions} formulates falsifiable predictions.

% ===========================================================
\section{The three primitive categories}\label{sec:primitives}
% ===========================================================

\subsection{A working categorical framework}

We adopt a minimal framework in which any configuration under study is described as a binding between three disjoint primitive categories. The framework is familiar to readers of Aristotelian metaphysics (substrate, form, agent) and of general systems theory (state space, constraints, process), but we state it here as a mathematical postulate, without appealing to any particular interpretation.

\begin{axiom}[Three disjoint primitive categories]\label{ax:categories}
There exist three pairwise disjoint categories $\PP$, $\DD$, $\TT$ with cardinalities
\[
|\PP| = \Omega, \qquad |\DD| = n, \qquad |\TT| = \bigl[\tfrac{0}{0}\bigr],
\]
where
\begin{itemize}[leftmargin=*]
\item $\Omega$ denotes an \emph{absolute infinity} (a proper class, understood as larger than every set-theoretic cardinal);
\item $n$ is an \emph{absolutely finite} cardinal (a positive integer, variable across applications);
\item $[0/0]$ denotes an \emph{indeterminate} cardinal (neither finite nor infinite; the cardinality of a primitive operator whose outputs are determined by binding rather than by elementary counting).
\end{itemize}
The three categories are pairwise disjoint:
\[
\PP\cap\DD = \emptyset, \qquad \DD\cap\TT = \emptyset, \qquad \TT\cap\PP = \emptyset.
\]
\end{axiom}

\begin{remark}
The symbols $P$, $D$, $T$ may be read as \emph{Point}, \emph{Descriptor}, \emph{Traverser}. The reader is invited to treat them initially as labels. Their mathematical content is exhausted by Axiom~\ref{ax:categories} and the composition rule below; no metaphysical commitments are required. In applications, $P$ will function as a bare substrate (the set over which a structure lives), $D$ as a system of finite constraints (rules, equations, metric, axioms), and $T$ as a resolution operator (the act of rounding a continuous quantity to a discrete one, the act of selecting one branch of a bifurcation, etc.).
\end{remark}

\begin{axiom}[Composition]\label{ax:compose}
There is a ternary binding operation
\[
\circ\colon \PP\times\DD\times\TT \longrightarrow \mathcal{E}
\]
which to every triple $(p,d,t)$ with $p\in\PP$, $d\in\DD$, $t\in\TT$ assigns an element $p\circ d\circ t\in\mathcal{E}$ of a space $\mathcal{E}$ of \emph{substantiated configurations}.
\end{axiom}

\begin{axiom}[Binding order]\label{ax:order}
No configuration is of the form $d\circ p$ with $d\in\DD$ and $p\in\PP$. Operationally, $p$ must exist before $d$ can describe it.
\end{axiom}

The force of Axiom~\ref{ax:order} is structural rather than set-theoretic. It asserts that descriptions presuppose what they describe, and therefore that, in any derivation that proceeds from the categories, the $P$-content must be specified first, the $D$-content second, the $T$-content third. This ordering will appear concretely in the projection protocol of Section~\ref{sec:lattice}, where the substrate (hence the reference period) is identified before any constraint and before any act of resolution.

\subsection{The four manifold states}

The power set of $\{\PP,\DD,\TT\}$ has eight elements. Two are trivial (the empty configuration and the three singletons, which describe nothing). Removing singletons and empties leaves four states which are the possible combinations of two or three of the categories.

\begin{definition}[Manifold states]\label{def:states}
The four \emph{manifold states} are
\[
\{\PP,\DD,\TT\},\qquad \{\PP,\DD\},\qquad \{\DD,\TT\},\qquad \{\PP,\TT\},
\]
named \emph{Exception}, \emph{Unsubstantiated}, \emph{Mediation}, and \emph{Incoherence} respectively.
\end{definition}
...
\label{eq:S}
\end{equation}
Thus $|\PP| = \Omega$, $|\DD| = n$, $|\TT| = [0/0]$, $|\Pi|=3$, $S=4$.

\begin{proposition}[Incoherence is structurally forbidden]\label{prop:incoherence}
The state $\{\PP,\TT\}$ admits no consistent binding: it contains substrate ($\PP$) and agency ($\TT$) but no finite constraint ($\DD$) by which agency can bind to substrate. Any attempt at a $\PP{-}\TT$ binding must produce a contradiction or dissolve into one of the other three states.
\end{proposition}

The proof is immediate from Axioms~\ref{ax:categories}--\ref{ax:order}: without $\DD$, an element of $\TT$ has no finite profile against which to test consistency with an element of $\PP$. In physical applications the $\{\PP,\TT\}$ state corresponds to null-pointer--like conditions: singularities, divisions by zero, paradoxes of self-reference without adequate stratification.
...
The two finite cardinals available in Axiom~\ref{ax:categories} and Definition~\ref{def:states} are
\[
...
The formula $N = |\Pi|\cdot S$ is the natural product of the independent counts: for each of the three primitive categories, there are four structurally distinct manifold states to which a given element can contribute (Exception, Unsubstantiated, Mediation, or Incoherence as a configuration to be avoided). The count is unique up to the ordering of the cardinalities fixed in Axiom~\ref{ax:categories}.
...
Here $V$ is the \emph{base variance}: the irreducible quantum of resolution on the logarithmic lattice, equal to $1/N$ octaves. The value $K$ is derived from Definition~\ref{def:states}: of the four manifold states, three contain $\PP$ (Exception, Unsubstantiated, Incoherence) and two contain both $\PP$ and $\DD$ (Exception, Unsubstantiated); more tellingly,
\[
...
The correspondence is elementary but structurally informative: the three Gaussian prime classes exactly parallel the three categories of primitive $\PP$, $\DD$, $\TT$. Ramified primes (which ``double'' in $\Z[i]$) are $P$-class; inert primes (which remain on the real axis) are $D$-class; split primes (which factor across the real-imaginary split) are mixed $D{+}T$-class.
