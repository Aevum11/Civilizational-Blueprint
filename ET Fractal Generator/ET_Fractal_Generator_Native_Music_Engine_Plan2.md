...
- **A₀ = (N−1)² + S² = 137** — integer, exact. Sum of squares of non-trivial semitone count (N−1=11) and manifold state count (S=4).
- **A₁ = √3/48 = σ/K_EM** — open shimmer over 8 EM channels. σ = √V = √(1/12) = √3/6. K_EM = N·K = 12·(2/3) = 8.
- **A_cross = √3/(93312π²) = (2/π)·A₁·A₂** — product interference of shimmer (open) with closed bilateral-mediation loop. A₂ = κ²/(N³π).
- **Σ A_k = 1/(216(18π−1)) = κ²/(N²(Nπ−κ))** — closed-form geometric series of higher closed-mediation loops.
...
**F.B.2** The metabolism architecture — α⁻¹ + K governs resource allocation:

The metabolism is the program's self-aware resource management system, derived entirely from ET constants. It reads the "floor of the computer" (CPU silicon, RAM, VRAM) as P — the physical substrate — and applies ET-derived Descriptors to allocate resources.

**Hard limits (non-negotiable):**
- **K = 2/3 for program:** The fractal generator claims at most 2/3 of available system resources. This is the Koide binding stability threshold — the program is bound to the hardware with two of three primitives aligned.
- **1−K = 1/3 for system:** Reserved for OS, other processes, thermal headroom. This is the system's share — one primitive unbound.

**Soft scaling (α⁻¹-derived, within the K=2/3 ceiling):**

The fine structure constant provides the impedance for fine-grained allocation within the K-ceiling:

| Resource | Hard ceiling | α-scaled allocation | Derivation |
|---|---|---|---|
| CPU threads | floor(K × N_CPU) | floor(A₁ × N_CPU × K) | A₁ = shimmer = the active fraction of available bandwidth |
| Memory (RAM) | floor(K × total_RAM) | Allocated per tile based on impedance | Tile memory = pixels × channels × dtype_size × ξ(d) where ξ(d) = A₀/((d−1)²+S²) |
| VRAM (GPU) | floor(K × total_VRAM) | Reserve V×VRAM for lattice engine overhead | V = 1/12 = base variance = minimum headroom |
| Tile rows | Current static preset values | Dynamic: floor(N × K × VRAM_available / tile_memory) | N=12 tiles per manifold cycle, scaled by K and available VRAM |
| Process priority | Normal | K × available priority range | On Windows: BELOW_NORMAL_PRIORITY_CLASS for K<1 |

**Metabolism as control loop (not just startup):**

The metabolism monitors resource usage during the render and adjusts dynamically:
1. **Before each tile:** Check available VRAM/RAM. If usage exceeds K×total, wait for GC.
2. **After each tile:** Report actual usage vs. metabolism limit. Log violations.
3. **Thermal consideration:** On systems with temperature sensors accessible via sysfs/WMI, the metabolism reduces thread count when CPU temp exceeds A₀ = 137°F ≈ 58.3°C (a structural threshold derived from the fine structure impedance).

**F.B.3** The metabolism report at startup:

```
═══════════════════════════════════════════════════════════════════════
  ET METABOLISM — Sempaevum α⁻¹ Resource Allocation
═══════════════════════════════════════════════════════════════════════
  α⁻¹(ET) = 137.035999177... (120 digits computed, matches CODATA ±0.46σ)
  K = 2/3 (program ceiling)   1−K = 1/3 (system reserve)

  P (substrate floor):
    CPU: AMD Ryzen 9 5900X — 12 cores / 24 threads
    RAM: 32.00 GB total
    GPU: NVIDIA RTX 2070 SUPER — 8.00 GB VRAM

  D (allocation constraints):
    CPU threads: floor(2/3 × 24) = 16 threads (program)  |  8 threads (system)
    RAM limit:   floor(2/3 × 32.00) = 21.33 GB (program) |  10.67 GB (system)
    VRAM limit:  floor(2/3 × 8.00) = 5.33 GB (program)   |  2.67 GB (system)
    Tile rows:   dynamic per-frame (based on VRAM availability)
    VRAM headroom: V × 8.00 = 0.67 GB (lattice engine overhead)

  T (verification):
    Self-projection: {N,1/N,K,1/K} → d=12, |ε|=1.955¢ ✓ (Koide attractor)
    α⁻¹ identity:    closed-form matches 50-term series ✓
    CODATA check:     |α⁻¹(ET) − α⁻¹(CODATA)| < 0.46σ ✓
═══════════════════════════════════════════════════════════════════════
```

**F.B.4** Wire the metabolism into the existing resource management:
...
```python
# ── Metabolism-derived resource limits ──────────────────────────────
N_CPU = max(1, os.cpu_count() or 1)
_metabolism = getattr(_dll, 'compute_metabolism')
# ... ctypes call to C module that returns the metabolism struct ...
N_THREADS_MAX = _metabolism_result.cpu_threads  # = floor(K × N_CPU)
N_THREADS = 1 if USE_GPU else N_THREADS_MAX
VRAM_PROGRAM = _metabolism_result.vram_limit    # = floor(K × total_VRAM)
RAM_PROGRAM  = _metabolism_result.ram_limit     # = floor(K × total_RAM)
```

The tile size calculation (currently static presets at lines 647–651) becomes dynamic:
```python
# Dynamic tile rows: metabolism-aware
if USE_GPU:
    _tile_memory = RENDER_W * TILE_ROWS * N_CHANNELS * FLOAT_SIZE * 2  # input+output
    while _tile_memory > VRAM_PROGRAM * V:  # V = 1/12 = headroom fraction
...
**R16: Metabolism thermal monitoring is platform-dependent. (Phase F)**
CPU temperature reading via sysfs (Linux) or WMI (Windows) is not universally available. **Mitigation:** thermal monitoring is optional — if temperature sensors are inaccessible, the metabolism operates without thermal throttling and prints a note. The K=2/3 hard ceiling still applies regardless.
