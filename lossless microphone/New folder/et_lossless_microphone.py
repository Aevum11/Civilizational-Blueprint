...
class ETLiveCapture:
    ...
    def record_interactive(self, output_name=None):
        """
        Start recording from the microphone. Press Enter to stop.

        Saves three files in the script directory:
          {name}.wav     — reconstructed WAV (for playback)
          {name}.etlm    — ET lossless format (the REAL lossless data)
          {name}_raw.wav — raw capture WAV (original PCM from ADC)

        Returns the list of ET samples, or empty list if no hardware.
        """
        ...
        raw_wav_path = script_path(f"{output_name}_raw.wav")
        ...
        # ─── Save raw WAV ───
        print(f"\n  Saving raw capture → {os.path.basename(raw_wav_path)}")
        max_val = (1 << (self.bit_depth - 1))
        samp_width = self.bit_depth // 8
        ...
        for i in range(n_frames):
            for ch in range(self.channels):
                # Scale int32 to bit_depth range
                ...
                if samp_width == 2:
                    raw_bytes.extend(struct.pack('<h', int_val & 0xFFFF))
                elif samp_width == 3:
        ...
        # ─── Project through ET bijection ───
        print(f"  Projecting {len(sample_strings)} samples through ET bijection...")
        et_samples = []
        for s_str in sample_strings:
        ...
        header = ETAudioFormat.encode_header(
            self.sample_rate, self.channels, self.N, self.R0,
        ...
        # ─── Reconstruct and save WAV ───
        print(f"  Reconstructing → {os.path.basename(recon_wav_path)}")
        metadata = {
        ...
        recon_strs, _ = ETWavIO.read_wav(recon_wav_path)
        mismatches = 0
        for orig_str, recon_str in zip(sample_strings, recon_strs):
            if mpf(orig_str) != mpf(recon_str):
                mismatches += 1

        if mismatches == 0:
            print(f"  ✓ BIT-EXACT — {len(sample_strings)} samples, zero loss")
        else:
            print(f"  ✗ {mismatches} mismatches out of {len(sample_strings)}")
        ...
        # ─── Summary ───
        raw_size = os.path.getsize(raw_wav_path)
        etlm_size = os.path.getsize(etlm_path)
        recon_size = os.path.getsize(recon_wav_path)

        print(f"\n  Output files (in {get_script_dir()}):")
        print(f"    {os.path.basename(raw_wav_path):<40} {raw_size:>10} bytes  (raw ADC)")
        print(f"    {os.path.basename(etlm_path):<40} {etlm_size:>10} bytes  (ET lossless)")
        print(f"    {os.path.basename(recon_wav_path):<40} {recon_size:>10} bytes  (reconstructed)")

        return et_samples

    def record_timed(self, duration_seconds=5, output_name=None):
        """
        Record for a fixed duration. Same output as record_interactive.
        """
        if not self.hardware_available:
        ...
        # Save the recording as a temporary WAV, then process
        max_val = (1 << (self.bit_depth - 1))
        raw_wav = script_path(f"{output_name}_raw.wav")
        samp_width = self.bit_depth // 8
        ...
        # Project and save ETLM + reconstructed WAV
        et_samples = []
        for s_str in sample_strings:
            et_samples.append(self.audio_projector.project_sample(s_str))
        ...
        print(f"  Saved: {os.path.basename(raw_wav)}, "
              f"{os.path.basename(etlm_path)}, {os.path.basename(recon_path)}")
        return et_samples
...
def cmd_process(cmd_args):
    ...
    # Project through ET bijection
    print(f"  Projecting through ET bijection...")
    audio_proj = ETAudioProjector(N, R0)
    et_samples = []
...
def show_usage():
  ...
  Commands:
    record [name]              Record from microphone (press Enter to stop)
                               Saves: name.wav, name.etlm, name_raw.wav
