...
(and vice versa) problem for all of mathematics and science.
...
class ETLiveCapture:
    ...
    AUTO-DETECTS maximum hardware capabilities:
      - Probes sample rates from 384000 Hz down to 8000 Hz
      - Probes bit depths from int32 (32-bit) down to int16 (16-bit)
      - Selects the input device with highest capability
      - Uses the maximum supported configuration automatically

    Records from the system microphone with interactive start/stop control.
    Each captured sample is projected through the ET bijection for lossless
    storage. Output files are saved in the same directory as this script.

    The result: the clearest recording possible from any given hardware.
    The math is perfectly lossless. The only variable is what the
    hardware provides to the bijection.
    """
    ...
    # Zero float in the chain — integer dtypes only
    ...
    def __init__(self, N=12, R0="1.0"):
        ...
        # Check for sounddevice + PortAudio availability
        try:
            ...
            self.hardware_available = len(input_devices) > 0
            self.sd = sd

            if self.hardware_available:
                # Select best input device and auto-detect maximum capabilities
                self._auto_detect_capabilities(input_devices)
        except (OSError, ImportError, AttributeError):
            self.hardware_available = False

    def _auto_detect_capabilities(self, input_devices):
        """
        Probe hardware for maximum supported sample rate, bit depth,
        and channel count. Uses the highest-capability input device.

        This is what makes every recording the clearest possible from
        the given hardware. The bijection handles everything the ADC provides.
        """
        sd = self.sd
        ...
        for dev_id, dev_info in input_devices:
            ...
            # Probe each dtype from highest precision down
            for probe_dtype in self.PROBE_DTYPES:
                # Probe each sample rate from highest down
                for probe_rate in self.PROBE_RATES:
                    try:
                        sd.check_input_settings(
                            device=dev_id,
                            channels=min(dev_channels, 2),  # Test mono or stereo
                            dtype=probe_dtype,
                            samplerate=probe_rate)
                        supported_rates.append(probe_rate)
            ...
            # Is this device better than our current best?
            if dev_best_rate > best_rate:
                best_device_id = dev_id
                best_device_info = dev_info
                best_rate = dev_best_rate
                best_dtype = dev_best_dtype
                best_channels = dev_channels
                best_supported_rates = sorted(set(supported_rates), reverse=True)

        if best_device_id is not None and best_rate > 0:
            self.device_id = best_device_id
            self.default_device = best_device_info
            self.sample_rate = best_rate
            self.max_sample_rate = best_rate
            self.dtype = best_dtype
            self.bit_depth = self.DTYPE_BITS.get(best_dtype, 16)
            self.channels = min(best_channels, 2)  # Mono or stereo
            self.all_supported_rates = best_supported_rates
        else:
            # Fallback to device defaults
            self.default_device = self.sd.query_devices(kind='input')
            self.device_id = self.default_device.get('index', None)
            self.sample_rate = int(self.default_device.get('default_samplerate', 48000))
            self.max_sample_rate = self.sample_rate
            self.channels = min(self.default_device.get('max_input_channels', 1), 2)
            self.bit_depth = 16
            self.dtype = 'int16'
            self.all_supported_rates = [self.sample_rate]

    def print_hardware_report(self):
        """Print a full report of detected hardware capabilities."""
        ...
        print(f"\n  ┌──────────────────────────────────────────────────────────┐")
        print(f"  │  HARDWARE CAPABILITY REPORT                              │")
        print(f"  ├──────────────────────────────────────────────────────────┤")
        print(f"  │  Device:      {dev.get('name', 'Unknown'):<42}│")
        print(f"  │  Channels:    {self.channels:<42}│")
        print(f"  │  Max rate:    {self.max_sample_rate} Hz{' ' * (36 - len(str(self.max_sample_rate)))}│")
        print(
            f"  │  Bit depth:   {self.bit_depth}-bit ({self.dtype}){' ' * (30 - len(str(self.bit_depth)) - len(self.dtype))}│")
        print(f"  │  Supported rates: {', '.join(str(r) for r in self.all_supported_rates[:6])}")

        # Oversampling analysis for common audio frequencies
        if self.max_sample_rate >= 44100:
            oversample_20k = self.max_sample_rate / 20000
            oversample_4k = self.max_sample_rate / 4000
            print(f"  │  Oversampling @20kHz: {nstr(mpf(str(oversample_20k)), 4)}×"
                  f"{' ' * max(0, 30 - len(nstr(mpf(str(oversample_20k)), 4)))}│")
            print(f"  │  Oversampling @4kHz:  {nstr(mpf(str(oversample_4k)), 4)}×"
                  f"{' ' * max(0, 30 - len(nstr(mpf(str(oversample_4k)), 4)))}│")

        print(f"  │  Status:      ✓ READY — maximum capabilities active     │")
        print(f"  └──────────────────────────────────────────────────────────┘")
    ...
    def record_interactive(self, output_name=None):
        ...
        # Start the audio stream at maximum precision
        stream = self.sd.InputStream(
            samplerate=self.sample_rate,
            channels=self.channels,
            dtype=self.dtype,  # Auto-detected maximum precision
...
def interactive_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            cmd_record([rec_name] if rec_name else [])
