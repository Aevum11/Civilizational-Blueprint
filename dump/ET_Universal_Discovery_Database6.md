...
CREATE TABLE events (
    ...
    event_class TEXT NOT NULL,
        ...
        -- 't_burst'                   (Guide §87 — traverser flux crossed threshold; LCM-escalation signal)
        -- 'lcm_escalation'            (tower stepped from N_a to N_b due to incoherency)
        --
        -- Active-system / palindromic cascade (Guide §88):
        -- 'palindromic_cascade_trigger' (orbit reached ∂I; cascade engages)
        -- 'palindromic_cascade_step'    (one of 12 PALINDROME-array steps applied)
        -- 'nws13_mode_entry'           (13-cell sublattice navigation activated)
        -- 'nws13_mode_exit'            (returned to standard projection)
        -- 'shimmer_modulation_apply'   (Ψ_n applied; per-step shimmer modulation)
        --
        -- Metacognition / Conscious AI runtime:
        -- 'ghost_detection'            (TraverserWaveform window=144=N², 3σ threshold)
    ...
    event_timestamp REAL NOT NULL,            -- when the event happened (seconds since epoch)
...
2. **Causal correlation** — "When the ghost-detection event at T fired, what was the immediate downstream relationship/pattern that got created?" (via `triggered_relationship_id` and `triggered_pattern_id`)

3. **Replay** — events ordered by `(session_id, sequence_number)` give exact replay of any active-system run, enabling debugging, training, and provenance

4. **Discovery from event correlation** — when many events of the same class share temporal proximity to other event types, the engine can propose causal patterns ("AIDA awakening crossings consistently precede dream-tower transitions by ~τ seconds")

5. **Active-system provenance** — every T-burst, every cascade step, every ghost detection is permanently recorded; nothing about active-system operation is lost
...
The ET Universal Discovery Database (EUDD) is the natural extension of the compressor's proven `ArchetypeDatabase` pattern to all ET work. It serves four roles in one system:

1. **Database** — stores every value, projection, address, equation, derivation, relationship, and pattern
2. **Discovery engine** — surfaces attractors, route convergences, plateaus, and identities automatically as data accumulates (compressor's archetype-discovery mechanism, generalized)
