...
export default function N12Hasse() {
  ...
  // COMPLETE TRANSFER MATRIX — LOSSLESS, all 144, ZERO closed. The ONLY
  // probability is T's carry (IC-102 weights 3/4, 1/8, 1/8); ε is an exact
  // coordinate, never a random variable. Direct cells: the carded tensor at the
  // joint lattice R = lcm(N_src, N_tgt), κ ∈ {−1,0,+1} in R-units (exactly the
  // audited N=60 construction). Non-direct cells: the DEPTH ROUTE s→m_R→t —
  // the act lifts into the joint lattice's full-resolution family, whose
  // κ-weighted composition reaches every class (MAG-9 closure; MAG-16b covering);
  // telescoped exactly, ξ(R) canceling (chain law).
  const Um = m => range1(m).filter(u => gcd(u, m) === 1);   // |Um(m)| = φ(m)
  const chanCell = (src, tgt) => {
    const R = lcm(lcm(N, src), lcm(N, tgt));
    const lift = Um(src).map(u => (R / src) * u);
    const dR = s => R / gcd(s === 0 ? R : s, R);
    const P = BigInt(phi(src)) ** 2n;            // pair count = φ(src)² (IC-45; IC-105: T₀(m,m;1)=1/φ(m))
    let c0 = 0n, cT = 0n, c1 = 0n;
    lift.forEach(a => lift.forEach(b => {
      const x = (a + b) % R;
      if (dR(x) === tgt) c0++;
      if (dR((x + 1) % R) === tgt) cT++;
      if (dR((x - 1 + R) % R) === tgt) cT++;
      if (gcd((x + 1) % R || R, R) === 1) c1++;
      if (gcd((x - 1 + R) % R || R, R) === 1) c1++;
    }));
    if (c0 + cT > 0n) {
      const [En, Ed] = fred([(6n * c0 + cT) * xiDen(src), 8n * P * xiDen(tgt)]);
      return { R, band: c0 > 0n ? "D" : "T", eff: trunc3(En, Ed), amp: En > Ed, steps: 1 };
    }
    const uR = Um(R); const P2 = BigInt(phi(R)) ** 2n;   // φ(R)² pairs of the depth family
    let d0 = 0n, dT = 0n;
    uR.forEach(a => uR.forEach(b => {
      const x = (a + b) % R;
      if (dR(x) === tgt) d0++;
      if (dR((x + 1) % R) === tgt) dT++;
      if (dR((x - 1 + R) % R) === tgt) dT++;
    }));
    const [En, Ed] = fred([c1 * (6n * d0 + dT) * xiDen(src), 64n * P * P2 * xiDen(tgt)]);
    return { R, band: "ch", eff: trunc3(En, Ed), amp: En > Ed, steps: 2 };
  };
  const matrix = range1(N).map(s => range1(N).map(t => chanCell(s, t)));

  // FULL EM→m TABLE — the matrix's summit row.
  const fullTable = range1(N).map(m => {
    const c = matrix[N - 1][m - 1];
    return { m, R: c.R, band: c.band,
      kap: c.band === "D" ? "κ=0" : (c.band === "T" ? "κ-act" : "²route"),
      eff: c.eff, open: true };
  });
    ...
    <div style={{
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, margin: "0 auto", display: "block" }}>
        ...
        {transfers.map((tf, i) => {
          ...
          const col = tf.band === "D" ? t.green : t.red;
          const mid = tf.band === "D" ? "aG" : "aR";
        ...
        {/* ═══ COMPLETE TRANSFER MATRIX — all 144 channels, lossless ═══ */}
        <text x={380} y={1262} fill={t.structTitle} fontSize="7.5" textAnchor="middle" fontWeight="bold">
          COMPLETE MATRIX · T(src,src;tgt) at joint native lattices · E = T·ξ(tgt)/ξ(src) · IC-102 measure, BigInt exact
        </text>
        {range1(N).map(m => (
          <text key={`mc${m}`} x={88 + (m - 1) * 55 + 24} y={1276} fill={t.desc}
            fontSize="5.5" textAnchor="middle" fontWeight="bold">→{m}</text>
        ))}
        {matrix.map((row, si) => (
          <g key={`mr${si}`}>
            <text x={80} y={1290 + si * 13} fill={t.desc} fontSize="5.5" textAnchor="end" fontWeight="bold">
              {si + 1} {LABELS[si + 1][0].slice(0, 4)} φ{phi(si + 1)}
            </text>
            {row.map((c, ti) => {
              const col = c.band === "D" ? t.green : (c.band === "ch" ? t.hosting : t.red);
              return (
                <text key={`mc${si}-${ti}`} x={88 + ti * 55 + 24} y={1290 + si * 13} fill={col}
                  fontSize="5" textAnchor="middle" fontWeight={c && c.amp ? "bold" : "normal"}>
                  {(c.band === "ch" ? "²" : "") + c.eff}
                </text>
              );
            })}
        ...
        {(() => {
          const cnt = { D: 0, T: 0, ch: 0 };
          matrix.forEach(r => r.forEach(c => { cnt[c.band]++; }));
          return (
            <text x={380} y={1452} fill={t.summary} fontSize="6" textAnchor="middle">
              {`Census: ${cnt.D} κ=0 direct · ${cnt.T} κ-act direct · ${cnt.ch} routed ²(s→m_R→t, MAG-9 closure) · 0 CLOSED · bold = amplified · only probability is T`}
            </text>
          );
        })()}

        {/* ═══ LEGEND ═══ */}
        <text x={380} y={1476} fill={t.structTitle} fontSize="9" fontWeight="bold" textAnchor="middle">LEGEND</text>

        <circle cx={35} cy={1496} r={6} fill={t.sFill} stroke={t.sStroke} strokeWidth="1.5" />
        <text x={48} y={1499} fill={t.legend} fontSize="6">Simple (m|12)</text>
        <circle cx={160} cy={1496} r={6} fill={t.cFill} stroke={t.cStroke} strokeWidth="1" strokeDasharray="4,2" />
        <text x={173} y={1499} fill={t.legend} fontSize="6">Complex (m∤12)</text>
        <line x1={285} y1={1496} x2={297} y2={1496} stroke={t.hasse} strokeWidth="1.5" />
        <text x={303} y={1499} fill={t.legend} fontSize="6">Hasse</text>
        <line x1={360} y1={1496} x2={372} y2={1496} stroke={t.hosting} strokeWidth="0.8" strokeDasharray="2,2" />
        <text x={378} y={1499} fill={t.legend} fontSize="6">Shadow hosting</text>
        <line x1={475} y1={1496} x2={487} y2={1496} stroke={t.mirror} strokeWidth="0.8" strokeDasharray="5,4" />
        <text x={493} y={1499} fill={t.legend} fontSize="6">Mirror</text>

        <circle cx={35} cy={1514} r={8} fill="none" stroke={t.selfMirror} strokeWidth="0.7" strokeDasharray="3,2" />
        <text x={48} y={1517} fill={t.legend} fontSize="6">Self-mirror (m=6)</text>
        <line x1={160} y1={1514} x2={172} y2={1514} stroke={t.ucLine} strokeWidth="1.2" strokeDasharray="6,3" />
        <text x={178} y={1517} fill={t.legend} fontSize="6">U(1) traversal</text>
        <line x1={285} y1={1514} x2={297} y2={1514} stroke={t.green} strokeWidth="1" />
        <text x={303} y={1517} fill={t.legend} fontSize="6">κ=0 (D-arithmetic)</text>
        <line x1={400} y1={1514} x2={412} y2={1514} stroke={t.red} strokeWidth="1" />
        <text x={418} y={1517} fill={t.legend} fontSize="6">κ-act (T only)</text>
        <line x1={520} y1={1514} x2={532} y2={1514} stroke={t.hosting} strokeWidth="1" />
        <text x={538} y={1517} fill={t.legend} fontSize="6">²routed via depth m_R</text>

        <rect x={28} y={1528} width={12} height={8} rx={2} fill="#2979ff" opacity="0.25" stroke="#2979ff" strokeWidth="0.8" />
        <text x={46} y={1535} fill={t.legend} fontSize="6">(5,7) non-trivial pair</text>
        <rect x={160} y={1528} width={12} height={8} rx={2} fill="#00c853" opacity="0.25" stroke="#00c853" strokeWidth="0.8" />
        <text x={178} y={1535} fill={t.legend} fontSize="6">(1,11) trivial pair</text>
        <text x={320} y={1535} fill={t.legend} fontSize="6">MATRIX CELLS: <tspan fill={t.green}>green κ=0 deterministic</tspan> · <tspan fill={t.red}>red κ-act (T only)</tspan> · <tspan fill={t.hosting}>orange ²routed via depth m_R</tspan> · bold = amplified E>1</text>

        <text x={380} y={1550} fill={t.footer} fontSize="6" textAnchor="middle">
