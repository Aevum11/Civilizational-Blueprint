...
const GRID=2,HEAT=4;
const WT={extreme:{label:"Extreme",lbs:200,color:"#581c87",pri:5,desc:"Safes, engines, anvils"},extra:{label:"Extra Heavy",lbs:100,color:"#991b1b",pri:4,desc:"Appliances, solid wood"},heavy:{label:"Heavy",lbs:60,color:"#c2410c",pri:3,desc:"Dressers, tables"},medium:{label:"Medium",lbs:30,color:"#a16207",pri:2,desc:"Packed boxes"},light:{label:"Light",lbs:10,color:"#15803d",pri:1,desc:"Clothes, pillows"},negligible:{label:"Negligible",lbs:3,color:"#0d9488",pri:0,desc:"Empty bins, foam, decor"}};
const WT_KEYS=["extreme","extra","heavy","medium","light","negligible"];
...
// Heat diffusion kernel — t(d)=100/(100+d×50), R=4, normalized to Σ=1.
// K=⅔ at d=1 (nearest neighbor). 49 cells. Overlapping kernels sum → hotspots.
const DIFF_R=4;const _dkRaw=[];let _dkS=0;
for(let ky=-DIFF_R;ky<=DIFF_R;ky++)for(let kx=-DIFF_R;kx<=DIFF_R;kx++){const d=Math.sqrt(ky*ky+kx*kx);if(d<=DIFF_R){const w=100/(100+d*50);_dkRaw.push({ky,kx,w});_dkS+=w;}}
const DIFF_K=_dkRaw.map(k=>({ky:k.ky,kx:k.kx,w:k.w/_dkS}));
...
async function computePlan(trailer,items){
  ...
  const phased=[1,2,3,4,5].map(ph=>items.filter(it=>CAT[it.category].phase===ph).sort((a,b)=>{const wp=WT[b.weight].pri-WT[a.weight].pri;if(wp)return wp;const ww=itemWeight(b)-itemWeight(a);if(Math.abs(ww)>0.01)return ww>0?1:-1;return(b.l*b.w)-(a.l*a.w);}));
  ...
  for(let pi=0;pi<5;pi++){const phase=pi+1;for(let ii=0;ii<phased[pi].length;ii++){const item=phased[pi][ii];
    ...
    if(res.pos){const p=res.pos,cat=CAT[item.category];
      ...
      for(let dy=0;dy<p.gl;dy++)for(let dx=0;dx<p.gw;dx++)if(p.mask[dy][dx]){
        ...
        // Heat diffusion — weight radiates outward via normalized tightness kernel
        const hcy=p.gy+dy,hcx=p.gx+dx;
        for(const k of DIFF_K){const ny=hcy+k.ky,nx=hcx+k.kx;if(ny>=0&&ny<gL&&nx>=0&&nx<gW)weightHeat[ny][nx]+=wtPC*k.w;}}
      totalPW+=itemWt;
...
function findPos(hmap,noStack,trailer,item,phase,gW,gL,lW,rW,maps,weightHeat,totalPW,validC,rotations){
  ...
  for(const rot of rotations){
    ...
    for(let gy=0;gy<=gL-rot.gl;gy++){for(let gx=0;gx<=gW-rot.gw;gx++){
      ...
      //    Reads from the diffused heat field — nearby items' weight already
      //    bleeds into these cells via the DIFF_K kernel at placement time.
...
export default function TrailerLoadPlanner(){
  ...
  const[layerStep,setLayerStep]=useState(999);const[wtVis,setWtVis]=useState({extreme:true,extra:true,heavy:true,medium:true,light:true,negligible:true});
        ...
        if(src){
          try{const res=await window.storage.get(`tlp-${src}`);if(res&&res.value){const d=JSON.parse(res.value);
            ...
            if(d.setup){const s=d.setup;
              ...
              if(s.step)setStep(s.step==="plan"?"items":s.step);
            if(s.csRows&&s.csRows.length||s.lpRows&&s.lpRows.length||s.obsRows&&s.obsRows.length||s.doorWIn||s.doorHIn)setShowAdv(true);}
          }}catch(e){}
        }
  ...
  if(step==="plan"&&plan&&trailer){
    ...
    const cohLabel=cohVal>=K+0.1?"SAFE":cohVal>=K?"CAUTION":"UNSAFE";
    ...
    return(<div style={{padding:"16px",maxWidth:480,margin:"0 auto",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      ...
      <div style={{...card,borderColor:cohCol,borderWidth:"2px",background:`linear-gradient(135deg, ${cohCol}11, #fff)`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontSize:"13px",fontWeight:"700",color:"#374151"}}>Load Safety</div>
            <div style={{fontSize:"11px",color:"#6b7280"}}>Worst-case across weight balance metrics</div></div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:"2px"}}><span style={{fontSize:"9px",color:"#9ca3af"}}>0%</span><span style={{fontSize:"9px",color:"#1f2937",fontWeight:"700"}}>Safe min</span><span style={{fontSize:"9px",color:"#9ca3af"}}>100%</span></div>
      </div>
      ...
      <div style={card}><h2 style={{fontSize:"16px",fontWeight:"700",marginBottom:"12px",color:"#1f2937"}}>Metrics</h2>
        ...
        {cohVal<K&&<div style={{marginTop:"10px",padding:"10px",backgroundColor:"#fef2f2",borderRadius:"8px",border:"1px solid #fca5a5"}}><p style={{fontSize:"12px",color:"#991b1b",margin:0,fontWeight:"600"}}>✗ UNSAFE — weight distribution is dangerously imbalanced. Redistribute heavy items before travel.</p></div>}</div>
        ...
        return <div style={card}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"4px"}}>
            {WT_KEYS.map(k=>{const c=WT[k],on=wtVis[k];return<button key={k} onClick={()=>setWtVis(p=>({...p,[k]:!p[k]}))}
              style={{padding:"8px 2px",fontSize:"11px",fontWeight:"700",border:`2px solid ${on?c.color:"#d1d5db"}`,borderRadius:"8px",
      ...
      {viewMode==="iso"&&(()=>{
        ...
        // ═══ Profile-accurate back wall (cross-section at y=L) ═══
        const _bwSc=_fW>0?Math.min(interp(_lp,L,"d","w")/_fW,1):1;const _bwPts=[];const _bwS=20;
        for(let i=0;i<=_bwS;i++){const z2=(i/_bwS)*H;const wz=interp(_cs,z2,"h","w")*_bwSc;_bwPts.push(ip(W/2-wz/2,L,z2));}
        for(let i=_bwS;i>=0;i--){const z2=(i/_bwS)*H;const wz=interp(_cs,z2,"h","w")*_bwSc;_bwPts.push(ip(W/2+wz/2,L,z2));}
        let backPath=`M ${_bwPts[0][0]},${_bwPts[0][1]}`;for(const bp of _bwPts)backPath+=` L ${bp[0]},${bp[1]}`;backPath+=" Z";
        // ═══ Profile-accurate left wall (cs × lp surface outline) ═══
        const _lwX=(y3,z3)=>{const wz=interp(_cs,z3,"h","w");const wy=interp(_lp,y3,"d","w");const sc=_fW>0?Math.min(wy/_fW,1):1;return W/2-wz*sc/2;};
        const _lwPts=[];const _lwS=20;
        for(let i=0;i<=_lwS;i++){const y3=(i/_lwS)*L;_lwPts.push(ip(_lwX(y3,0),y3,0));}
        for(let i=1;i<=_lwS;i++){const z3=(i/_lwS)*H;_lwPts.push(ip(_lwX(L,z3),L,z3));}
        for(let i=_lwS-1;i>=0;i--){const y3=(i/_lwS)*L;_lwPts.push(ip(_lwX(y3,H),y3,H));}
        for(let i=_lwS-1;i>=1;i--){const z3=(i/_lwS)*H;_lwPts.push(ip(_lwX(0,z3),0,z3));}
        let leftPath=`M ${_lwPts[0][0]},${_lwPts[0][1]}`;for(const wp of _lwPts)leftPath+=` L ${wp[0]},${wp[1]}`;leftPath+=" Z";
        ...
        return <div style={card}><p style={{...hint,marginBottom:"8px"}}>Isometric 3D. Tap item for details.</p>
          <div style={{display:"flex",justifyContent:"center",overflowX:"auto"}}>
            <svg width={svgIW} height={svgIH} viewBox={`0 0 ${svgIW} ${svgIH}`} style={{maxWidth:"100%"}}>
              ...
              <path d={backPath} fill="#d1d5db" stroke="#9ca3af" strokeWidth="1"/>
              <path d={leftPath} fill="#e5e7eb" stroke="#9ca3af" strokeWidth="1"/>
