...
### 7.10 The EUDD Manager — standalone .exe program

The EUDD is managed by a standalone executable program (PyInstaller single-file .exe, matching the compressor's deployment model). This program is the central hub for all ET software. It performs three roles:

**1. Bootstrap and generation.** On first run, the manager generates the initial EUDD by ingesting all bootstrap content (§3.17 + §3.18): the 24+42 family catalog, 227 particle projections, all named constants, impedance table, cascade residuals, formal system projections, fine-structure identity, Koide attractor, Rosetta Stone mappings, and every other bootstrap entry defined in this document. The result is a fully-populated SQLite database (sharded per §7.1) ready for all projects.

**2. Ongoing management and discovery.** The manager runs continuously (or on schedule), performing:
- Discovery engine scans (§3.16): insert-time, background, and generator-candidate discovery
- Subsumption-driven compaction (§3.8, §4.3): archetype promotion, generator augmentation
- Self-recording (§3.1b): the manager's own metrics (query patterns, cache-hit rates, discovery rates) are projected onto the Sempaevum and stored as values/events in the EUDD
- Ingestion adapters: accepts data from the compressor, Conscious AI, fractal generator, emotion engine, ETPL programs, sensor streams, and any future ET software
- Consistency checks (§5.5): cross-project validation on every ingest

**3. Central database for all projects.** The EUDD is the single shared database that every ET project connects to. No project maintains its own separate discovery database. The compressor, Conscious AI, fractal generator, emotion engine, ETPL runtime, and any future software all read from and write to the same EUDD through the manager's API. Cross-project discovery is automatic because all data lives in shared address space.
...
| Operation | `ArchetypeDatabase` | EUDD (SQLite Phase 1) |
|---|---|---|
...
It scales to 20+ TB via sharding (Phase 1) or proprietary lattice-native storage (Phase 2). Generator discovery via the compressor's CDF/Tier 7 pipeline continuously reduces K-complexity of stored data without destroying any records.
