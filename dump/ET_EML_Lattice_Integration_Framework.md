...
**Part IV — The Three Projection Pathways** (§10–§13)
 10. Pathway A — Direct Value Projection ({P,D,T} Exception)
 11. Pathway B — Limit Convergence Projection ({D,T} Mediation → {P,D,T} Exception)
 12. Pathway C — Meta-Descriptor Projection ({P,D} Unsubstantiated)
...
# PART IV — THE THREE PROJECTION PATHWAYS

Given an object $X \in \Sigma$ whose lattice address we want, the correct pathway depends on $X$'s manifold-state classification (§3).
...
**When:** $X$ is not individually computable (no finite algorithm produces its value to arbitrary precision) but has a finite *defining Descriptor* — a consistent formal definition. This includes Chaitin's Ω, large cardinals, many undecidable propositions, and impredicative constructions.
...
```
Given a mathematical object X:
  Identify the manifold state of X at the relevant integrative level.
    IF X is a {P,D,T} Exception (fully substantiated value):
        Pathway A (direct value projection)
    ELSE IF X is a {D,T} Mediation (process in progress, limit-definable):
        Pathway A or B (evaluate in-progress state or take the limit)
    ELSE IF X is {P,D} Unsubstantiated:
        IF X is definable and computable in principle:
            Pathway B (limit convergence)
        ELSE IF X is definable but not individually computable:
            Pathway C (meta-descriptor projection)
    ELSE IF X is {P,T} Incoherence:
        X lives at ∂I boundary;
        the Incoherence Filter places it; no further projection is possible
        (this is the correct lattice-classification of contradictions —
        Guide §87, the Koide threshold K=2/3 marks the ∂I boundary).
```

This decision rule covers every mathematical object because the four manifold states classify all of Σ (§3), and the three pathways handle all four states (state {P,T} produces a diagnostic rather than a projection, which is the correct outcome).
...
**Example:** $\mathrm{erf}$ sampled at $\{0.5, 1, 1.5, 2\}$:
- $\mathrm{erf}(0.5) \approx 0.5205 \to (-11, 12, +28.46)$
- $\mathrm{erf}(1.0) \approx 0.8427 \to (-3, 4, +3.711)$
- $\mathrm{erf}(1.5) \approx 0.9661 \to (0, 1, -49.76)$ (near ∂I boundary!)
- $\mathrm{erf}(2.0) \approx 0.9953 \to (0, 1, -8.16)$

Reading the signature: erf starts in d=12 EM (near zero), transitions through d=4 quartic (mid-range), and **crosses the ∂I boundary near $x=1.5$** (ε ≈ 50¢), approaching the unison asymptotically. The ∂I crossing at $x \approx 1.5$ is a structural feature: it's where erf's growth rate becomes indistinguishable from the octave closure at unity.
