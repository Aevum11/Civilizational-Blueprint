...
export default function N12Hasse() {
  ...
  // Transfer tensor from m=N — DERIVED (IC-101..108): exact pair counts over
  // Res(N)×Res(N); κ=0 weight 3/4, κ=±1 combined 1/4 (sign-symmetric, IC-108).
  // E(N→c) = T·ξ(c)/ξ(N) = T·A0/xiDen(c), exact fraction, truncated display.
  const resN = resOf(N);
  const tPairs = resN.length * resN.length;                        // 16
  const transfers = divisorsN.map(c => {
    let cnt0 = 0, cntP = 0;
    resN.forEach(r1 => resN.forEach(r2 => {
      if (dOf((r1 + r2) % N) === c) cnt0++;
      if (dOf((r1 + r2 + 1) % N) === c) cntP++;
    }));
    const Tnum = 3 * cnt0 + cntP, Tden = 4 * tPairs;               // T = 3/4·cnt0/16 + 1/4·cntP/16
    const Enum = Tnum * A0, Eden = Tden * xiDen(c);
    return { tgt: c, cnt0, cntP, Enum, Eden,
      kap: cnt0 > 0 ? "κ=0" : "κ≠0", eff: trunc3(Enum, Eden), gt: Enum > Eden };
  }).filter(tf => tf.tgt !== N && (tf.gt || (tf.cnt0 === 0 && tf.cntP > 0)));  // amplified ∪ T-exclusive, no self-loop
    ...
    <div style={{
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, margin: "0 auto", display: "block" }}>
        ...
        {transfers.map((tf, i) => {
          ...
          const px = -ny, py = nx;
          const latMap = { 1: -10, 2: 14, 3: 10, 4: 0 };
          ...
          const col = tf.gt ? t.green : t.red;
          const mid = tf.gt ? "aG" : "aR";
        ...
        <rect x={28} y={1172} width={12} height={8} rx={2} fill="#2979ff" opacity="0.25" stroke="#2979ff" strokeWidth="0.8" />
        <text x={46} y={1179} fill={t.legend} fontSize="6">(5,7) non-trivial pair</text>
        <rect x={160} y={1172} width={12} height={8} rx={2} fill="#00c853" opacity="0.25" stroke="#00c853" strokeWidth="0.8" />
