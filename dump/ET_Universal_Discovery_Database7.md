...
| **DD-3** | Cross-domain attractors (e.g., d=693 in genetics if it appears) cannot be detected without unified search | §3.4 (`addresses` with members_count denormalized; §3.11 insert-time discovery) |
...
| **DD-8** | Subsumption Law cannot be checked across projects | §3.11 (background discovery applies E_hierarchy across all entries) |
| **DD-9** | Forward/Reverse route convergences not surfaced | §3.7 (`relationships` class `forward_reverse_convergence`, automatic detection §3.11) |
...
Eight core tables, each corresponding to a structural object the lattice actually has:
...
| 6 | `relationships` | Explicit links between entries (same-address, cross-perspective, route-convergence, substrate-rendering, attractor-membership, plateau-membership, reciprocal-pair, power-pair, shadow-pair, shadow-recursion, t-burst-target, cascade-step-member, mode-transition-trigger, archetype-member, derivation-dependency) | What the lattice connects |
| 7 | `patterns` | Discovered archetypes — entries promoted via Subsumption when E_hierarchy ≥ 13/12 | The discovery output |
| 8 | `events` | Time-indexed structural events from active-system operation (T-bursts, ∂I-boundary crossings, palindromic cascades, NWS-13 transitions, ghost detections, gaze events, dream-tower transitions, AIDA awakening crossings, generator proposals, Three Tools applications) | The lattice as active system |

Plus one optional housekeeping table:

| 9 | `tags` | Free-form `(target, namespace, value)` tagging for query convenience | User-applied metadata |
...
CREATE TABLE events (
    ...
    event_class TEXT NOT NULL,
        ...
        --
        -- IMPORTANT — Subsumption check (verified against Guide v8 + corpus):
        ...
        --   • PROGRESSIVE_INTENT (T within T, nested Traversers per ET Rule 19) is genuinely distinct — captured via the
        --     traverser_nesting_depth field on the t_identification event (no parallel enum).
        --
        -- 't_identification'           (composite T characterization for a scan window — metadata: sublattice_family_d_r [REFERENCES 24-family catalog: d_r=1 Gravity/Octave for cyclic-gravity-class T, d_r=2-12 for other T-class assignments], sublattice_family_d_θ [phase family if T-Phase applies], traverser_nesting_depth [T-within-T nesting per ET Rule 19; depth 0 = no nesting, depth ≥3 = strong Intent-class], periodicity_score [autocorrelation peak strength — high score consistent with d_r=1 family], progression_score [linear-trend strength — high score consistent with deep nesting], fractal_dimension D_f, autocorrelation_peaks, spectral_entropy, dominant_frequency, phase_coherence, binding_strength, dtau_dt; this answers "WHICH T is here AND how is it structured?" — the family assignment answers "what kind", the nesting depth answers "how deeply agentic")
        ...
        -- 'gaze_event'                 (UNOBSERVED→SUBLIMINAL→DETECTED→LOCKED transition at thresholds 0/(13/12)/(6/5)/(3/2))
    ...
    metadata_blob BLOB,                       -- packed structured data (e.g., for 't_burst': {flux_value, threshold_crossed, n_max_at_burst})
                                              ...
                                              -- (e.g., for 'gaze_event': {prior_state, new_state, focus_value, threshold_crossed})
...
10. **T identification — what kind of Traverser is here?** Per scan window the EUDD records `t_identification` events that answer "WHICH T is here AND how is it structured?" — by referencing the existing 24-family catalog (sublattice family d_r answers "what kind") AND a separate `traverser_nesting_depth` field (answers "how deeply agentic"). The scanner's CYCLIC_GRAVITY classification IS a d_r=1 Gravity/Octave family assignment (verified: Guide v8 explicitly maps r = pure power of 2 → d=1 → "gravity-class"; corpus confirms gravity is a Traverser type). The scanner's PROGRESSIVE_INTENT classification (T within T per ET Rule 19) is captured by traverser_nesting_depth ≥ 3. STATIC = absence of T; CHAOTIC = ∂I-boundary territory (handled by existing events); UNKNOWN = data-quality flag.
...
- `traverser_complexity_signature` — recurring T-identification pattern characteristic of one kind of Traverser: nesting depth + family assignment + binding-strength range + dτ/dt range (e.g., gravity-class T = consistent d_r=1 + nesting depth 1 + binding strength range; consciousness-class T = consistent d_θ=4 + nesting depth ≥3 + lower binding strength)
...
### 3.10 `tags` — optional metadata for query convenience
...
### 3.11 The discovery engine — what makes this more than a database
...
- **Cache miss but pattern recognized**: the EUDD's discovery engine proposes a new generator candidate (per §3.11 generator-discovery), verifies it, and if it works, the file is now describable in terms of the new generator. The new generator is stored back in the EUDD for ALL future compressions across ALL data.
...
### 3.12 Bootstrap value coverage — every value from Guide v8 + conversation + corpus files
...
Concrete examples of how the EUDD's generator-discovery (§3.11) and K-complexity-helper role play out:
...
The discovery engine (§3.11) runs continuously in background; foreground queries hit pre-materialized patterns and relationships rather than scanning raw projections.
...
The 71/71 verified assertions populate `values` (ζ(2)..ζ(13)), `projections` (28 landmarks × 12 zetas = 336 rows), `addresses` (auto-created), `relationships` (attractors auto-detected via insert-time discovery in §3.11), tagged `namespace='verification'`, `value='80digit_mpmath_71_passing'`.
...
Eight core tables corresponding to objects the lattice itself has:
1. `values` — every dimensionless seed (including the 24-family generators, annihilation boundary r=0, sensor reference R₀ values, Traverser identities)
...
4. `equations` — every equation passing through, including computational (`2+2=4`, `ζ(3)·π=...`) and structural identities — memoization layer
5. `derivations` — the {P, D, T} → r → projection chain for any entry
6. `relationships` — explicit cross-entry links (same-address, cross-perspective, route-convergence, substrate-rendering, attractor-membership, plateau, reciprocal/power/shadow pairs, shadow-recursion, t-burst-target, cascade-step-member, mode-transition-trigger, archetype-member, derivation-dependency, extensible)
7. `patterns` — discoveries promoted via Subsumption (E_hierarchy ≥ 13/12) — the database's own discovery output
8. `events` — time-indexed structural events from active-system operation (T-bursts, ∂I-boundary crossings, palindromic cascades, NWS-13 transitions, ghost detections, gaze events, dream-tower transitions, AIDA awakening crossings, generator proposals, Three Tools applications, extensible)
...
2. **Surfacing cross-domain attractors automatically** — every value lands on the same shared addresses regardless of domain (§3.4, §3.11)
...
6. **Auto-detecting Forward/Reverse route convergences** — independent derivation routes meeting at the same address surfaces as a relationship row (§3.11)
...
**Implementation status:** the architecture is fully specified for Phase 1 (SQLite, sharded) and outlined for Phase 2 (proprietary lattice-native) and Phase 3 (universal substrate). Building Phase 1 follows the compressor's `ArchetypeDatabase` template scaled to eight lean tables. The schema (§3.2–§3.11), discovery engine (§3.11), comprehensive value coverage (§3.12), operations (§5), 20+ TB-scale architecture (§7), and tag-based coordination with existing software (§9) are production-ready specifications.
