...
function computePlan(trailer,items){
  ...
  for(let pi=0;pi<5;pi++){const phase=pi+1;for(const item of phased[pi]){
...
export default function TrailerLoadPlanner(){
  ...
  useEffect(()=>{if(loading)return;(async()=>{try{await window.storage.set('tlp-auto',JSON.stringify({trailer,items,setup:{tFt,tIn,wLim,csRows,lpRows,obsRows,doorWIn,doorHIn,step}}));}catch(e){}})();},[trailer,items,tFt,tIn,wLim,csRows,lpRows,obsRows,doorWIn,doorHIn,step,loading]);
  ...
  const calc=()=>{if(!trailer||!items.length)return;setPlan(computePlan(trailer,items));setStep("plan");setLayerStep(999);setSelItem(null);};
  ...
  if(step==="items"){const valid=cn.trim()&&parseFloat(cL)>0&&parseFloat(cW)>0&&parseFloat(cH)>0&&cWt&&cCat;
    return(<div style={{padding:"16px",maxWidth:480,margin:"0 auto",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      ...
      {items.length>0&&<><div style={{maxHeight:"280px",overflowY:"auto",marginBottom:"14px"}}>
        ...
        <button onClick={calc} style={{...btnP,backgroundColor:"#059669",marginBottom:"10px"}}>Calculate Loading Plan</button>
        <button onClick={manualSave} style={{...btnS,marginBottom:"10px"}}>{savedMsg||"💾 Save"}</button></>}
    </div>);}
