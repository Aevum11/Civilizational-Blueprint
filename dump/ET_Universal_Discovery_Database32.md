...
| **GPU rendering module** | OpenGL 4.6 lattice visualization with six-level LOD (Sempaevum → Tower → Resolution → Family → Address → Entity), freecam with no physics, 3D perspective switching, Force Quadrant Grid renderer, tower trajectory renderer, attractor cluster renderer, heat maps |
| **GUI module** | Dear ImGui panels: management dashboard (live metrics, discovery status, session history), property inspector (click any entity → full 120-digit detail + bidirectional provenance), manual input (L₁/L₂/L₃ backbone + direct value/projection/triple/text entry with real-time preview), file/stream ingestion (drag-and-drop, progress), search and retrieval (persistent search bar, all search modes, file/stream regeneration from generators), connection manager (active pipes, incoming queue, outgoing), query builder, event log viewer, settings (input remapping for keyboard/mouse/XInput gamepads) |
...
| §7 Implementation (all subsections) | BSV (format), LBP (precision), API (protocol + API-11), OMN (Omniscient + §7.18 shutdown: OMN-08..12), BAK (backup), EXT (extensions), HOM (§7.11), FRT (§7.12/§7.13 including FRT-08..10 text/PDF/enhanced), SRE-05..06 (§7.19 metabolism re-sensing and data ingestion) |
