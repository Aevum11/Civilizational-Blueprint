...
## MATHEMATICAL ROSETTA STONE (§18, key entries)
...
| D_string = 10 | 2^{\|Π\|} + d₂ | Division algebra route |
| D_M = 11 | N−1 | Four independent routes |
...
- v1.7 — **CRITICAL: PDF assumed never provided again.** Added ALL missing core PDF content: topological characterization of manifold states (E closed, I open, M/U neither), Nesting Principle (Absolutes actual not limits, Substrate Potential), Anti-Emergence Principle (E/◦/I non-emergent, constitutive), temporal triple (P_time∘D_time∘T_time=E_moment, Lorentz factor=dt/dτ), T's operational manifold U(1) with period 2π (three derivations), horizon physics (surface gravity=descriptor-gap gradient, Hawking temp=κ/(2π), information preservation via Multifold birth triad, fermion stats from {P,T} forbidden), Sublattice-to-Force Map (d=3→strong/8 gluons, d=4→weak/3 bosons via dW=N(1-K), d=1→EM/photon, total 8+3+1=12=N), N-Exhaustion Theorem (SU(3)×SU(2)×U(1) UNIQUE), Structural Significance Principle (P1-P4), geometric content of primitives (ℝ⁺ flat, U(1) curved, effective curvature convex combination), Pure Relationalism (no outside, no absolute nothing). Online research moved to separate ONLINE_RESEARCH_JOURNAL.md with full distilled findings.
