...
# ============================================================
# LCM Tower — INFINITE, no maximum level
# Convergence: d-family stabilization (Chaitin approach)
# ============================================================
STABILITY_DEPTH = 2   # ceil(1/K) = ceil(3/2) = 2 consecutive LCM landmarks
VERIFY_DEPTH = 2      # ceil(1/K) additional verification landmarks

def cf_home_find(r_mp):
    """
    Continued Fraction home-finding (§7.11 Step 3a).
    
    For r ∈ ℝ⁺, compute the CF expansion of |log₂(r)| and find
    the convergent p_n/q_n with maximal following partial quotient a_{n+1}.
    
    d_home = q_n (denominator of that convergent)
    quality = a_{n+1} (how many denominators before a better approximation exists)
    
    The CF method runs IN PARALLEL with the LCM tower. It succeeds for
    tower-resistant values (algorithmically random, irrational log₂).
    
    Returns dict with cf_d_home, cf_quality, cf_p, cf_q, cf_eps, cf_class.
    """
    log2_r = abs(mpmath.log(r_mp, 2))
    
    # Continued fraction expansion of |log₂(r)|
    x = log2_r
    a_coeffs = []        # partial quotients [a₀, a₁, a₂, ...]
    p_prev, p_curr = 0, 1   # convergent numerators p_{-1}=0, p_0 not yet
    q_prev, q_curr = 1, 0   # convergent denominators q_{-1}=1, q_0 not yet
    convergents = []     # list of (p_n, q_n, a_n)
    
    for i in range(300):  # 300 terms → well beyond 120-digit precision
        ...
        convergents.append((p_new, q_new, a_i))
        
        p_prev, p_curr = p_curr, p_new
        q_prev, q_curr = q_curr, q_new
        
        # Next: x ← 1/(x - a_i)
        frac = x - mpmath.mpf(a_i)
        if abs(frac) < mpmath.power(10, -110):
            break  # Terminated — exact rational log₂ (rare)
        x = mpmath.mpf(1) / frac
    
    if not convergents:
        return {"cf_d_home": 1, "cf_quality": 0, "cf_p": 0, "cf_q": 1,
                "cf_eps": mpmath.mpf(0), "cf_class": "cf_marginal"}
    
    # Find convergent with maximal FOLLOWING partial quotient a_{n+1}
    best_quality = 0
    best_idx = 0
    for i in range(len(convergents) - 1):
        next_a = convergents[i + 1][2]  # a_{i+1}
        if next_a > best_quality:
            best_quality = next_a
            best_idx = i
    
    p_home, q_home, _ = convergents[best_idx]
    
    # Compute ε_CF at N = q_home using the standard projection
    if q_home > 0:
        r_str = mpmath.nstr(r_mp, TARGET_DIGITS + 10)
        proj = sempaevum_project_mp(r_str, q_home)
        if proj:
            cf_eps = proj[2]
        else:
            cf_eps = mpmath.mpf(0)
    ...
    # Classify per §7.11 Step 3a thresholds
    abs_eps_micro = abs(int(mpmath.nint(cf_eps * 1000)))
    KOIDE_MICRO = 1955
    DI_MICRO = 50000
    QUALITY_THRESHOLD = 4  # ceil(1/K)^2 = 4
    
    if best_quality >= QUALITY_THRESHOLD:
        if abs_eps_micro <= KOIDE_MICRO:
            cf_class = "cf_deep_home"
        elif abs_eps_micro < DI_MICRO:
            cf_class = "cf_home"
        else:
    ...
    return {
        "cf_d_home": q_home,
        "cf_quality": best_quality,
        "cf_p": p_home,
        "cf_q": q_home,
        "cf_eps": cf_eps,
        "cf_eps_micro": abs_eps_micro,
        "cf_class": cf_class,
        "cf_convergents_count": len(convergents),
    }



def generate_lcm_landmarks(max_k=200):
    """
    Generate distinct LCM landmarks: lcm(1..k) for k=1,2,...
    Only yields when the LCM actually increases (new prime or prime power).
    This is a HELPER for iteration, not a cap — the tower extends as needed.
    """
    ...
    for k in range(1, max_k + 1):
        current_lcm = current_lcm * k // gcd(current_lcm, k)
        if current_lcm >= 12 and current_lcm != prev_lcm:
            landmarks.append((current_lcm, f"LCM(1..{k})", k))
            prev_lcm = current_lcm
    return landmarks

# Pre-generate landmarks for fast iteration (extends to lcm(1..200))
LCM_LANDMARKS = generate_lcm_landmarks(200)
...
def compute_tower_projection(r_mp, r_str, mass_err_ppm=None):
    """
    Project a ratio through the infinite LCM tower until d-family stabilizes.
    
    The tower is INFINITE. There is no maximum level. Convergence is by
    d-family stabilization: d persists across STABILITY_DEPTH consecutive
    LCM landmarks, then verified for VERIFY_DEPTH additional landmarks.
    This is the ET-native convergence criterion from the Chaitin projection.
    
    For measured values (mass_err_ppm provided), also accept convergence
    when |ε| drops below measurement uncertainty — the data has no more
    information to give.
    ...
    # Optional ε-threshold for measured data
    if mass_err_ppm is not None and mass_err_ppm > 0:
        threshold_cents = mpmath.mpf(mass_err_ppm) / mpmath.mpf(577)
    else:
        threshold_cents = None  # No ε-threshold — use d-stabilization only
    
    tower_results = []
    converged = False
    
    # d-family stabilization state
    last_lcm_d = None
    consec_same_d = 0
    home_found = False
    home_idx = None
    ...
    # Iterate through LCM landmarks — the tower extends as deep as needed
    for current_lcm, lcm_label, n in LCM_LANDMARKS:
        
        proj = sempaevum_project_mp(r_str, current_lcm)
        if proj is None:
            continue
        k, d, eps_mp = proj
        ...
        # Sublattice family characterization
        abs_k = abs(k)
        g = et_gcd(abs_k, current_lcm) if abs_k > 0 else current_lcm
        sublattice_g = g
        ...
        # ── d-family stabilization (Chaitin approach) ──
        if d == last_lcm_d:
            consec_same_d += 1
        else:
            # d changed — reset, and invalidate any pending home
            if home_found and verify_remaining > 0:
                home_found = False
                home_idx = None
            consec_same_d = 1
            last_lcm_d = d
        
        if consec_same_d >= STABILITY_DEPTH and not home_found:
            home_found = True
            home_idx = len(tower_results) - 1
            verify_remaining = VERIFY_DEPTH
        elif home_found and verify_remaining > 0:
            verify_remaining -= 1
            if verify_remaining <= 0:
                converged = True
                break  # Home verified — tower complete for this ratio
        
        # ── Also accept ε-threshold convergence for measured data ──
        if threshold_cents is not None and abs_eps < threshold_cents:
            converged = True
            break
...
def find_true_home(tower_results):
    """
    Find the tower level where d-family first stabilized.
    
    The home was identified DURING tower computation (d persisted for
    STABILITY_DEPTH + VERIFY_DEPTH consecutive LCM landmarks). This
    function locates that level in the stored results.
    ...
    # Walk backwards from the end to find where d first stabilized
...
def compute_particle_projections(pdg_particles=None):
    ...
    for p in pdg_particles:
        ...
        # ── N/Z STABILITY RATIO: r = N/Z ──
        A_val = p.get("A", 0)
        Z_val = p.get("Z", 0)
        N_val = A_val - Z_val
        if N_val <= 0 or Z_val <= 0:
            continue  # Skip: N=0 (proton) or invalid
        r_mp = mpmath.mpf(N_val) / mpmath.mpf(Z_val)
        mass_mev_mp = mpmath.mpf(0)
        mass_mev = mpmath.mpf(0)
        
        r_str_full = mpmath.nstr(r_mp, TARGET_DIGITS + 10)
        mass_str = mpmath.nstr(r_mp, 30)  # N/Z as string
        ...
        # N/Z convergence handled by d-family stabilization
        else:
        ...
        # ── CF METHOD (§7.11 Step 3a) — parallel pathway ──
        cf_result = cf_home_find(r_mp)
        ...
        # ── N/Z STABILITY PROJECTION ──
        A_val = p.get("A", 0)
        N_neutrons = A_val - Z_num
        if N_neutrons > 0 and Z_num > 0:
            nz_mp = mpmath.mpf(N_neutrons) / mpmath.mpf(Z_num)
            nz_str = mpmath.nstr(nz_mp, 30)
            nz_proj = sempaevum_project_mp(nz_str, N)
        else:
            nz_mp = mpmath.mpf(0)
            nz_proj = None
        ...
        result = {
            ...
            # CF Method (§7.11 Step 3a) — parallel home-finding
            "cf_d_home": cf_result["cf_d_home"],
            "cf_quality": cf_result["cf_quality"],
            "cf_p": cf_result["cf_p"],
            "cf_q": cf_result["cf_q"],
            "cf_eps": cf_result["cf_eps"],
            "cf_class": cf_result["cf_class"],
            ...
            "A": p.get("A", 0),
            "N": p.get("N", 0),
            "is_naturally_occurring": p.get("is_naturally_occurring", False),
            "abundance": p.get("abundance", mpmath.mpf(0)),
            "binding_energy_per_A_keV": p.get("binding_energy_per_A_keV", "0"),
            "mass_excess_keV": p.get("mass_excess_keV", "0"),
            "Ar_str": p.get("Ar_str", p.get("mass_str", "")),
            "mass_unc_str": p.get("mass_unc_u", "0"),
            "IE_eV": p.get("IE_eV", "0"),
            # ── N/Z STABILITY PROJECTION ──
            # The Descriptor that reveals nuclear stability: r_nz = N/Z
            # Valley of stability = lattice band k_nz ∈ [0, 7]
            "k_nz": nz_proj[0] if nz_proj else None,
            "d_nz": nz_proj[1] if nz_proj else None,
            "eps_nz": nz_proj[2] if nz_proj else None,
            "nz_ratio": nz_mp,
        }
...
def compute_structural_ratios(results):
    ...
    def project_ratio(name, r_mp, et_pred=None):
        ...
        entry = {
            ...
            "projection_12": (proj[0], proj[1], proj[2]),
            "eps_mp": proj[2],
            "eps_120": mpmath.nstr(proj[2], TARGET_DIGITS, strip_zeros=False),
        }
        if et_pred is not None:
            entry["ET_prediction"] = et_pred
            entry["ET_prediction_mp"] = et_pred
            dev_mp = abs(r_mp - et_pred) / et_pred * mpmath.mpf(1000000)
            entry["deviation_ppm"] = dev_mp
            entry["deviation_ppm_mp"] = dev_mp
        return entry
    # For N/Z projection, the "structural ratios" are ratios of N/Z values
    # between key nuclides — comparing stability positions
    
    def get_nz(Z, A):
        """Get an isotope's N/Z as mpmath."""
        r = by_id.get(Z * 1000 + A)
        if r and r.get("r_mp"):
            return r
        return None
    
    # Deuteron N/Z vs He-4 N/Z
    D = get_nz(1, 2)   # N/Z = 1
    He4 = get_nz(2, 4)  # N/Z = 1
    Fe56 = get_nz(26, 56)  # N/Z = 30/26 = 15/13
    Pb208 = get_nz(82, 208)  # N/Z = 126/82
    U238 = get_nz(92, 238)   # N/Z = 146/92 = 73/46
    Sn132 = get_nz(50, 132)  # N/Z = 82/50 = 41/25
    Ca48 = get_nz(20, 48)    # N/Z = 28/20 = 7/5
    
    if Fe56 and D:
        ratios.append(project_ratio("(N/Z)_Fe56 / (N/Z)_D",
                                     Fe56["r_mp"] / D["r_mp"]))
    if Pb208 and He4:
        ratios.append(project_ratio("(N/Z)_Pb208 / (N/Z)_He4",
                                     Pb208["r_mp"] / He4["r_mp"]))
    if U238 and Fe56:
        ratios.append(project_ratio("(N/Z)_U238 / (N/Z)_Fe56",
                                     U238["r_mp"] / Fe56["r_mp"]))
    if Pb208 and Fe56:
        ratios.append(project_ratio("(N/Z)_Pb208 / (N/Z)_Fe56",
                                     Pb208["r_mp"] / Fe56["r_mp"]))
    if Sn132 and Ca48:
        ratios.append(project_ratio("(N/Z)_Sn132 / (N/Z)_Ca48",
                                     Sn132["r_mp"] / Ca48["r_mp"]))
    if Ca48 and D:
        ratios.append(project_ratio("(N/Z)_Ca48 / (N/Z)_D",
                                     Ca48["r_mp"] / D["r_mp"]))
...
def generate_3d_html(results, ratios, fqg_html=""):
    ...
    # Stability colors — the N/Z viewer shows stability, not chemistry
    category_colors = {
        "Natural (valley)": "#00FF7F",    # Bright green — naturally occurring, in band
        "Natural (edge)": "#7FFF7F",      # Light green — natural but near edge
        "Proton-rich": "#FF4444",         # Red — N/Z too low (k_nz < 0)
        "Neutron-rich": "#4488FF",        # Blue — N/Z too high (k_nz > 7)
        "Unstable (in band)": "#FFD700",  # Gold — unstable but in stability band
    }
    ...
    # Build trace data — grouped by STABILITY STATUS
    traces_data = {}
    for r in results:
        ...
        t.setdefault("_symbols", []).append(r["symbol"])
        
        # Size based on log mass (visual emphasis — Plotly needs float)
        # Size based on mass number A (uniform-ish for N/Z viewer)
        A_for_size = r.get("A", 50)
        size_val = max(mpmath.mpf(4), min(mpmath.mpf(12), mpmath.mpf(3) + mpmath.mpf(A_for_size) / mpmath.mpf(40)))
        ...
        hover = (
            f"<b>{r['name']}</b><br>"
            f"Z={r.get('Z',0)} N={r.get('A',0)-r.get('Z',0)} | N/Z={mpmath.nstr(r.get('r_mp', mpmath.mpf(0)), 8)}<br>"
            f"Charge: {r['charge']}e | J={r['spin']}<br>"
            f"━━━ N/Z Stability @ N=12 (120-digit) ━━━<br>"
            f"r = N/Z = {mpmath.nstr(r.get('r_mp', mpmath.mpf(0)), 12)}<br>"
            ...
            f"━━━ CF Method: d_home={r.get('cf_d_home','?')} quality={r.get('cf_quality','?')} [{r.get('cf_class','?')}] ━━━<br>"
    ...
    for d_r in sorted(impedance_groups.keys()):
        ...
        # Show isotopes grouped by stability status
        by_stab = {"Natural": [], "Unstable": []}
        for p in particles:
            key = "Natural" if p.get("is_naturally_occurring") else "Unstable"
            by_stab[key].append(p["name"])
        
        for stab_key in ["Natural", "Unstable"]:
            names = by_stab[stab_key]
            if names:
                color = "#00FF7F" if stab_key == "Natural" else "#FF4444"
                impedance_groups_html += (
                    f'<div><span style="color:{color}">{stab_key} ({len(names)})</span>: '
                    f'{", ".join(names[:30])}{" ..." if len(names) > 30 else ""}</div>'
                )
        
        impedance_groups_html += '</div></div>\n'
    
    # Full HTML
    # Count stability categories for stat cards
    nat_in_band = sum(1 for r in results if r.get("is_naturally_occurring") and r.get("k_nz") is not None and 0 <= r.get("k_nz", -1) <= 7)
    unstable_in_band = sum(1 for r in results if not r.get("is_naturally_occurring") and r.get("k_nz") is not None and 0 <= r.get("k_nz", -1) <= 7)
    proton_rich = sum(1 for r in results if r.get("k_nz") is not None and r.get("k_nz") < 0)
    neutron_rich = sum(1 for r in results if r.get("k_nz") is not None and r.get("k_nz", 0) > 7)
    
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>Sempaevum N/Z Stability Viewer — Valley of Stability — P ∘ D ∘ T = E</title>
...
<div class="header">
    <h1>SEMPAEVUM</h1>
    <div class="subtitle">Atomic Lattice Viewer — N/Z Stability Projection — The Valley of Nuclear Stability</div>
    <div class="equation">P ∘ D ∘ T = E &nbsp;&nbsp;|&nbsp;&nbsp; 3 = 3 = 3 = Σ</div>
    <div class="subtitle" style="margin-top:8px;">r = N/Z → Sempaevum Bijection → Stability Band k_NZ ∈ [0,7] Revealed</div>
</div>
...
<div class="section">
    ...
    <div class="info-box">
        <b>Identification Principle:</b> P = nuclear substrate (the nuclide's existence), D = structural ratio N/Z (neutron-to-proton ratio), T = the projection that identifies stability.<br>
        <b>Descriptor Gap Principle:</b> The mass projection is blind to stability — the gap IS the Descriptor N/Z. Projecting r = N/Z reveals the valley of stability as a lattice band k ∈ [0,7].<br>
        <b>Subsumption Law:</b> Every N/Z ratio finds a lattice address. All 2324 isotopes project without remainder. Natural isotopes concentrate in k ∈ [0,7]; unstable isotopes spread beyond.
    </div>
</div>
...
<div class="section">
    <h2>Stability Band Summary</h2>
    <div class="stats-grid">
    <div class="stat-card" style="border-left:3px solid #00FF7F">
        <div class="stat-label">Natural (in band)</div>
        <div class="stat-value" style="color:#00FF7F">{nat_in_band}</div>
        <div class="stat-detail">k_NZ ∈ [0,7]</div>
    </div>
    <div class="stat-card" style="border-left:3px solid #FFD700">
        <div class="stat-label">Unstable (in band)</div>
        <div class="stat-value" style="color:#FFD700">{unstable_in_band}</div>
    </div>
    <div class="stat-card" style="border-left:3px solid #FF4444">
        <div class="stat-label">Proton-rich (k&lt;0)</div>
        <div class="stat-value" style="color:#FF4444">{proton_rich}</div>
    </div>
    <div class="stat-card" style="border-left:3px solid #4488FF">
        <div class="stat-label">Neutron-rich (k&gt;7)</div>
        <div class="stat-value" style="color:#4488FF">{neutron_rich}</div>
    </div>
    </div>

    <h2>Complete N/Z Stability Projections — Sempaevum Bijection at N=12</h2>
    <div class="info-box">
...
<div class="section">
    ...
    <div class="info-box">
        Isotopes with N=0 (the bare proton ¹H) have N/Z=0, placing them at the ∂I annihilation boundary (log₂(0) = −∞). These are excluded from the N/Z projection. All other isotopes with N≥1 project onto the lattice. The valley of stability occupies k_NZ ∈ [0,7] — 8 semitones wide.
    </div>
</div>
...
<div class="footer">
    ...
    N/Z Stability Projection — AME2020 isotope data, r = N/Z projected at 120-digit mpmath precision<br>
...
const layout = {{
    ...
    title: {{
        text: 'N/Z Stability Projection — Green=Natural, Gold=Unstable(in band), Red=Proton-rich, Blue=Neutron-rich',
...
def generate_report(results, ratios):
    ...
    report = []
    report.append("# Sempaevum N/Z Stability Viewer — Valley of Stability — Complete Report")
    report.append("## The N/Z Stability Projection — Valley of Nuclear Stability on the Lattice")
    report.append("")
    report.append("**Author:** Michael James Muller (Aevum Defluo)")
    report.append("**Theory:** Exception Theory — P ∘ D ∘ T = E")
    report.append("**DSR:** r = N/Z (neutron-to-proton ratio) | **Data:** AME2020 (Wang et al., 2021)")
    ...
    report.append("")
    report.append("| Isotope | N/Z | r = N/Z | k_r | d_r | ε_r (¢) | FORCE Channel | ξ(d_r) | k_θ | d_θ | d_comb | FQG |")
    report.append("|---|---|---|---|---|---|---|---|---|---|---|---|")
    ...
    for r in results:
        ...
        fqg_q = r.get("fqg_quadrant", "—")
        report.append(
            f"| {r['name']} | {mpmath.nstr(r['r_mp'], 8)} | {mpmath.nstr(r['r_mp'], 8)} | {r['k_r_12']} | "
...
if __name__ == "__main__":
    print("=" * 70)
    print("SEMPAEVUM N/Z STABILITY VIEWER — Valley of Nuclear Stability on the Lattice")
    ...
    print(f"      Tower: infinite LCM escalation with d-family stabilization ({len(LCM_LANDMARKS)} landmarks available)")
    ...
    html_path = "/mnt/user-data/outputs/Sempaevum_NZ_stability_viewer.html"
    ...
    report_path = "/mnt/user-data/outputs/sempaevum_nz_stability_report.md"
