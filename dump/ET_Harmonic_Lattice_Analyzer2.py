...
_CMDS = {"note","ratio","midi","harmonics","families","scale","scala",
         "compare","elegance","selfproject","lattice2freq","freqs","scales"}

def cli():
    if len(sys.argv) < 2:
        print("  Lossless frequency↔note converter (Sempaevum bijection, 1200-digit)")
        print("  Give it input — it converts:\n")
        print("    tool.py 440                Hz → note")
        print("    tool.py 261.63 329.63 440  batch Hz → notes")
        print("    tool.py 3/2                ratio → cents + note")
        print("    tool.py C4                 note → Hz")
        print("    tool.py A#3 Bb5 F#4        notes → Hz\n")
        print("  Deep commands: scale scala compare harmonics families selfproject elegance scales")
        print(f"  {len(SCALE_REGISTRY)} tuning systems. 'scales' to list. '<n>tet' for any equal temperament.")
        return
    # Bare args (not a command keyword) → auto-convert
    if sys.argv[1].lower() not in _CMDS:
