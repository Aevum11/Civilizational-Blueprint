...
<style>
    ...
    /* Custom LaTeX Tag Styling (Invisible to user, found by JS) */
    force-math { display: inline; visibility: visible; }
</style>
"""

class NuclearMathPreprocessor(Preprocessor):
    """
    The Nuclear Option Preprocessor:
    1. Scans raw text for LaTeX delimiters.
    2. Wraps them in custom <force-math> tags.
    3. Hides them from Markdown logic completely.
    4. Uses raw strings to prevent SyntaxWarnings.
    """
    def run(self, lines):
        new_lines = []
        
        # Regex patterns using raw strings r""
        # We look for $$...$$, \[...\], \(...\), and $...$
        # The goal is to wrap them in <force-math>...</force-math>
        
        for line in lines:
            # 1. Clean invisible garbage
            clean_line = line.replace('\xa0', ' ').replace('\u200b', '')
            
            # 2. Substitution Strategy
            # We must be careful with order. Block math first.
            
            # Pattern: \[ ... \] -> <force-math>\[ ... \]</force-math>
            # r'(\\\[.*?\\\])' matches \[...\] non-greedily
            clean_line = re.sub(r'(\\\[.*?\\\])', r'<force-math>\1</force-math>', clean_line)
            ...
            # Pattern: $ ... $ (Simple inline)
            # r'(?<!\\)(\$.*?\$)' matches $...$ not preceded by \
            # We risk false positives here but "Nuclear" means we accept risk to force display.
            # We filter out empty $$.
            # clean_line = re.sub(r'(?<!\\)(\$[^\$]+?\$)', r'<force-math>\1</force-math>', clean_line)
...
class NuclearMathExtension(Extension):
    def extendMarkdown(self, md):
        # Register with highest priority to run before any other block processing
        md.preprocessors.register(NuclearMathPreprocessor(md), 'nuclear_math', 200)
...
class MarkdownViewer(QMainWindow):
    ...
    def load_markdown(self, markdown_text, base_path=None, title="Untitled", file_path=None, view=None):
        try:
            extensions = [
                NuclearMathExtension(), # The new engine
            ...
            # We remove arithmatex completely to avoid conflicts.
            # Our Nuclear preprocessor handles the delimiters now.
            extension_configs = {
        ...
        <body class="{body_class}">
            ...
            <script>
                // MATH HUNTER ENGINE
                // 1. Finds <force-math> tags and unwraps/typesets them.
                // 2. Scans text nodes for lost sheep and typesets them.
                window.onload = function() {{
                    if (!window.MathJax) return;

                    // PHASE 1: PRE-TAGGED MATH (From Python)
                    const explicitMath = document.getElementsByTagName('force-math');
                    const nodesToProcess = [];
                    
                    // Convert live collection to array immediately
                    const mathArray = Array.from(explicitMath);
                    ...
                    // PHASE 2: HUNTER-KILLER (Scan for missed delimiters in text nodes)
                    ...
                    while(currentNode = treeWalker.nextNode()) {{
                        ...
                        if (val.includes('\\\\(') || val.includes('\\\\[') || val.includes('$$')) {{
                            ...
                            // Check if inside code block, we render it anyway as per instruction.
                    ...
                    if (nodesToProcess.length > 0) {{
                        console.log("MathHunter: Processing " + nodesToProcess.length + " elements.");
                        window.MathJax.typesetPromise(nodesToProcess).catch((err) => {{
