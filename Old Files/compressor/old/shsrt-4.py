...
class ArchetypeDatabase:
    ...
    def default_db_path() -> str:
        """
        Platform-appropriate persistent database path.

        The database survives reboots and updates. Located in the user's
        home directory under .cdf_compressor/ — the compressor's long-term
        memory lives alongside the user's data, not in volatile temp.
        """
        home = Path.home()
        db_dir = home / '.cdf_compressor'
        db_dir.mkdir(parents=True, exist_ok=True)
        return str(db_dir / 'archetypes.db')
