...
export default function N12Hasse() {
  ...
  const t = dark ? {
    bg: "#0d1117", title: "#e6edf3", sub: "#c0a868", desc: "#8b949e",
    sFill: "#c9d1d9", sStroke: "#58a6ff", sText: "#0d1117", sPhi: "#586069",
    sForce: "#e6edf3", sPhase: "#b1bac4", sXi: "#c0a868",
    cFill: "#161b22", cStroke: "#8b949e", cText: "#c9d1d9",
    cForce: "#c9d1d9", cPhase: "#8b949e", cHost: "#ff9d3d",
    hasse: "#484f58", mirror: "#bc8cff", hosting: "#ff9d3d",
    ucLine: "#ffab40", green: "#4caf50", red: "#ef5350",
    cascBg: "#0d1117", stepN: "#8b949e", panelM: "#8b949e",
    ann: "#b1bac4", legend: "#8b949e", summary: "#8b949e",
    footer: "#484f58", footBorder: "#21262d", selfMirror: "#bc8cff",
    structTitle: "#e6edf3",
  } : {
    bg: "#faf8f4", title: "#1a1a2e", sub: "#8a7a5a", desc: "#6a6a7a",
    sFill: "#1a1a2e", sStroke: "#3a3a5e", sText: "#ffffff", sPhi: "#aaaacc",
    sForce: "#1a1a2e", sPhase: "#5a5a6a", sXi: "#8a7a5a",
    cFill: "#f0ede8", cStroke: "#6a6a7a", cText: "#3a3a4a",
    cForce: "#4a4a5a", cPhase: "#7a7a8a", cHost: "#e65100",
    hasse: "#2a2a3a", mirror: "#7b1fa2", hosting: "#e65100",
    ucLine: "#ff6f00", green: "#1b5e20", red: "#c62828",
    cascBg: "#faf8f4", stepN: "#aaa", panelM: "#999",
    ann: "#5a5a6a", legend: "#4a4a5a", summary: "#6a6a7a",
    footer: "#8a8a9a", footBorder: "#d0ccc4", selfMirror: "#7b1fa2",
    structTitle: "#1a1a2e",
  };
  const W = 680, H = 1260;
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: 8, position: "relative" }}>
        ...
        <div style={{ color: t.title, fontSize: 15, fontWeight: "bold" }}>N=12 · τ=6 · 6/12 active · ξ(m) = 137/((m−1)²+16)</div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, margin: "0 auto", display: "block" }}>
        ...
        {hasse.map(([d1, d2], i) => {
          ...
          return <line key={`h${i}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y}
            stroke={t.hasse} strokeWidth="1.8" opacity="0.1" />;
        })}
        ...
        {mirrors.map((mp, i) => {
          ...
          return <line key={`m${i}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y}
            stroke={t.mirror} strokeWidth="1" strokeDasharray="5,4" opacity="0.15" />;
        })}
        ...
        {complex.map(c => {
          ...
          return <line key={`s${c.m}`} x1={c.x} y1={c.y} x2={h.x} y2={h.y}
            stroke={t.hosting} strokeWidth="0.9" strokeDasharray="2,2" opacity="0.3" />;
        })}

        {/* ── m=6 SELF-MIRROR ── */}
        <circle cx={420} cy={280} r={30} fill="none"
          stroke={t.selfMirror} strokeWidth="0.8" strokeDasharray="3,2" opacity="0.3" />
          ...
          return (
            <g key={`uc${i}`}>
              <line
                ...
                stroke={t.ucLine} strokeWidth="1.4" opacity="0.5" strokeDasharray="6,3"
                markerEnd="url(#aO)" />
        ...
        <text x={480} y={500} fill={t.ucLine} fontSize="6" opacity="0.7">
          m=3 excluded
        </text>
        <text x={480} y={509} fill={t.ucLine} fontSize="5.5" opacity="0.6">
          from U(1) traversal
        </text>
        ...
        {complex.map(c => (
          <g key={`cn${c.m}`}>
            <circle cx={c.x} cy={c.y} r={R_SHADOW}
              fill={t.cFill} stroke={t.cStroke} strokeWidth="1.3" strokeDasharray="4,2" />
                ...
                {pts.map((p, i) => (
                  <g key={`cm${casc.g}n${i}`}>
                    <circle cx={p.x} cy={p.y} r={6}
                      fill={t.cascBg} stroke={casc.col} strokeWidth="1.2" />
        ...
        {/* ═══ LEGEND ═══ */}
        <text x={340} y={1120} fill={t.structTitle} fontSize="9" fontWeight="bold" textAnchor="middle">LEGEND</text>

        <circle cx={35} cy={1140} r={6} fill={t.sFill} stroke={t.sStroke} strokeWidth="1.5" />
        <text x={48} y={1143} fill={t.legend} fontSize="6">Simple (m|12)</text>
        <circle cx={160} cy={1140} r={6} fill={t.cFill} stroke={t.cStroke} strokeWidth="1" strokeDasharray="4,2" />
        <text x={173} y={1143} fill={t.legend} fontSize="6">Complex (m∤12)</text>
        <line x1={285} y1={1140} x2={297} y2={1140} stroke={t.hasse} strokeWidth="1.5" />
        <text x={303} y={1143} fill={t.legend} fontSize="6">Hasse</text>
        <line x1={360} y1={1140} x2={372} y2={1140} stroke={t.hosting} strokeWidth="0.8" strokeDasharray="2,2" />
        <text x={378} y={1143} fill={t.legend} fontSize="6">Shadow hosting</text>
        <line x1={475} y1={1140} x2={487} y2={1140} stroke={t.mirror} strokeWidth="0.8" strokeDasharray="5,4" />
        <text x={493} y={1143} fill={t.legend} fontSize="6">Mirror</text>

        <circle cx={35} cy={1158} r={8} fill="none" stroke={t.selfMirror} strokeWidth="0.7" strokeDasharray="3,2" />
        <text x={48} y={1161} fill={t.legend} fontSize="6">Self-mirror (m=6)</text>
        <line x1={160} y1={1158} x2={172} y2={1158} stroke={t.ucLine} strokeWidth="1.2" strokeDasharray="6,3" />
        <text x={178} y={1161} fill={t.legend} fontSize="6">U(1) traversal</text>
        <line x1={285} y1={1158} x2={297} y2={1158} stroke={t.green} strokeWidth="1" />
        <text x={303} y={1161} fill={t.legend} fontSize="6">κ=0 (D-arith, E{">"}1)</text>
        <line x1={400} y1={1158} x2={412} y2={1158} stroke={t.red} strokeWidth="1" />
        <text x={418} y={1161} fill={t.legend} fontSize="6">κ≠0 (T-act, E{"<"}1)</text>

        <rect x={28} y={1172} width={12} height={8} rx={2} fill="#1565c0" opacity="0.15" stroke="#1565c0" strokeWidth="0.5" />
        <text x={46} y={1179} fill={t.legend} fontSize="6">(5,7) non-trivial pair</text>
        <rect x={160} y={1172} width={12} height={8} rx={2} fill="#2e7d32" opacity="0.15" stroke="#2e7d32" strokeWidth="0.5" />
        <text x={178} y={1179} fill={t.legend} fontSize="6">(1,11) trivial pair</text>

        <text x={340} y={1200} fill={t.footer} fontSize="6" textAnchor="middle">
