...
═══════════════════════════════════════════════════════════════════
THEOREM C.4 (d=1 Self-Composition — Universal d=1 Channel).
For every sublattice family d at N=12:
  1 ∈ d ⊗ d

Every family's self-composition ALWAYS includes the d=1 family.

PROOF: Since Res(d) is symmetric (Theorem C.3), for any k ∈ Res(d),
  (N−k) ∈ Res(d). Their sum k + (N−k) = N ≡ 0 mod N.
  gcd(0, N) = N, so d = N/N = 1.  ∎

HARMONIC-LAYER READING (via Sublattice Visitation Theorem):
  At N=12, the d=1 sublattice family is where the gravity harmonic
  family inhabits. Therefore this sublattice composition theorem has
  the harmonic-layer consequence that the gravitational channel is
  always structurally available through self-composition. But the
  THEOREM is about sublattice families — the gravity identification
  belongs to the harmonic layer and is attached via the Visitation
  Theorem, not by the sublattice arithmetic itself.

═══════════════════════════════════════════════════════════════════
THEOREM C.5 (d=12 Universality — Complete Self-Composition).
d=12 (the coprime sublattice family) satisfies:
...
HARMONIC-LAYER READING (via Sublattice Visitation Theorem):
  At N=12, d=12 is where the EM harmonic family inhabits. The
  sublattice universality theorem therefore has the harmonic-layer
  consequence that EM-family self-interaction can produce any
  harmonic family. But the THEOREM is about sublattice families —
  the EM identification is a harmonic-layer attribution.
...
# Property 1: d=1 Self-Composition Channel (d ⊗ d always includes 1)
grav_pass = True
for d in families:
    if 1 not in comp_table[(d, d)]:
        grav_pass = False
        print(f"  d=1 CHANNEL VIOLATION: d={d} ⊗ d={d} does not include 1!")

print(f"  Theorem C.4 (d=1 Self-Composition): d ⊗ d ∋ 1 for all d: {'✓ VERIFIED' if grav_pass else '✗ FAILED'}")

# Property 2: d=12 Universality (12 ⊗ 12 = all families)
em_complete = comp_table[(12, 12)] == set(families)
print(f"  Theorem C.5 (d=12 Universality): 12 ⊗ 12 = all families: {'✓ VERIFIED' if em_complete else '✗ FAILED'}")
...
print(f"""
  KEY OBSERVATION: d=12 under squaring produces d=6 (deterministic),
  because k→2k mod 12 maps {{1,5,7,11}} to {{2,10}} which gives d=6.
  Cubing d=12 produces d=4: 3·{{1,5,7,11}} mod 12 = {{3,9}} → d=4.
  (Harmonic-layer reading via Sublattice Visitation Theorem:
  squaring the EM sublattice → hexadic sublattice; cubing → quartic.
  The force identifications EM/hexadic/quartic/weak belong to the
  harmonic family layer, not the sublattice arithmetic itself.)
""")
...
print(f"""
  ...
  d=1 self-composition (Thm C.4):    {'✓ PASS' if grav_pass else '✗ FAIL'}  (d⊗d ∋ 1 for all d)
  d=12 universality (Thm C.5):       {'✓ PASS' if em_complete else '✗ FAIL'}  (12⊗12 = all families)
  ...
  Key sublattice structural results for field engineering:
  • d ⊗ d ∋ 1 for all d (the d=1 channel is always available;
    at the harmonic layer this is why gravity couples universally)
  • d=12 ⊗ d=12 = all families (coprime sublattice is the universal
    mixer; at the harmonic layer this is EM universality)
  • d=1 is the identity under κ=0 composition
  • Division and multiplication have IDENTICAL composition sets
    (residue symmetry)
  • Power sequence d=12 → d=6 → d=4 → d=3 → d=12 → d=2 → ...
    (at the harmonic layer: EM → hexadic → weak → strong → EM → ...)
  
  NOTE: All force/phase identifications (gravity, EM, strong, weak,
  etc.) belong to the HARMONIC FAMILY layer and attach to sublattice
  families via the Sublattice Visitation Theorem. The theorems in
  this script are about SUBLATTICE families — pure gcd arithmetic.
