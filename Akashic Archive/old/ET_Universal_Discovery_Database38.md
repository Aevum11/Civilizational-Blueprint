...
|---|---|---|---|
| **1 — Precision Stack** | 0 | **VERIFIED ✓** | 62/62 tests passed. Files: `precision_stack.h` (665), `precision_stack.cpp` (1525), `main.cpp` (892), `CMakeLists.txt` (131). Total: 3213 lines. 1200-bit/361-dps. ETInteger arbitrary-precision (zero overflow paths). 25 [[nodiscard]]. N-Weight Subsumption shadow test (12ET + 27720ET). Expression tree evaluator deferred to Stage 1b. Added: `ETValue::from_integer(ETInteger)` and `ETInteger::from_etvalue(ETValue)` — direct GMP↔MPFR bridges via `mpfr_set_z`/`mpfr_get_z`, zero string intermediary, zero int64 intermediary. |
| **2 — Core Lattice Engine** | 1 | **VERIFIED ✓** | 87/87 tests passed (62 Stage 1 + 25 [[nodiscard]] exports + 87 Stage 2 assertions — 87 total Stage 2). Files: `core_lattice.h` (403), `core_lattice.cpp` (789). Projection Π_N(r)→(k,d,ε) with bijection-teleporter ε computation: computes lattice point L_k=2^(k/N) via pullback, compares r==L_k directly (MPFR correct rounding guarantees match for on-lattice values → ε=0 exactly), off-lattice uses stable ratio formula ε=1200·log₂(r/L_k) (zero catastrophic cancellation). k determined at 2×precision (2400-bit). Bijection pullback Π_N⁻¹. k-arithmetic (add/negate/scale, all ETInteger). All derived properties: elegance (symmetry+simplicity+universal), coupling ξ(d) dynamic for any d (no cap at 12), variance V(n,k), tightness, ∂I distance, Gaussian signature (R/I/S over ℤ[i]), FQG quadrant (SR/CR/SI/CI), palindromic partner, manifold state, d_factorization, coprime skeleton, quintic tension, rational approximation via continued fractions. Zero int64 computation paths — all GMP/MPFR native. C++20 nested namespaces. |
