...
mp.dps = 400  # Working precision — 400 decimal places
# STRUCTURAL DERIVATION OF MINIMUM PRECISION:
#   The lattice has 1200 cents per octave.
#   To resolve the ε residual with full binary precision across
#   the entire octave requires 1200 binary digits minimum:
#     1200 binary digits × log₁₀(2) ≈ 361 decimal places
#   Plus 39 guard digits for intermediate computation = 400 dps total.
#   This pushes the computational residual to ~10⁻³⁹⁵, far below
#   any physical or mathematical relevance. The bijection itself
#   has ZERO error — only computation introduces residual.
GUARD_DPS = 39  # Guard digits beyond 1200-binary-digit minimum
WORK_DPS = mp.dps
...
class ETProjector:
    ...
    def verify_roundtrip(self, r_str):
        ...
        is_exact = rel_error < mppow(mpf(10), -395)
...
class ETAudioProjector:
    ...
    def verify_sample(self, sample_str):
        ...
        is_lossless = rel_error < mppow(mpf(10), -390)
...
class ETTowerProjector:
    ...
    def project_tower_via_transition(self, r_str):
        ...
        for N in self.tower_levels[1:]:
            ...
            k_match = k_d == k_t
            d_match = d_d == d_t
            eps_match = fabs(eps_d - eps_t) < mppow(mpf(10), -350)
...
class ETVerificationSuite:
    ...
    def test_cross_resolution(self, verbose=True):
        ...
        for val_str in test_values:
            for N1, N2 in tower:
                ...
                eps_ok = fabs(eps2_direct - eps2_trans) < mppow(mpf(10), -350)
    ...
    def test_lattice_arithmetic(self, verbose=True):
        ...
        # All string representations at WORK_DPS to match projection precision
        test_pairs = [
        ...
        for v1, v2 in test_pairs:
            ...
            # Multiplication — product string at full WORK_DPS precision
            prod_val = nstr(mpf(v1) * mpf(v2), WORK_DPS)
            k_d, d_d, eps_d = self.projector.project(prod_val)
            k_a, d_a, eps_a, kappa = arith.multiply(k1, eps1, k2, eps2)
            mult_ok = (k_d == k_a and d_d == d_a and fabs(eps_d - eps_a) < mppow(mpf(10), -350))
            
            # Division — quotient string at full WORK_DPS precision
            div_val = nstr(mpf(v1) / mpf(v2), WORK_DPS)
            k_dd, d_dd, eps_dd = self.projector.project(div_val)
            k_ad, d_ad, eps_ad, kappa_d = arith.divide(k1, eps1, k2, eps2)
            div_ok = (k_dd == k_ad and d_dd == d_ad and fabs(eps_dd - eps_ad) < mppow(mpf(10), -350))
    ...
    def test_sign_encoding(self, verbose=True):
        ...
        for val_str in test_values:
            ...
            value_correct = rel_err < mppow(mpf(10), -390)
    ...
    def test_backbone_bridge(self, verbose=True):
        ...
        for name, val_str in test_values:
            ...
            k_match = k_dir == k_bb
            d_match = d_dir == d_bb
            eps_match = fabs(eps_dir - eps_bb) < mppow(mpf(10), -395)
    ...
    def test_d_family_composition(self, verbose=True):
        ...
        for v1, v2 in test_pairs:
            ...
            k2, d2, eps2 = self.projector.project(v2)
            k_a, d_a, eps_a, kappa = arith.multiply(k1, eps1, k2, eps2)
            prod_str = nstr(mpf(v1) * mpf(v2), WORK_DPS)
    ...
    def test_boundary_identity(self, verbose=True):
        ...
        hc_pass = fabs(half_cell - expected_half) < mppow(mpf(10), -390)
