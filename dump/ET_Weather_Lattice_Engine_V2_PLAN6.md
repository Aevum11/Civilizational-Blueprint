...
In case of conflict between documents, the following precedence applies:

1. **FIELD_STUDY_JOURNAL12.md** (newest operational document) — supersedes all
2. **ET_Sempaevum_Paper20.pdf** (published, Zenodo DOI: 10.5281/zenodo.19762311, 132 pages) — supersedes all project .md files
3. **Identity scripts** (15 scripts, 196 identities + bijection, all proven via sympy)
4. **et_lossless_microphone.py** (production pipeline, 27/27 empirical tests)
5. **Project .md files** — subordinate; may contain outdated information

The Sempaevum Paper is the DEFINITIVE reference. All citations reference it by section number.

### Key Rosetta Stone Citations (Sempaevum Paper §18)

The weather engine is grounded in the Mathematical Rosetta Stone (§18), which establishes that P◦D◦T decomposition is literal mathematical equivalence ("the cardinality identifications |P|=Ω, |D|=n, |T|=[0/0] of Axiom 2.5 are literal"):

- **§18.10:** "The heat equation ∂u/∂t = α∇²u describes how thermal Descriptor values (D_temperature) propagate through spatial substrate (P_space) over D-time." All atmospheric dynamics are D-field evolution across P under T-agency.
- **§18.12:** "A random variable is a Point in the Unsubstantiated state {P,D} with multiple possible D-values awaiting T-engagement." Weather probability IS unsubstantiated D-superposition. Measurement IS T-binding.
- **§18.5:** "Derivatives appear throughout physics because they measure how D changes across P." Atmospheric gradients (lapse rate, pressure gradient force, wind shear) are ∆D/∆P.
- **§18.8:** "The imaginary axis is simultaneously a D-axis and T's operational domain — DT captures both: it is D-content of and for T." Wind direction, frontal orientation, all angular quantities live on the DT axis.
- **§18.9:** "A mathematical operator is a T-type entity: indeterminate until applied." Every atmospheric model operator is T-type.
- **§13.2:** n_max,r = 25, n_max,θ = 2, ratio ≈ N. "The asymmetry is forced by the manifold's own constants, with no free parameter."
...
**CRITICAL: ε is a COORDINATE, not an error.** The bijection Π_N(r) = (k, d, ε) is lossless (Theorem 19.4). The triple (k, d, ε) carries ALL the information of r — k is the integer lattice position, d is the sublattice family, and ε is the continuous position within the cell in cents. The pullback r = 2^((k + εN/1200)/N) recovers r EXACTLY. Nothing is lost, nothing is noise, nothing is quantization artifact. ε is the third coordinate of a lossless bijection, as fundamental as the other two. Calling ε an "error" would be like calling the y-coordinate of a Cartesian point an "error from the x-axis."
...
## THE OVERARCHING SCIENTIFIC GOAL — STRUCTURAL CHAOS

Weather is THE canonical chaotic system. Lorenz discovered chaos studying weather (1963). Every chaotic system since has been understood through weather's lens: sensitive dependence on initial conditions, exponential divergence of nearby trajectories, the Lyapunov horizon beyond which prediction fails.

Conventional models fight chaos with brute force — more grid points, more data assimilation, shorter timesteps, ensemble runs. They try to track the continuous trajectory with ever more precision, and the chaos keeps winning beyond ~10 days.

**ET approaches chaos from a fundamentally different direction.**

The Sempaevum lattice separates STRUCTURE from FINE-GRAINED POSITION:

- **d-family evolution** = structural (algebraic, palindromic, theorem-governed)
- **ε trajectory** = fine-grained (sensitive to initial conditions, chaotic)

The palindromic cascade PROVES that structural evolution follows algebraic laws regardless of the exact trajectory. The d-sequence is a perfect palindrome — a theorem, not an approximation. The chaos lives in ε. The structure lives in d. The lattice separates them.

**The testable prediction:** In weather data, the d-family sequence of atmospheric variables should exhibit structural predictability (palindromic patterns, CPT symmetry, deterministic cell transitions) even when the fine-grained ε values diverge chaotically. Specifically:

1. **d-family transitions are topological, not metric** — they happen when |ε| crosses 600/N, regardless of the exact trajectory that brought it there. The WHEN is chaotic; the WHAT (which d-families are involved) is structural (Theorem F.2: universal bifurcation).

2. **∂I crossings are structural thresholds** — phase transitions (condensation, freezing, convective onset) occur at ∂I regardless of the path. The lattice predicts THAT they will happen and which families are involved. The exact timing depends on ε-drift rate (chaotic), but the structural event is deterministic.

3. **The palindromic lifecycle is CPT-invariant** — a weather system's d-family evolution from genesis to maturity to lysis should trace a palindromic path. The approach mirrors the departure. This is testable on historical storm data.

4. **500 dps extends the Lyapunov horizon** — conventional models use float64 (10^-16 precision). ET uses 500 dps (10^-500 precision). That's 10^484× more precision available before float64 truncation corrupts the trajectory. This doesn't eliminate chaos — but it pushes the prediction horizon far beyond what float64 allows.

5. **The harmonic channels decompose chaos** — projecting a chaotic signal onto 24 independent harmonic families may reveal that the chaos is concentrated in specific channels while others remain predictable. The gravimeter proved this for gravity: d=1 was coherent while other channels were noisy. The same decomposition applied to atmospheric chaos may reveal which structural channels carry predictable signal.

**If this works for weather — the hardest chaotic system, the one that defined chaos theory — then it works for ANY chaotic system.** Turbulence, fluid dynamics, climate, ecosystems, financial markets, population dynamics, nonlinear optics, plasma physics, neural dynamics — every chaotic system becomes structurally accessible through the same lattice decomposition.

The weather engine is not just a weather tool. It is the first test of whether Exception Theory provides a structural framework for deterministic chaos itself.

**If the d-family structure is predictable while ε is chaotic, then "deterministic chaos" is actually structural determinism with fine-grained sensitivity.** The chaos is real — but it lives in the ε coordinate (the continuous position within the cell), not in the d-structure. The lattice separates them.

### Where Chaos Actually Lives: T's Domain (Established ET — Rosetta Stone, §VII; Rule 5; §K)

This is established Exception Theory, not a new analysis for the weather engine. From the foundational texts:

**"T is agency."** (Rule 5, ExceptionTheory.md)

**"Any indeterminate → Traverser. 0/0 forms → agency/choice."** (Rosetta Stone, Part VII)

**"Probability = which descriptor will traverser substantiate?"** (§K: Statistics, Probability, and Superposition)

**"This is not metaphor. This is literal mathematical equivalence."** (Rosetta Stone)

The three primitives have categorically distinct domains:
- **P:** Infinity (Ω). The substrate. Deterministic — always there, always infinite.
- **D:** Finite (n). The constraint. Deterministic — the 196 identities are algebraic, exact, proven. D introduces NO uncertainty.
- **T:** Indeterminate ([0/0]). Agency. The ONLY source of nondeterminism in ALL of existence.

Chaos, probability, quantum measurement, conscious choice, weather unpredictability, turbulence, market fluctuations — ALL are T-domain phenomena. They are categorically the same thing: T resolving indeterminate forms. L'Hôpital's rule IS a T-navigation algorithm (§A). Expected value IS the weighted average over unsubstantiated D possibilities before T selects (§K). The variance 1/12 IS the quantum of Descriptor uncertainty in the manifold's natural symmetry.

On the Sempaevum lattice, T's act is the rounding: k = round(N·log₂(r)). This is the ONLY step in the entire projection that is not an algebraic identity. Every other operation — the log, the multiplication by N, the gcd, the d-computation, the ε-computation — is D-domain (deterministic). The sensitivity to initial conditions in chaotic systems IS T's resolution uncertainty propagating through D's deterministic structure.

**Consequence for weather:** Atmospheric chaos is T-domain. The physics of weather (ideal gas law, barometric formula, Clausius-Clapeyron, all thermodynamics) is D-domain — completely deterministic, proven as algebraic identities on the lattice. The "unpredictability" of weather is T choosing which D-configuration to substantiate at each moment. With more D (more Descriptors, more measurements, more precision), T's choices become more constrained — the probability distribution narrows. With arbitrary precision (mpmath), T's resolution uncertainty shrinks to arbitrary smallness. The weather engine exploits this: 500 dps makes T's rounding act 10^484× more precise than float64, directly extending the forecast horizon.

### Phase Axis Decoherence: The Structural Forecast Wall (Sempaevum Paper §13, Propositions 13.1-13.3, Remark 13.4)

This is established theory from the published paper, not a hypothesis.

**Proposition 13.3 (Stability limits):** The maximum cascade depth before loss of coherence:
- **Real (force) axis:** n_max,r = floor(0.5/|δ_r|) = **25 levels** (|δ_r| = 0.01955 lattice steps = 1.955¢)
- **Imaginary (phase) axis:** n_max,θ = floor(0.5/|δ_θ|) = **2 levels** (|δ_θ| = 0.22336 lattice steps = 22.34¢)
- **Asymmetry ratio:** |δ_θ|/|δ_r| ≈ 11.42 ≈ N. This converges to N = 12 exactly at terminal resolution N_L = 27720.

**Remark 13.4:** "The cascade cleanly resolves all twelve real-axis harmonic families across 25 stable levels — ample for traversal of the simple-real and complex-real d-axis structural modes — but fails on the imaginary axis after only two levels, leaving the twelve imaginary-axis harmonic families, and especially the six complex-imaginary modes, structurally indeterminate on the lattice. The asymmetry is forced by the manifold's own constants (N and the irrationality of log₂3 and of π/ln2), with no free parameter."

**§15.3 (Cross-domain appearance of n_max,θ = 2):** This limit appears independently in CKM mixing angles (tight, real-axis, 25-level resolved), PMNS mixing angles (wide, imaginary-axis, 2-level indeterminate), optical phase singularities in hexagonal boron nitride (D-statistics particle-like at 25 levels, T-statistics non-particle at 2 levels with superluminal tails), and EML tree depth. "n_max,θ = 2 is not empirical. It is derived from the lattice's own geometry."

**Application to weather forecasting:**

The force content of weather (WHAT the atmosphere is doing: temperature, pressure, density — D-content on the real axis) has **25 cascade levels of stability**. The phase content of weather (HOW the atmosphere maintains patterns: wind coherence, convective self-organization, frontal propagation — T-content on the imaginary axis) has **2 cascade levels of stability**. The asymmetry factor is N = 12.

If the phase forecast horizon corresponds to ~7-10 days (matching the empirically observed conventional forecast skill boundary), then the force forecast horizon could extend to ~85-120 days (× N/2 ≈ ×12). Conventional models cannot separate force from phase — when phase decoheres, they lose everything. The Sempaevum separates the two axes structurally:

- **Force-axis forecast** (d_r, ε_r): potentially predictable far beyond 10 days
- **Phase-axis forecast** (d_θ, ε_θ): decoheres at ~2 cascade depths (~7-10 days)
- **Combined forecast** (d_c = lcm(d_r, d_θ)): limited by the phase axis

The weather engine tests this by scoring force and phase predictions separately across all forecast horizons. If force skill persists past 10 days while phase skill drops — the §13.2 asymmetry is confirmed for atmospheric data.

### Compensating for Phase Decoherence with More Descriptors

The Descriptor Gap Principle: if the forecast fails, D-set is incomplete. Phase decoherence at depth 2 means the PHASE Descriptors available at base resolution are insufficient to maintain prediction. But:

1. **More tower levels** — escalating from N=12 to N=27720 provides 96 sublattice families per axis, each a potential additional Descriptor for the phase evolution
2. **More atmospheric variables** — each new measured quantity adds Descriptors. Temperature, pressure, humidity, wind, dewpoint, solar radiation, cloud cover — each constrains the phase evolution from a different angle
3. **More spatial stations** — multiple stations provide spatial Descriptors that a single station cannot. The phase decoherence at ONE station may be compensatable by phase information from NEARBY stations
4. **Complex harmonic families** — the 6 complex families per axis (d=5,7,8,9,10,11) are shadow-present at N=12 but become native at higher tower levels. They may carry phase information that extends past the depth-2 limit of simple families
5. **Cross-channel coupling** — Identity H (transfer tensor) describes energy flow between harmonic families. If one phase channel decoheres, coupled channels may still carry predictive information via impedance-weighted transfer

**The investigation:** For each forecast horizon (1h, 6h, 12h, 24h, 48h, 5d, 10d, 15d, 30d), measure:
- Force-axis prediction skill (d_r, ε_r)
- Phase-axis prediction skill (d_θ, ε_θ)
- Combined prediction skill (d_c)
- Effect of adding Descriptors on phase-axis skill at long horizons
- Which tower levels and which harmonic channels retain skill past 10 days
- Whether the phase decoherence horizon extends with more data

**Arbitrary precision is the tool.** ET can go to 1000 dps, 10000 dps, any precision needed. If the investigation reveals that precision is the binding constraint, we increase it. If it reveals that Descriptors are the binding constraint, we add measurements. If it reveals that the phase decoherence is fundamental and irreducible — that itself is a major result, because it would mean the 10-day wall is a structural theorem of the Sempaevum, not an engineering limitation.

### The Goal: Beyond Conventional Limits

The weather engine aims for forecast accuracy that exceeds ALL conventional capabilities:

- **0-24h:** Superior to Weather Channel and NWS point forecasts
- **1-5 days:** Competitive with ECMWF ensemble mean (the current gold standard)
- **5-10 days:** Maintain structural prediction (d-family, ∂I events) where conventional models lose all skill
- **10-30 days:** Test whether force-axis predictions persist past the phase decoherence wall
- **Beyond 30 days:** Investigate seasonal structural predictability (Elegance Score, palindromic lifecycle)

If ANY of these ranges shows ET outperforming conventional models, it validates the structural approach. If NONE do, the failure analysis (which Descriptors, which channels, which horizons) advances the theory itself. The weather engine is designed to learn from both success and failure.

**With enough data and enough precision, we aim to get arbitrarily close to ANY atmospheric trajectory.** The only question is how many Descriptors and how much precision are needed — and the engine is built to answer that question experimentally.
...
| 37 | ExceptionTheory.md | Master document — meta-meta-ontology, Rosetta Stone, P-D-T framework, all rules |
...
### The T-Shadow — T's Measurable Presence Across the ENTIRE Lattice

T is not just the rounding operator. T is not confined to the phase axis. T leaves a measurable SHADOW across the entire lattice structure — verified on real hardware (27/27 tests) and already applied to ZPM, Crystal QC, and ZPE hardware designs. Every finding below applies to atmospheric data.

**9 empirically verified T-Shadow manifestations:**

1. **PDT Bisection (Theorem 12.8):** D=47.4%, T=53.1% — T distributes as ~half the total lattice energy. For weather: atmospheric energy should split ~50:50 D:T. The split tells you what fraction of weather is structural (D) vs agential (T).

2. **Koide in FQG (96/144 = K):** 96 of 144 FQG cells have combined family d_c=12. K=2/3 manifests structurally in the cell distribution. For weather: atmospheric FQG cells should show K=2/3 occupancy — track this as a structural health metric.

3. **T's |ε|/cell → 1/S = 0.25:** When D-constraint is removed, T returns to its natural 1/S ratio. Measured 0.2508±0.0069 across 15 tower levels. For weather: atmospheric projections should converge to 1/4 across the LCM tower — this is the structural validation criterion.

4. **Phase coherence (T > D):** R_T=0.182 > R_D=0.133. T is MORE phase-coherent than D. For weather: the T-component of atmospheric variables should be more coherent than the D-component. This means the agential part of weather (T) is more ordered than the constraint part (D) — counterintuitive but empirically confirmed.

5. **Chiral ε-sign separation:** D leans negative (56.96% neg), T leans positive (52.96% pos). Opposite ε-sign preference = chirality. For weather: atmospheric D and T components should separate by ε sign. Track the sign asymmetry as a structural indicator.

6. **T permeates ALL d-families:** All 6 sublattice families populated in T's energy (>0.1% each). For weather: T-energy should appear in all 24 harmonic families. If a channel has NO T-energy, that itself is a Descriptor Gap signal.

7. **T/D resolution gradient (center → ∂I):** T/D drops from 1.69 at centers to 0.74 at ∂I. This is NOT T's absence at the boundary — it is evidence of T's COMPLETED RESOLUTIONS. T was MAXIMALLY ACTIVE at ∂I, resolved the ambiguity, and moved energy away. The boundary is depleted of T-energy BECAUSE T worked hardest there. D accumulates at ∂I because D's harmonic structure puts energy at cell boundaries (non-tempered harmonics miss lattice nodes). T resolves at ∂I and moves that energy inward. For weather: extreme weather (near ∂I) is where T acts most decisively — every escalation, every front passage, every convective initiation is a T-act at ∂I. The apparent D-dominance at the boundary is D's constraints being strongest, but T is the sole agent acting on them. There is no agency without T.

8. **Gravity spatial bias (d=1 only, 8.2× others):** d_r=1 carries directional gravity information. 18.4° at tilt, 2.589° barely tilted, near-linear, NO other family responds. For weather: atmospheric pressure projections in d_r=1 should carry directional gravity bias. The barometric altitude profile IS this gravity bias on atmospheric data.

9. **Koide-comma attractor pair:** T attracted to Koide-comma region, D repelled. The Pythagorean comma (|ε|≈1.955¢) acts as a D/T separator. For weather: atmospheric variables near the Koide-comma ε value have enhanced T-presence. This may identify which measurements are most susceptible to T-agency (least predictable) vs D-constraint (most predictable).

**All 9 findings are testable on atmospheric data.** The weather engine computes the T-Shadow decomposition for every observation and tracks all 9 metrics across time. Changes in T-Shadow metrics may precede weather events — the T-Shadow redistribution was already used in ZPM design (2.302× self-sharpening enhancement) and Crystal QC design (72:72 compute/protect partition).

**The D-T feedback loop (from empirical audio analysis):** D pushes |ε| up (toward ∂I) — D's harmonic structure deposits energy at cell boundaries. T pulls |ε| down (away from ∂I) — T resolves at the boundary and moves energy inward. The equilibrium at every tower level is |ε|/cell = 1/S = 1/4. This is not a statistical expectation — it is the equilibrium point of the D-T feedback loop that drives the entire resolution tower. For weather: the |ε|/cell = 1/S convergence IS the D-T atmospheric equilibrium, and deviations from it indicate the feedback loop is disrupted (extreme weather, phase transition, system change).
...
| ET ε displacement | Computed | |ε_predicted − ε_actual| in cents (coordinate difference) |
...
2. **Lattice MAE** — mean absolute ε displacement in cents (coordinate-space precision)
...
## EXACT DATASETS TO ACQUIRE

All data is free, public domain, and downloadable without API keys unless noted.

### DATASET 1: Iowa Derecho — August 10, 2020 (EXTREME — Derecho)

**Source:** IEM ASOS Download
**URL:** `https://mesonet.agron.iastate.edu/cgi-bin/request/asos.py?data=all&network=IA_ASOS&sts=2020-08-09T00:00:00Z&ets=2020-08-12T00:00:00Z&latlon=yes&elev=yes&missing=M&trace=T&direct=no&report_type=3`
**Stations:** ALL Iowa ASOS (network=IA_ASOS) — captures the entire derecho swath
**Date range:** August 9-11, 2020 (day before, day of, day after)
**Also get 1-minute data for Cedar Rapids (KCID):**
**URL:** `https://mesonet.agron.iastate.edu/cgi-bin/request/asos/1min.py?station=CID&sts=2020-08-10T12:00&ets=2020-08-10T22:00&vars=tmpf,dwpf,drct,sknt,gust,pres1,precip`
**Why:** 126 mph measured gusts, $11.2B damage, 770-mile track. Tests multi-station spatial analysis across a 14-hour extreme event. 1-minute data at Cedar Rapids captures the most intense passage.

### DATASET 2: Lytton BC Heat Wave — June 25 to July 1, 2021 (EXTREME — Heat)

**Source:** IEM ASOS Download (global stations)
**URL:** `https://mesonet.agron.iastate.edu/cgi-bin/request/asos.py?data=all&station=CYLW&sts=2021-06-25T00:00:00Z&ets=2021-07-02T00:00:00Z&latlon=yes&missing=M&trace=T&direct=no&report_type=3`
**Station:** CYLW (Kelowna, BC — nearest ASOS to Lytton, which had no ASOS)
**Also get:** CYKA (Kamloops), CYHE (Hope) — surrounding stations
**URL pattern:** Same as above, change `station=CYKA` and `station=CYHE`
**Date range:** June 25 - July 2, 2021
**Why:** 49.6°C — hottest ever recorded in Canada. Town burned down the next day. Tests extreme temperature ε-drift and ∂I approach.

### DATASET 3: Hurricane Patricia — October 20-24, 2015 (EXTREME — Hurricane)

**Source:** NOAA HRD Dropsonde Archive
**URL:** `https://www.aoml.noaa.gov/hrd/Storm_pages/patricia2015/sonde.html`
**Also:** NHC Tropical Cyclone Report: `https://www.nhc.noaa.gov/data/tcr/EP202015_Patricia.pdf`
**Also:** HURDAT2 best track: `https://www.nhc.noaa.gov/data/hurdat/hurdat2-nepac-1949-2023-042624.txt`
**What to download:** All dropsonde files from the sonde page (tar.gz archive). These contain vertical profiles (T, P, Td, wind) from aircraft through the eye wall.
**Why:** 872 hPa — strongest Western Hemisphere hurricane. Dropsonde profiles through the eye wall are the most extreme atmospheric gradient measurements on Earth.

### DATASET 4: Radiosonde Sounding — Severe Thunderstorm Day (EXTREME — Convective)

**Source:** University of Wyoming Radiosonde Archive
**URL:** `http://weather.uwyo.edu/cgi-bin/sounding?region=naconf&TYPE=TEXT:LIST&YEAR=2011&MONTH=05&FROM=2212&TO=2212&STNM=72357`
**Station:** 72357 = Norman, OK (OUN) — heart of tornado alley
**Date:** May 22, 2011 12Z (morning sounding before the Joplin EF5 tornado that afternoon)
**Also get May 24, 2011 12Z (before the massive Oklahoma tornado outbreak):**
`http://weather.uwyo.edu/cgi-bin/sounding?region=naconf&TYPE=TEXT:LIST&YEAR=2011&MONTH=05&FROM=2412&TO=2412&STNM=72357`
**Why:** Tests convective initiation prediction — morning sounding → afternoon severe weather. The lattice stability analysis should identify ∂I crossings that predict convective onset.

### DATASET 5: Bomb Cyclone — January 3-5, 2018 (EXTREME — Rapid Cyclogenesis)

**Source:** IEM ASOS Download
**URL:** `https://mesonet.agron.iastate.edu/cgi-bin/request/asos.py?data=all&station=KBOS&sts=2018-01-02T00:00:00Z&ets=2018-01-06T00:00:00Z&latlon=yes&missing=M&trace=T&direct=no&report_type=3`
**Station:** KBOS (Boston Logan) — directly impacted
**Also get:** KJFK (JFK), KEWR (Newark), KPHL (Philadelphia)
**Date range:** January 2-6, 2018
**Why:** "Bomb cyclone" — pressure dropped ~59 hPa in 24 hours. Tests rapid d-family evolution in pressure. The barometric lattice shift accelerated dramatically.

### DATASET 6: Frontal Passage — Any Clear Example (DAILY FORECAST)

**Source:** IEM ASOS Download
**URL:** `https://mesonet.agron.iastate.edu/cgi-bin/request/asos.py?data=all&station=KORD&sts=2023-11-01T00:00:00Z&ets=2023-11-05T00:00:00Z&latlon=yes&missing=M&trace=T&direct=no&report_type=3`
**Station:** KORD (Chicago O'Hare) — frequent frontal passages, high-quality ASOS
**Date range:** November 1-5, 2023 (active fall weather pattern with multiple fronts)
**Why:** Tests frontal passage prediction from single-station data via d-family transitions in pressure, temperature, wind. The palindromic approach/departure signature should be visible.

### DATASET 7: Fair Weather Baseline — 1 Week Calm (DAILY FORECAST)

**Source:** IEM ASOS Download
**URL:** `https://mesonet.agron.iastate.edu/cgi-bin/request/asos.py?data=all&station=KPHX&sts=2023-06-01T00:00:00Z&ets=2023-06-08T00:00:00Z&latlon=yes&missing=M&trace=T&direct=no&report_type=3`
**Station:** KPHX (Phoenix Sky Harbor) — summer high pressure, minimal weather
**Date range:** June 1-8, 2023 (pre-monsoon, persistent ridge)
**Why:** Establishes the baseline. All T-Shadow metrics, all tower convergence, all d-family distributions under calm conditions. The restoration law τ values for fair weather are calibrated here.

### DATASET 8: 30-Day Continuous Record (LONG-RANGE FORECAST TEST)

**Source:** IEM ASOS Download
**URL:** `https://mesonet.agron.iastate.edu/cgi-bin/request/asos.py?data=all&station=KORD&sts=2023-10-01T00:00:00Z&ets=2023-11-01T00:00:00Z&latlon=yes&missing=M&trace=T&direct=no&report_type=3`
**Station:** KORD (Chicago O'Hare) — active weather, good data quality
**Date range:** October 1-31, 2023 (full month, autumn transition)
**Why:** Tests the force-axis vs phase-axis forecast skill decay across 30 days. Sliding window hindcast at every observation point. This is the primary test of the §13.2 cascade stability asymmetry prediction (force skill persists beyond phase decoherence).

### DATASET 9: Sudden Stratospheric Warming — January 2019 (EXTREME — SSW)

**Source:** University of Wyoming Radiosonde Archive
**Station:** 72451 = Dodge City, KS (DDC) — central US, good upper-air coverage
**Dates (12Z soundings, one per day):**
```
http://weather.uwyo.edu/cgi-bin/sounding?region=naconf&TYPE=TEXT:LIST&YEAR=2019&MONTH=01&FROM=0112&TO=0112&STNM=72451
```
**Get daily from January 1-31, 2019** (change FROM=DDMM to each day: 0112, 0212, 0312, ...)
**Why:** SSW events show 50°C temperature rise at 30km altitude in days. Tests the most extreme temperature ε-drift in the atmosphere. The restoration law must handle stratospheric timescales.

### DATASET 10: Multiple Weather Regimes — Mike's Local Station (LIVE FORECASTING)

**Source:** IEM ASOS Download
**Station:** Whichever ASOS is nearest to Mike's location
**Date range:** Most recent 90 days (for initial hindcast calibration), then continuous for live mode
**Why:** This becomes the primary live forecasting station. Mike records Weather Channel/NWS forecasts alongside ET forecasts for the competitive scoreboard.

### DOWNLOAD INSTRUCTIONS

All IEM URLs return CSV when accessed in a browser. To download via command line:

```bash
curl -o dataset1_iowa_derecho.csv "https://mesonet.agron.iastate.edu/cgi-bin/request/asos.py?data=all&network=IA_ASOS&sts=2020-08-09T00:00:00Z&ets=2020-08-12T00:00:00Z&latlon=yes&elev=yes&missing=M&trace=T&direct=no&report_type=3"
```

For Wyoming soundings, the URL returns HTML with embedded text data. Save the page as .txt.

For Patricia dropsondes, download the tar.gz from the AOML page and extract.

**Total: 10 datasets covering 7 extreme events + 3 forecast validation sets. All free, all public domain, all CSV or text format.**

---

## ARCHITECTURE — MODULAR PYTHON + C++ PROJECT

Following the established ET project pattern: C++ core for precision-critical computation (like the EUDD), Python for analysis/interface/verification (like the lossless microphone pipeline). Modular, not monolithic.

### C++ Core (GMP/MPFR/FLINT, CMake + MSVC + vcpkg)

The performance-critical mathematical engine, matching the EUDD stack:

| Module | Purpose | Key dependencies |
|---|---|---|
| `et_core/` | Bijection Π_N, pullback Π_N⁻¹, all 196 identities | GMP (mpz_t), MPFR (mpfr_t), FLINT/Arb |
| `et_tower/` | LCM tower (uncapped, dynamic prime gen), CF home-finding | GMP for arbitrary integer arithmetic |
| `et_lattice/` | Lattice arithmetic (A), differential control (B), composition (C) | MPFR at WORK_DPS=400 |
| `et_complex/` | Complex lattice (D), phase projection, FQG, 24 families | MPFR complex arithmetic |
| `et_boundary/` | ∂I detection (F), tightness, bifurcation, cell transitions | MPFR |
| `et_tensor/` | Transfer tensor (H), impedance ξ(d), cross-channel coupling | MPFR rational arithmetic |
| `et_cascade/` | Palindromic cascades, CPT symmetry, Stability Window | GMP/MPFR |
| `et_shape/` | Shape projection (K), spherical harmonics | MPFR |
| `et_metabolism/` | Koide-ceiling resource governance (3 layers) | OS kernel APIs (Win32/Linux) |
| `et_weather_core/` | Atmospheric projection, R₀ references, coupled dynamics | All above modules |
| `et_forecast/` | Restoration law, ε-drift extrapolation, multi-variable coupling | et_lattice + et_boundary |
| `et_ingest/` | Observation ingestion pipeline (string → mpfr → Π_N → triple) | MPFR, file I/O |

**Build system:** CMake + MSVC + vcpkg (Windows primary, Linux secondary). Statically-linked executables. Same vcpkg overlay port approach as EUDD for GMP.

**Precision:** MPFR at 400 working dps + 100 guard = 1328 bits minimum (matching EUDD's 1200-bit / 361 dps but weather uses 400 dps working). All arithmetic through MPFR — zero IEEE 754 float in the computation chain.

**Output:** Shared library (.dll on Windows, .so on Linux) exposing C API for Python bindings.

### Python Layer (mpmath for verification, ctypes for C++ core)

| Module | Purpose |
|---|---|
| `et_weather/core.py` | Python bindings to C++ core via ctypes (or standalone mpmath fallback) |
| `et_weather/ingest.py` | Data loading: CSV, JSON, METAR parsing, unit conversion, API polling |
| `et_weather/analysis.py` | T-Shadow decomposition, 24-channel monitoring, d-family tracking |
| `et_weather/forecast.py` | Forward forecasting engine, sliding window hindcast, τ calibration |
| `et_weather/verify.py` | All 196 identity verifications, round-trip tests, empirical convergence |
| `et_weather/score.py` | Competitive verification framework, MAE, skill scores, scoreboard |
| `et_weather/viz.py` | Visualization: lattice plots, tower plots, forecast charts, scoreboard display |
| `et_weather/interface.py` | Interactive menu, manual entry, file watch, continuous operation loop |
| `et_weather/metabolism.py` | Python metabolism (mirrors C++ et_metabolism for standalone mode) |
| `et_weather/__main__.py` | Entry point — double-clickable on Windows |

**Standalone mode:** If C++ core is not compiled/available, the Python layer falls back to mpmath for all computation. Slower but functionally identical. This means the Python layer is self-contained and works on any machine with Python + mpmath.

**C++ accelerated mode:** When the shared library is available, Python calls it via ctypes for all precision-critical computation. Orders of magnitude faster for tower escalation and CF home-finding.
...
C++ CORE                          PYTHON LAYER
─────────                         ────────────
et_core (bijection)               et_weather/core.py (ctypes → C++ or mpmath fallback)
  ↓                                 ↓
et_tower (LCM + CF)              et_weather/ingest.py (data loading)
et_lattice (A,B,C)                 ↓
et_complex (D, FQG)              et_weather/analysis.py (T-Shadow, 24 channels)
et_boundary (F)                    ↓
et_tensor (H)                    et_weather/forecast.py (restoration law, hindcast)
et_cascade (palindromic)           ↓
et_shape (K)                     et_weather/score.py (competitive verification)
  ↓                                ↓
et_metabolism (resource gov)     et_weather/viz.py (visualization)
  ↓                                ↓
et_weather_core (atmospheric)    et_weather/interface.py (interactive)
et_forecast (prediction)           ↓
et_ingest (observation pipeline) et_weather/__main__.py (entry point)
```

### Design Principles

1. **Modular** — each module has a single responsibility, clear interface, independent tests. No module exceeds ~1000 lines. 

2. **Dual-language** — C++ for speed, Python for flexibility. Both share the same mathematical definitions. The Python layer can operate standalone (mpmath) or accelerated (C++ via ctypes).

3. **Zero float64 in computation chain** — C++ uses MPFR exclusively. Python uses mpmath exclusively. Float64 only at unavoidable OS/API/rendering boundaries.

4. **Every measurement is a (k, d, ε) triple** — ε is a COORDINATE, not an error. The triple is the fundamental data type throughout both layers.

5. **Dynamic everything, static nothing** — no hardcoded lists, no caps, no static arrays. Everything discovered from the mathematics.

6. **Metabolism-governed** — every large allocation passes through the K=2/3 gate. The algorithm is invariant; only the execution strategy adapts.

7. **Production-ready** — no placeholders, no stubs, no simulations. Every module tested against the 196 identity verification suite.

8. **Windows double-clickable** — `__main__.py` with try/except/finally error capture and `input("Press Enter...")`. The C++ core compiles to a statically-linked .dll that the Python layer loads.

9. **The gravimeter principle** — all 24 harmonic families monitored as independent physical measurement channels. Discovery, not prescription.

10. **The T-Shadow** — all 9 empirically verified T-Shadow metrics computed for every observation.
...
|---|---|
| et_core/ (bijection, pullback) | ~500 |
| et_tower/ (LCM + CF + prime gen) | ~600 |
| et_lattice/ (A, B, C identities) | ~800 |
| et_complex/ (D, FQG, 24 families) | ~500 |
| et_boundary/ (F, ∂I, tightness) | ~400 |
| et_tensor/ (H, impedance) | ~300 |
| et_cascade/ (palindromic, CPT) | ~400 |
| et_shape/ (K, spherical harmonics) | ~300 |
| et_metabolism/ (3-layer governance) | ~500 |
| et_weather_core/ (atmospheric projection) | ~600 |
| et_forecast/ (restoration law, coupling) | ~500 |
| et_ingest/ (observation pipeline) | ~400 |
| CMake + headers + tests | ~1000 |
| **C++ total** | **~6800** |

### Python Layer
| Module | Estimated lines |
|---|---|
| et_weather/core.py (ctypes + mpmath fallback) | ~800 |
| et_weather/ingest.py (CSV, METAR, API, units) | ~600 |
| et_weather/analysis.py (T-Shadow, 24 channels) | ~700 |
| et_weather/forecast.py (hindcast, τ calibration) | ~800 |
| et_weather/verify.py (196 identities + empirical) | ~500 |
| et_weather/score.py (competitive framework) | ~400 |
| et_weather/viz.py (visualization) | ~500 |
| et_weather/interface.py (interactive + continuous) | ~600 |
| et_weather/metabolism.py (Python metabolism) | ~400 |
| et_weather/__main__.py (entry point) | ~100 |
| tests/ | ~800 |
| **Python total** | **~6200** |

**Combined: ~13,000 lines across C++ and Python.**

The existing `et_lossless_microphone.py` (2000+ lines) provides the proven reference implementation. The C++ core reimplements its mathematics at native speed; the Python layer inherits its architecture and extends it for weather.
