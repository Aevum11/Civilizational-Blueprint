...
| **API module** | Hooks for external programs to connect directly — all ET software reads from and writes to the same EUDD |
...
3. **Adding new data**: Ingest module accepts data from any source. New values project at 120 dps, find lattice addresses, check generator membership (is this address already covered by a known generator?). If yes → generator instance recorded. If no → raw entry stored, pending generator discovery. Discovery engine periodically scans raw entries for new generator patterns.
4. **Querying**: Any query → lattice operation → human-readable result. "What's at d=693?" → list of generators and values at that address. "What's the elegance of this value?" → single number from materialized properties. "Show cross-domain hits" → visual map of shared addresses. All answers come from the lattice structure itself, not from SQL joins.
