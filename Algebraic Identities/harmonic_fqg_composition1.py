...
print(f"  EVERY d_c > 12 IN THIS SET is a COMPOSITE: lcm of two harmonic families.")
print(f"  Each decomposes fully into its harmonic factor pair — no independent content.")
print(f"  (Sublattice families d > 12 from the TOWER are a different object entirely —")
print(f"  they are NOT composites of harmonic families. See Identity E3.)")

# Show decompositions for a few composites
print(f"\n  Sample composite decompositions:")
for dc in [35, 77, 132, 55]:
    if dc in all_dc:
        cells = all_dc[dc]
        print(f"    d_c={dc:>3}: {len(cells)} FQG cell(s) → {cells}")

# ═══════════════════════════════════════════════════════════════
# THE CLOSURE PROPERTY — HARMONIC COMPOSITION IS CLOSED
# ═══════════════════════════════════════════════════════════════
print(f"""
  ═══════════════════════════════════════════════════════════════
  THEOREM E1.2 (Harmonic Closure — Subsumption Verification).
  
  The 42 d_c values are the COMPLETE closure set of the 12 harmonic
  families under lcm composition. Every d_c decomposes back into
  harmonic families. No d_c value outside this set is reachable.
  
  Specifically: no prime p > 12 appears in the set. lcm(a,b) for
  a,b ∈ {{1,...,12}} cannot produce 13, 17, 19, 23, or any prime > 12.
  
  This is NOT a limitation — it is the VERIFICATION that the 12-family
  framework is closed under composition (Subsumption Law: subsumes
  everything within its category WITHOUT REMAINDER).
  
  If lcm of two harmonic families COULD produce a value outside the
  set, the framework would be generating structure beyond its own
  domain — a Subsumption violation. The framework would be leaking.
  The unreachability of primes > 12 IS the closure proof.
  
  Primes > 12 that appear as sublattice families at higher tower
  levels (d=13 at N=360360, d=17 at N=12252240, etc.) are NEW
  INTEGRATIVE STRUCTURE from the tower's LCM growth. They are:
    - NOT harmonic families (harmonic families stop at 12)
    - NOT composites of harmonic families (unreachable by lcm)
    - New sublattice structure from the tower growing beyond
      what the 12 cascade modes express
  
  The harmonic families are the fixed structural skeleton.
  The tower is the infinite growth on that skeleton.
  The 42 d_c values are the skeleton's complete joint set.  ∎
""")

# Verify closure: no prime > 12 in the 42
primes_in_dc = [d for d in dc_list if all(d % p != 0 for p in range(2, d)) and d > 1]
primes_gt_12 = [p for p in primes_in_dc if p > 12]
print(f"  Primes in the 42 d_c values: {primes_in_dc}")
print(f"  Primes > 12 in the set: {primes_gt_12 if primes_gt_12 else 'NONE'}")
print(f"  Harmonic closure verified: {'✓ CLOSED' if len(primes_gt_12)==0 else '✗ LEAKING'}")
...
overall = all_native and (len(dc_list)==42) and all_verify_pass and all_phase_pass and (len(primes_gt_12)==0)

print(f"""
  All 12 harmonic families native at N=27720:  {'✓' if all_native else '✗'}
  42 distinct d_c values confirmed:            {'✓' if len(dc_list)==42 else '✗'}
  Harmonic closure (no primes > 12):           {'✓' if len(primes_gt_12)==0 else '✗'}
  ...
  • Sublattice families d > 12 from the tower are NOT harmonic families
    and NOT composites of harmonic families — they are new integrative
    structure from the tower's LCM growth
  • The 42 d_c values are the COMPLETE closure set: 12 harmonic-range
    + 30 composite, all decomposing back into harmonic families
  • No prime > 12 is reachable — this VERIFIES closure (Subsumption Law)
  • Harmonic families are the fixed skeleton; the tower is infinite growth
