...
**Per-step cost analysis from Guide §54.5.1** (active-system projection, derived from the `et_iterate_di` CUDA kernel specification): aggregate per-element per-step cost is approximately **300–400 FLOPS + ~60 transcendental cycles** at full 27720ET resolution. The dominant costs are:
- 2× log₂(|z|) at ~20 cycles (f32) / ~40 cycles (f64) per call
- |z|^p with variable p per step (~30–50 cycles, prevents SIMD/loop unrolling)
- 24-family perturbation accumulator (~200 ops)
- GCD via prime factorization for d-derivation (~10 cycles, 5 divisibility tests for 27720)

The per-step cost difference between 420ET and 27720ET is approximately **40% per GCD call** (Guide §54.5.5) — a small marginal cost for the structural fidelity gained. The real cost is paid by the lattice-reading machinery (log₂, |z|^p) that runs every step regardless of resolution; the GCD scaling with prime count is gentle.

**For workloads where speed matters more than full-resolution structural detail**, the canonical optimization is the **Coarse-Pass + Boundary-Refine method** documented in Guide §54.5.4 — see §7.5 below. This achieves 5–10× speedup while preserving full 27720ET precision exactly where it matters (near the ∂I boundary).
...
### 7.5 The Coarse-Pass + Boundary-Refine Method — Canonical Speed-Optimized Approach (from Guide §54.5.4)

When throughput matters more than universal full-resolution structural detail, the canonical ET speed-optimization is **Coarse-Pass + Boundary-Refine** — a two-pass strategy documented in `ET_Universal_Projection_Guide8.md` §54.5.4. This is **not** a change to the lattice mathematics — it is a computational pattern that exploits a structural fact: full 27720ET resolution is only necessary near the ∂I boundary (where sublattice-family assignment is ambiguous). Far from the boundary, 12ET resolution suffices to classify a value's family with high confidence.

**The structural fact being exploited:** A value with low quintic-tension τ_5, low ∂I-distance, and high tightness at 12ET is already structurally well-classified. A value near ∂I (tightness near 2/3, |ε| near 50¢) is the only case where higher resolution provides additional information. For typical computational workloads, the near-∂I population is a small fraction of total values.

**The two passes:**

**Pass 1 — Coarse projection at 12ET.** Project every value at base resolution N=12. Use the cheap 12-element GCD (only requires divisibility tests for {2, 3, 4, 6, 12}) instead of the 5-prime-factor 27720ET GCD (requires divisibility tests for {2, 3, 5, 7, 11} and their products). For each value, compute:
- $(k_{12}, d_{12}, \varepsilon_{12})$ at N=12
- Tightness $t_{12} = 100/(100 + |\varepsilon_{12}|)$
- ∂I-distance: $|\varepsilon_{12}|/50$

Classify each value as **clearly resolved** (tightness > 2/3, ∂I-distance < 0.5, d-family unambiguous) or **near-boundary** (tightness ≤ 2/3, ∂I-distance ≥ 0.5, d-family potentially ambiguous).

**Pass 2 — Refinement at full resolution for boundary cases only.** For values flagged as near-boundary, rerun the projection at full target resolution (N = 27720 or higher). For clearly-resolved values, accept the 12ET projection as definitive — the structural information is fully captured.

**Speedup characteristics:**

For typical workloads where the near-boundary population is 5–10% of total values, this achieves **5–10× speedup** (Guide §54.5.4) while preserving full 27720ET precision exactly where it matters. The speedup ratio depends on the population distribution:
- If 100% of values are near-boundary: no speedup (full pass needed)
- If 0% are near-boundary: ~10× speedup (only coarse pass)
- Typical scientific workloads: 5–10× speedup

**Applicability:**

This method is appropriate for:
- High-throughput numerical workflows (eigenvalue problems, large-scale simulations, image rendering)
- Iterative algorithms where most values stabilize away from ∂I
- Real-time systems where 12ET-resolution accuracy is sufficient for most decisions
- Memory-constrained systems (12ET tuples are smaller than 27720ET tuples in compact storage formats)

It is **not** appropriate for:
- Symbolic computation requiring proof of equality
- Applications where every value's deep structure (Gaussian signature at 27720ET, deep-home attractor membership) must be known
- Verification of mathematical conjectures involving deep tower trajectories
- Cryptographic applications where adversarial inputs may concentrate at ∂I

**ET-derived classification threshold (from §3.6 — Koide):**

The "clearly resolved vs near-boundary" threshold is **K = 2/3** for tightness, derived from the Koide ratio (one of the four cardinal ET constants — see §3.7 of Guide §12.2). A value with $t_{12} > K = 2/3$ is in the structural-coherence zone at 12ET; a value with $t_{12} \leq K$ is approaching ∂I and warrants refinement.

This is **not** a tuned threshold — it is the ET-native cutoff between manifold coherence and incoherence boundary.

**Implementation pseudocode (illustrative):**

```
function lattice_project_fast(value, target_N):
    # Pass 1 — coarse
    k12, d12, eps12 = project(value, N=12)
    tightness = 100 / (100 + |eps12|)

    if tightness > 2/3:
        # Clearly resolved at 12ET — return scaled equivalent
        return rescale_12_to_target(k12, d12, eps12, target_N)

    # Pass 2 — refine
    return project(value, N=target_N)
```

The `rescale_12_to_target` operation is itself cheap when target_N is a multiple of 12 (which all canonical lattice resolutions are): it scales k by target_N/12 and re-derives d. The structural information from 12ET is preserved, just expressed at finer resolution.

**Generalization beyond two passes:** The same principle applies hierarchically. A three-pass version uses 12ET → 420ET → 27720ET, refining only the boundary cases at each step. For most workloads, two passes (12ET → 27720ET) capture the speedup without unnecessary complexity. For very-deep-tower analyses (LCM(1..13) = 360360 and beyond), three or four passes may be appropriate.

### 7.6 Per-step cost anatomy (from Guide §54.5.1) — when speed matters

For real-time or batch-large applications, the per-step cost of lattice operations is dominated by specific kernels documented in Guide §54.5.1:

| Operation | Approximate cost | Triggered |
|---|---|---|
| 2 × log₂(\|z\|) | ~20 cycles per log₂ in f32, ~40 in f64 | Every step |
| 2 × round() | ~2 cycles each | Every step |
| GCD via prime factorization (5 divisibility tests at 27720ET) | ~10 cycles | Every step |
| 2 × integer division for d = N/gcd | ~4 cycles | Every step |
| Tightness: 1 abs, 1 add, 1 divide | ~5 cycles | Every step |
| ∂I-boundary compare (tightness > 2/3) | 1 compare | Every step |
| Palindrome cascade lookup (mod + index + load) | ~3 instructions | Only on fallback trigger |
| p_dom = 12 / d_dom | 1 divide | Every step |
| sin() for shimmer Ψ_k | ~20 cycles | Every step |
| \|z\|^p with variable p (prevents SIMD) | ~30–50 cycles | Every step |
| Derivative update for distance estimation | ~5 ops | Every step |
| 24-family perturbation accumulator | ~200 ops | Every step |

**Aggregate: ~300–400 FLOPS + ~60 transcendental cycles per element per step at full 27720ET resolution.**

**Cost reduction at 12ET (coarse pass):** The 24-family perturbation accumulator can be reduced to a 4-family accumulator (only the families native at N=12: d ∈ {1, 2, 3, 4, 6, 12}); the GCD has only 5 divisibility tests at small primes; |z|^p uses only small p values. The aggregate at 12ET is approximately **80–120 FLOPS + ~50 transcendental cycles per element per step** — roughly 3–5× cheaper than 27720ET.

Combined with the boundary-refine pass (where only ~5–10% of elements need full resolution), the **net speedup of Coarse-Pass + Boundary-Refine is typically 5–10×** with no loss of structural information at points that need it.

**For storage as well:** the same coarse-vs-fine distinction applies. Most values can be stored as 12ET tuples (16 bytes in compact format); only near-boundary values need 27720ET tuples (32 bytes). Mixed-resolution storage gives both speed and memory benefits.

**Critical observation from Guide §54.5.3:** The fallback (palindromic cascade) itself is essentially free — its cost is one mod, one index, one load. The expensive operation is *knowing whether fallback should trigger*, which requires the full lattice projection. The Coarse-Pass strategy reduces this by deferring the full projection until known to be necessary.

### 7.7 Edge cases (closes FP-12)
