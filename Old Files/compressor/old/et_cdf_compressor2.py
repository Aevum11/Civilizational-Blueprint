...
class CDFEngine:
    ...
    def _encode_lattice_block(self, n, r0, k0, mode, dk0_saved, k_table, rc):
        ...
        use_wide = total_syms > 256
        for pat in arch_defs:
            parts.append(struct.pack('<H', len(pat)))
            for s in pat:
                parts.append(struct.pack('<H' if use_wide else '<B', s))
        
        # Bit-pack final stream
        bits_per_sym = max(1, math.ceil(math.log2(max(total_syms, 2))))
        sym_stream = rc['final_stream']
        n_syms = len(sym_stream)
        packed_val = 0; packed_bits = 0
        packed_bytes = bytearray()
        for sym in sym_stream:
    ...
    def decompress_block(self, block_data: bytes) -> bytes:
        ...
        # Mode 0: read k_table for k-direct decoding
        k_direct_table = None
        if mode == 0:
        ...
        for _ in range(n_arch):
            pat_len = struct.unpack_from('<H', block_data, pos)[0]; pos += 2
            pat = []
