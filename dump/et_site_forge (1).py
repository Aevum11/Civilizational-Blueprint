...
def _boundary_chroma(x, y, Lstar):
    """CIELAB chroma C* of the gamut-boundary color on the ray from the
    spectral chromaticity (x, y) toward D65 at lightness L*."""
    _, rgb = gamut_pull_to_white(x, y, Lstar, iters=60)
    Xb, Yb, Zb = lin_rgb_to_xyz(*[min(max(ch, mpf(0)), mpf(1))
                                  for ch in rgb])
    _, a, b = xyz_to_lab(Xb, Yb, Zb)
    return (a * a + b * b) ** mpf("0.5")

def cusp_lightness(x, y):
    """The per-class natural lightness: L* = argmax over (0,100) of the
    in-gamut CIELAB chroma along the class's spectral ray — the gamut
    CUSP. This is an extremum condition on the observer + device
    translation layer: C* vanishes at both device poles (a, b → 0 as
    Y → 0; the ray is forced onto D65 as L* → 100) and is unimodal
    between, so a ternary search converges. No free parameter is
    ...
    class's own cusp lightness, not a shared lightness.)"""
    lo, hi = mpf(1), mpf(99)
    for _ in range(80):
        m1 = lo + (hi - lo) / 3
        m2 = hi - (hi - lo) / 3
        if _boundary_chroma(x, y, m1) >= _boundary_chroma(x, y, m2):
            hi = m2
        else:
            lo = m1
    return (lo + hi) / 2
...
for cl in CLASSES:
    ...
    for suffix, key, law_tail in (
            ("", "hex",
             f" @ cusp L*={nstr(mpf(cl['cusp_L']),5)} (argmax in-gamut"
             " chroma — the gamut cusp, translation-layer extremum)"),
