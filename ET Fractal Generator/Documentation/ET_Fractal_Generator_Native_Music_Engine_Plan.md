...
**Document version:** Plan v1.4 — Phase A complete, Phase B pending; §9 rule 6 strengthened (a fallback is an error)
...
**Current resume point (as of plan v1.3, 2026-04-09):** Phase A is complete. The next un-completed phase step is Phase B.1 (add `_compute_probe_pixels` + the 21-voice catalog as ET-derived constants). A resumer can verify Phase A state by running: `grep -c -i 'sonif' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c 'et_sonify' ET_FRACTAL_GENERATOR50-10.py` (must return 0), `grep -c 'def et_native_music_' ET_FRACTAL_GENERATOR50-10.py` (must return 3), `grep -c 'def et_visual_' ET_FRACTAL_GENERATOR50-10.py` (must return 4), `grep -c 'STAGE 21.6' ET_FRACTAL_GENERATOR50-10.py` (must return 1), and `python3 -c "import ast; ast.parse(open('ET_FRACTAL_GENERATOR50-10.py').read())"` (must parse cleanly). If any of these return the expected value, Phase A is intact; if any return something else, the file has drifted and the resumer should ask the user for the authoritative version before proceeding.
...
**Next action:** Begin Phase B.1 — add the `_compute_probe_pixels(voices_selected, W, H, cx, cy, zoom, fractal_type)` function that returns the probe pixel index array and the `voice_pixel_map` dict used by the per-step probe kernels. This requires the voice catalog from §3.4 to be available, so the voice catalog dict will be added to the module as a new ET-derived constants block before `_compute_probe_pixels` itself. Phase B.1 is text-only (no kernel changes) and will be followed immediately by Phase B.2 (the `et_iterate_probe_di` CUDA kernel) in the same session when possible.
