...
def _eq_canonical(operation, *args):
    ...
    mpf values serialized at current working precision (mp.dps) — fully dynamic.
    Two operations at different precisions produce different cache keys."""
    parts = [operation]
    for a in args:
        if isinstance(a, type(mpf('0'))):
            parts.append(nstr(a, mp.dps))
        else:
            parts.append(str(a))
...
def eq_lookup(operation, *args):
    ...
    if entry and entry.get('content_type') == CONTENT_EQUATION:
        _EQ_HITS += 1
