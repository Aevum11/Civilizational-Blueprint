...
:: Create package.json (Valid JSON structure)
echo { > package.json
...
:: --- STEP 3: CREATE CONSOLE BRIDGE ---
echo [3/7] Creating Error Bridge...

:: server.js - Forwards browser errors to CMD
...
echo   }); >> server.js
echo. >> server.js
echo   // LOGGING MIDDLEWARE >> server.js
...
:: Create index.html (Permissive CSP for logging)
echo ^<!doctype html^> > index.html
echo ^<html lang="en"^> >> index.html
echo   ^<head^> >> index.html
echo     ^<meta charset="UTF-8" /^> >> index.html
echo     ^<meta name="viewport" content="width=device-width, initial-scale=1.0" /^> >> index.html
echo     ^<meta http-equiv="Content-Security-Policy" content="default-src * 'unsafe-inline' 'unsafe-eval'; script-src * 'unsafe-inline' 'unsafe-eval'; connect-src * 'unsafe-inline'; style-src * 'unsafe-inline';" /^> >> index.html
...
:: Create Entry Point (main.jsx) - Captures and Sends Errors
...
echo const reportError = (err) =^> { >> src\main.jsx
...
echo }; >> src\main.jsx
echo window.addEventListener('error', (e) =^> reportError(e.error)); >> src\main.jsx
echo window.addEventListener('unhandledrejection', (e) =^> reportError(e.reason)); >> src\main.jsx
echo. >> src\main.jsx
echo // GLOBAL SHIMS >> src\main.jsx
echo window.React = React; >> src\main.jsx
echo window.ReactDOM = { createRoot }; >> src\main.jsx
echo. >> src\main.jsx
echo // LOAD USER SCRIPT >> src\main.jsx
echo import('./UserScript.jsx').catch(reportError); >> src\main.jsx
...
:: --- STEP 4.5: APPEND RENDER TRIGGER ---
:: This ensures the app actually mounts if the user file lacks the render call
echo. >> src\UserScript.jsx
echo. >> src\UserScript.jsx
echo // AUTO-INJECTED RENDER TRIGGER >> src\UserScript.jsx
echo try { >> src\UserScript.jsx
echo   if (typeof App !== 'undefined') { >> src\UserScript.jsx
echo     const root = window.ReactDOM.createRoot(document.getElementById('root')); >> src\UserScript.jsx
echo     root.render(^<App /^>); >> src\UserScript.jsx
echo   } else { >> src\UserScript.jsx
echo     console.log("App component not found in global scope."); >> src\UserScript.jsx
echo   } >> src\UserScript.jsx
echo } catch (e) { console.error("Auto-render failed:", e); } >> src\UserScript.jsx
...
echo  Any browser crash will be printed here in RED.
echo ===================================================
echo.
