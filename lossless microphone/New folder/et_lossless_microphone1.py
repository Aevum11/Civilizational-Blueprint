...
class ETDifferentialTracker:
    ...
    def process_sample(self, sample_str):
        ...
        Input: sample_str — string representation of sample value
               (e.g., "12345/8388608" for a 24-bit PCM sample)
        
        Returns: ET sample dict with refined ε (sub-quantization precision),
                 plus tracking metadata.
        """
        s = mpf(sample_str)
        self.sample_count += 1
        ...
        # ── STEP 1: MEASURE — raw projection of quantized sample ──
        ...
        can_predict = (
            ...
            r > mpf(0) and
            sign == self.prev_sign  # No sign change (sign changes reset tracking)
        )
        
        if not can_predict:
            # First sample, or sign change, or zero crossing: no prediction
            ...
            # Predicted ε: previous refined ε + predicted change
            predicted_eps_raw = self.prev_refined_eps + predicted_deps
            
            # Account for cell transitions: if k changed, adjust for the
            # cell_width shift that the T-act introduced
            ...
            # ── STEP 3: QUANTIZATION THRESHOLD ──
            # One LSB at the current amplitude, expressed in cents:
            #   Λ · LSB / r = (1200/ln2) · (1/max_int) / r
            # This is the ε uncertainty introduced by the ADC's quantization
            quantization_cents = LAMBDA_MANIFOLD * self.lsb_normalized / r
            
            # ── STEP 4: FUSE ──
            disagreement = fabs(predicted_eps - eps_meas)
            
            if disagreement <= quantization_cents:
                # PREDICTION ACCEPTED: the prediction was computed from
                # refined_ε[n-1] (which has accumulated sub-quantization info)
                # plus the exact control law. It has higher precision than
                # the raw measured ε which is limited by ADC bit depth.
                refined_eps = predicted_eps
                tracking_source = "predicted"
                self.prediction_count += 1
            else:
                # MEASUREMENT OVERRIDE: the signal changed more than the
                # control law predicted — transient, discontinuity, or noise.
                # Reset to measured value, losing accumulated precision.
                refined_eps = eps_meas
        ...
        # Build the complete ET sample with refined ε
        from math import lcm as math_lcm
    ...
    def get_statistics(self):
        ...
        # Estimate resolved lattice depth beyond initial ADC resolution
        # When prediction_rate is high, the tracker resolves additional
        # crystal structure through the control law
        if prediction_rate > mpf(0) and prediction_rate < mpf(1):
            resolved_depth = mplog(mpf(1) / (mpf(1) - prediction_rate)) / (mpf(2) * LOG2)
        elif prediction_rate == mpf(1):
            resolved_depth = mpf(self.bit_depth)  # full additional resolution
        else:
        ...
        return {
            ...
            'sigma_ET': SIGMA_AUDIO,  # σ = 1/√12 — structural cell scale
        }
    ...
    def generate_test_wav(filepath, signal_type="sine", frequency=440,
        ...
        for i in range(n_frames):
            ...
            if signal_type == "sine":
                ...
                # Phase integral of f_t: ∫₀ᵗ f(τ) dτ = f₀t + (3f₀/(2T))t²
                phase = TWO_PI * (mpf(frequency) * t + mpf(3 * frequency) * t * t / (mpf(2) * T_dur))
                # Instantaneous frequency f_t verifies: dphase/dt = 2π·f_t
...
class ETLiveCapture:
    ...
    Recording modes:
      - Interactive: press Enter to stop recording
      - Timed: record for a specified duration
    
    When hardware is unavailable, reports clearly and exits.
    """
    ...
    def __init__(self, N=12, R0="1.0", sample_rate=48000, channels=1,
        ...
        try:
            ...
            input_devices = [d for d in all_devices
    ...
    def record_interactive(self, output_name=None):
        ...
        print(f"  ╚══════════════════════════════════════════════════════════╝")
        print(f"  Device: {self.default_device['name']}")
        print(f"  Rate: {self.sample_rate} Hz | Channels: {self.channels} | "
              f"Depth: {self.bit_depth}-bit")
        ...
        stream = self.sd.InputStream(
            ...
            dtype='int32',  # Maximum integer precision from ADC
        ...
        for i in range(n_frames):
            for ch in range(self.channels):
                ...
                int_val = int(raw_audio[i, ch]) >> (32 - self.bit_depth)
                int_val = max(-(1 << (self.bit_depth - 1)),
                            min((1 << (self.bit_depth - 1)) - 1, int_val))
...
def cmd_record(cmd_args):
    """Record from microphone. Press Enter to stop. Lossless output."""
    N = 12
    R0 = "1.0"
    name = cmd_args[0] if cmd_args else None
