...
|---|---|---|
| **P** | The disk substrate — the raw byte space on NVMe/NTFS |
...
**Verification against hardware.** The standard NVMe logical block size is 4096 bytes. The standard Windows memory page size (for `CreateFileMapping` / `MapViewOfFile`) is 4096 bytes. The NTFS cluster size (default for volumes > 16 GB) is 4096 bytes. This alignment is the digital substrate's N = 12 resolution manifesting in the hardware that implements it.
...
...continue through LCM landmarks indefinitely...
    ...
    ↓
Home found: d stable across 2 consecutive landmarks + verified across 2 more
    ...
    ↓
OR: session ends before home found
...
**Architecture:** Two files: `EUDD_Manager.exe` + `Sempaevum.akashic`. The `.akashic` file IS the Sempaevum on disk — a single monolithic file whose address space (N, k, d) is the organizational structure. The file IS a minimal generator (§7.1a) — generators are the primary content; memoized raw entries are the Descriptor Gap. Queries are lattice operations. The `EUDD_Manager.exe` (§7.10) — native C++ binary with MPFR/GMP/Arb precision stack, Dear ImGui + OpenGL 4.6 GPU-accelerated visualization, GLFW, ImPlot — bootstraps, manages, discovers, serves all ET software through a central database with GPU-rendered lattice navigation and management dashboard. Build: CMake + MSVC → single statically-linked .exe.
