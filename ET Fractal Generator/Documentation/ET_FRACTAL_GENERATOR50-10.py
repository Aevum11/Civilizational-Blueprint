...
def _choose_audio():
    """Audio sonification — yes/no, then bitrate if yes."""
    print('  ┌──────────────────────────────────────────────────────────────┐')
    ...
    print('  │                                                              │')
    print('  │   Image: horizontal scan-line sonification (~15 s)         │')
...
def _render_frame(cx, cy, zoom, mode, tower, jc, is_julia,
    ...
    if USE_GPU:
        ...
        # Transfer raw iteration data for audio sonification
        it_raw = None
        if AUDIO_ENABLED:
            try:
    ...
    # Raw data assembly for audio sonification
    raw_arrays = {}
    if AUDIO_ENABLED:
        for _key in ('smooth_n', 'd_r', 'd_t', 'tight', 'orbit'):
...
# ══════════════════════════════════════════════════════════════════════════════
#  STAGE 21.5 — ET-DERIVED SONIFICATION ENGINE
#
...
# ── Additional ET audio tables for v3.0 sonification ──────────────────────
...
# audio sonification's per-d amplitude weighting matches the visual coupling.
...
# ── Image sonification: v4.0 — professional architecture ─────────────────

def et_sonify_image(final_f32, stem, script_dir, kbps, scan_row_frac=0.5,
                    audio_duration=20.0, n_scan_rows=5, it_raw=None):
    """
    v4.0 Professional ET sonification: note-segmented, phase-continuous,
    with portamento, reverb, and MIDI output.
    """
    print(f'  Generating audio (v4.0 — professional sonification)…', flush=True)
...
# ── Video sonification: per-frame evolving chord ──────────────────────────

def et_sonify_video_frame(final_f32, scan_row_frac=0.5, it_raw=None):
    """
...
def et_sonify_video(frame_stats_list, fps, stem, script_dir, kbps,
                    video_path=None):
    """
    Build continuous audio from per-frame d-family distributions.

    frame_stats_list:  list of dicts from et_sonify_video_frame(), one per frame
    ...
    if AUDIO_ENABLED and it_raw is not None:
        ...
        try:
            ...
            _et_error('Saving raw audio data .npz', e, fatal=False,
                      fallback_msg='Will use image-based sonification instead')
            npz_path = None

    # ── Audio sonification (if enabled) ──────────────────────────────────────
    audio_path = None
    if AUDIO_ENABLED:
        try:
            audio_path = et_sonify_image(final_f32, stem, script_dir, AUDIO_KBPS,
                                         it_raw=it_raw)
        except Exception as e:
            _et_error('Audio sonification', e, fatal=False,
...
def generate_zoom_video():
    ...
    for fi, zoom_fi in enumerate(zooms):
        ...
        if AUDIO_ENABLED:
            try:
                fstats = et_sonify_video_frame(final_f32, it_raw=it_raw_frame)
    ...
    if AUDIO_ENABLED and frame_audio_stats:
        ...
        try:
            et_sonify_video(frame_audio_stats, fps, _audio_stem, script_dir,
                            AUDIO_KBPS, video_path=str(video_path))
        except Exception as e:
            _et_error('Video audio sonification', e, fatal=False,
