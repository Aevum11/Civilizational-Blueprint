...
def v_config_encode(sym_stream: List[int], total_symbols: int) -> Tuple[bytes, bytes]:
    ...
    def _assign_code(node, sym, target_depth, current_depth, code_so_far):
        """Recursively find an empty slot at target_depth in the trie."""
        if current_depth == target_depth:
            if node[2] is None and node[0] is None and node[1] is None:
                node[2] = sym
                return True
            return False
        ...
        for bit in (0, 1):
            ...
            if child is None:
                child = _make_node()
                node[bit] = child
            if _assign_code(child, sym, target_depth, current_depth + 1,
    ...
    def _extract_codes(node, depth, code):
        if node is None:
            return
        if node[2] is not None:
            code_map[node[2]] = (code, depth)
            return
        if node[0] is not None:
            _extract_codes(node[0], depth + 1, code)
        if node[1] is not None:
            _extract_codes(node[1], depth + 1, code | (1 << depth))
...
def v_config_decode(encoded_data: bytes, table_data: bytes, n_symbols: int) -> List[int]:
    ...
    def _assign_code(node, sym, target_depth, current_depth):
        if current_depth == target_depth:
            if node[2] is None and node[0] is None and node[1] is None:
                node[2] = sym
                return True
