...
class UniversalLattice:
    ...
    def compute_cross_tower_elegance(self, byte_val: int,
        ...
        # ── L3 sublattice coherence: d_u and d_p must be LCM-compatible ──
        # The Subsumption Law: the combined cross-tower sublattice must be
        # subsumable within the manifold (LCM ≤ N_FULL). If d_combined > N_FULL,
        # the byte occupies incompatible sublattice families across perspectives.
        d_combined = (d_u * d_p) // math.gcd(d_u, d_p)
        if d_combined > N_FULL:
            return 0.0  # Cross-sublattice incoherence
    ...
    def compress_tower_with_universal(self, tower_idx: int,
        ...
        if dk_personal and dk_u and len(dk_personal) == len(dk_u):
            ...
            for dp, du in zip(dk_personal, dk_u):
                ...
                t_u = lattice_tightness(eps_u)
                # Koide gate: use whichever perspective is tighter
                if t_p * t_u >= K_KOIDE:
...
class CDFCompressor:
    ...
    def compress_batch(self, input_paths: List[str], output_dir: str) -> dict:
        ...
        towers: List[LatticeTower] = []
        for fi, fpath in enumerate(input_paths):
            if not os.path.isfile(fpath):
                self._log(f"  SKIP: {fpath} (not found)")
                continue
            self._log(f"  Tower {fi}: {os.path.basename(fpath)}")
            tower = LatticeTower.from_file(fpath)
            towers.append(tower)
            self._log(f"    R₀_p = {tower.personal_r0:.6f}, "
                      f"size = {tower.data_size:,}B, "
                      f"Δk_len = {len(tower.dk_stream):,}")
            self._progress(fi / len(input_paths) * 10,
                           f"Tower {fi + 1}/{len(input_paths)}: "
