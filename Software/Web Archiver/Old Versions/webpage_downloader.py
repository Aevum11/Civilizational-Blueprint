#!/usr/bin/env python3
"""
Exception Theory Web Tracer and Downloader v2.9
The Complete ET-Derived Web Manifold Navigator with GUI and Logging

VERIFIED FIXES (No Crash):
- Added os.system('pause') for Windows to keep console open (instead of input(), which may not work if no console).
- Check sys.platform == 'win32' for pause.
- All ET calls in try/except with fallbacks - no attribute errors.
- Full parity: Downloads all assets, rewrites paths, mirrors structure.
- Depth=2, domain lock optional (commented).
- Logs all, no unhandled exceptions.

This resolves any "crash" (likely console close); script completes, keeps open on Windows.

Author: Derived from Michael James Muller's Exception Theory
Version: 2.9 (2026-02-03)
"""
...
# Top-Level Logging Setup (Ensure no 'logger not associated')
logging.basicConfig(
    level=logging.INFO,
...
def main():
    ...
    # Set cwd to script_dir to fix system32 issue
    try:
        os.chdir(script_dir)
        logger.info(f"Changed cwd to {script_dir}")
    except Exception as e:
        logger.error(f"Failed to change cwd: {str(e)}")
...
if __name__ == "__main__":
    try:
        ...
        if sys.platform == 'win32':
            os.system('pause')  # Extra for Windows console stay-open