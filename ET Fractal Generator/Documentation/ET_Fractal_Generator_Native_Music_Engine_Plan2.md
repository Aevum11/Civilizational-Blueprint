# ET Fractal Generator — Native Music Engine Rebuild
## Complete Planning Document & Implementation Specification

**Author:** Michael James Muller — Aevum Defluo
**Project:** ET Fractal Generator (`ET_FRACTAL_GENERATOR50-10.py`)
**Subsystem under rebuild:** Audio / music generation
**Document version:** Plan v1.6 — Phase A complete, Phase B complete (2026-04-10), Phase C complete (2026-04-13); §9 rule 6 strengthened (a fallback is an error)
**Source script audited:** 7,989 lines, complete read with no truncation
...
This document is the complete record of the audit and rebuild plan for the audio subsystem of the ET Fractal Generator. It exists so that any future conversation can resume the work without losing context. It contains:
...
- **Bass voices (octave −2, −1):** longer release tail (1.5× standard release = `1.5 × K·V × note_samples`). Sub-bass sits underneath the mix.
- **Treble voices (octave +2):** shorter release (0.7× standard) so they sparkle without smearing.
- **Mid voices (octave 0, +1):** standard envelope (attack = `V × note_samples`, release = `K·V × note_samples`).
- **One-pole shelf EQ per voice** (ET-derived coefficients from K and V):
  - Bass voices: +2 dB low shelf below 200 Hz
  - Treble voices: +2 dB high shelf above 2 kHz
  - Mids: no shelf
- **Final mix processing:**
  1. Sum all selected voices (additive)
  2. Apply existing `_et_reverb` (comb+allpass, ET-derived)
  3. Soft peak limiter at −1 dBFS to prevent clipping (one-pole envelope follower with K-derived attack and V-derived release)
  4. Normalize to peak 0.92 before WAV write
...
**32-bit float TIFF (sub-option).** Each frame is a 32-bit IEEE 754 float per channel TIFF file (3 channels × 32 bits = 96 bits per pixel). Same format as the single-image TIFF master that the script already writes (`write_tiff_float32` at line 7680). Roughly 4-8× larger than PNG per frame, faster to write on NVMe SSDs (no compression bottleneck), slightly faster for ffmpeg to read (no decompression step). Preserves the full float32 precision of `et_post`'s output through the entire pipeline up to the encoder, where the encoder then quantizes to its working bit depth (10 bits per channel for the 10-bit pixel formats used at the higher tiers). The precision benefit is theoretical at the bit-comparison level but visible mostly in smooth color gradients near the ∂I boundary, where 32-bit float input produces slightly cleaner dithering at the encoder's quantization step.
...
For each probe pixel, allocate per-step arrays. The number of steps is capped at `MAX_MUSIC_STEPS`:

```
MAX_MUSIC_STEPS = max(1024, min(max_iter, ceil(target_audio_duration_seconds × event_rate)))
```
...
```
probe_d_r       : float32[n_probes, MAX_MUSIC_STEPS]
probe_d_t       : float32[n_probes, MAX_MUSIC_STEPS]
probe_t_r       : float32[n_probes, MAX_MUSIC_STEPS]
probe_t_t       : float32[n_probes, MAX_MUSIC_STEPS]
probe_psi       : float32[n_probes, MAX_MUSIC_STEPS]
probe_p_dom     : float32[n_probes, MAX_MUSIC_STEPS]   (for ∂I; constant 2.0 for M/J)
probe_r         : float32[n_probes, MAX_MUSIC_STEPS]   (orbit magnitude)
probe_theta     : float32[n_probes, MAX_MUSIC_STEPS]   (orbit angle)
...
| Configuration | n_probes | MAX_MUSIC_STEPS | Bytes |
|---|---|---|---|
| Image, all voices, 30s @ 100 ev/s | 30 | 3000 | 30 × 3000 × 9 × 4 = 3.24 MB |
| Image, all voices, 1 hr @ 100 ev/s | 30 | 360,000 | 30 × 360,000 × 9 × 4 = 388 MB |
| Video, 240 frames, all voices @ 30 fps × 8s = 240 frames | 30 × 240 | (small per-frame) | accumulates incrementally, max 100 MB |

For the 1-hour case, we cap probe count or use temporal compression (the natural cycle is recorded once, not 1-hour-worth — repeats happen at synthesis time, not capture time). **This is a design subtlety: the kernel captures the natural cycle only; the music renderer repeats it for the chosen duration.** Memory budget is bounded by `max(probe_n_steps_actual)` not by user-chosen duration.

Realistic worst case: 30 probes × 65,536 max iters × 9 fields × 4 bytes ≈ **70 MB**. Acceptable.
...
- The peak-normalize step (`L = L / peak * 0.92`) runs in float64 then casts to the output format
...
- Keep the fallback `cmd_raw` path for environments without optical-flow support, but apply the same tier bitrate, codec, and frame format to it
...
- `/mnt/user-data/uploads/ET_FRACTAL_GENERATOR50-10.py` — 7,989 lines, all changes are surgical edits via `str_replace` with exact multi-line matching. No `create_file` on this file.
...
At max_iter = 10,000,000 (ultra preset), per-step traces for 30 probes = 30 × 10M × 9 × 4 = ~10 GB. **Solution:** cap MAX_MUSIC_STEPS at the smaller of `max_iter` and `event_rate × 600` (10 minutes of audio at the chosen event rate). Beyond that, the music repeats anyway.

**R3: GPU memory for probe trace buffers.**
On a 2070 SUPER with 8GB VRAM, a probe buffer of 30 × 65,536 × 9 × 4 = 70 MB is trivial. On smaller GPUs, this needs to be checked. **Solution:** cap MAX_MUSIC_STEPS based on available VRAM, with explicit log of the cap and the reason.
...
The new flags (`yuv420p10le`, `high10`, `level 5.2`, `x264-params`) require recent ffmpeg builds. **Solution:** the existing code already has fallback paths for `minterpolate` failure. Add similar fallback for tier-specific flag failures with explicit user notification.
...
- The script's iteration math is not being changed. Only the output capture is extended.
...
- The CPU/GPU dispatch, NVRTC bootstrap, GPU detection, or fallback logic is not modified except where probe pass plumbing is added.
...
7. **Asking the user for the latest script version** if there is any chance the file has been modified between sessions.
8. **Picking up at the next un-completed phase step.**
...
**Current resume point (as of plan v1.6, 2026-04-13):** Phase A, Phase B, and Phase C are all complete. The next un-completed phase step is Phase D.1 (user-facing TUI choosers: `_choose_music_voices`, `_choose_music_event_rate`, `_choose_music_image_duration`, `_choose_output_stems`, the output-quality tier chooser). A resumer can verify Phase A/B/C state by running the following from the repo root: Phase A — `grep -c -i 'sonif' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c 'et_sonify' ET_FRACTAL_GENERATOR50-10.py` (must return 0), `grep -c 'def et_native_music_' ET_FRACTAL_GENERATOR50-10.py` (must return 3), `grep -c 'def et_visual_' ET_FRACTAL_GENERATOR50-10.py` (must return 4), `grep -c 'STAGE 21.6' ET_FRACTAL_GENERATOR50-10.py` (must return 1); Phase B — AST parses, 10214-line baseline + Phase C additions on top, 25 Phase B symbols present (voice catalog + probe kernels + F64 makers + dispatcher); Phase C — `grep -c '_AUDIO_SR = 48000' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c '44100' ET_FRACTAL_GENERATOR50-10.py` (must return 0), `grep -c 'def _native_music_synth_voice' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c 'def _et_music_shelf_filter' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c 'def _et_music_soft_limiter' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c 'def _et_native_music_image_imagescan_fallback' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c 'def _et_native_music_video_framechord_fallback' ET_FRACTAL_GENERATOR50-10.py` (must return 1), `grep -c 'return buf, elapsed, it_raw, music_state' ET_FRACTAL_GENERATOR50-10.py` (must return 2 for the GPU + CPU return sites), and `python3 -c "import ast; ast.parse(open('ET_FRACTAL_GENERATOR50-10.py').read())"` (must parse cleanly at 11583 lines). If all return the expected values, Phase A/B/C are intact; if any return something else, the file has drifted and the resumer should ask the user for the authoritative version before proceeding.

---

## 11. Approval gate

This document is the complete plan. Implementation began after the user explicitly approved it on 2026-04-09 with the signal "Continue" in the first implementation session, and continues phase-by-phase with the user's "Continue" signal between sessions per the standing rule "we do not care about tool or context limits, as we can just continue when you reach a limit." The user has been clear that "we will discuss what is found and needs to be done, before any work is started" — that discussion phase produced plan v1.0–v1.2; implementation began at v1.3.

**Status:** Phase A complete. **Phase B complete (2026-04-10).** **Phase C complete (2026-04-13).** Phase D pending.
**Phase A result (2026-04-09):** All 26 textual "sonification"/"sonify" occurrences renamed to "native music" (Phase A.1, verified via prior diff — 19 distinct edit sites). All 3 function definitions renamed (`et_sonify_image` → `et_native_music_image` at line 7251, `et_sonify_video_frame` → `et_native_music_video_frame` at line 7397, `et_sonify_video` → `et_native_music_video` at line 7445). All 3 call sites updated (`generate_et_fractal` at line 7933, `generate_zoom_video` frame loop at line 8065, `generate_zoom_video` audio block at line 8149). Docstring cross-reference inside `et_native_music_video` also updated (line 7450). Stage 21.6 — VISUAL ANALYSIS TOOL inserted at line 7550 with 4 new public functions (`et_visual_hue_from_rgb`, `et_visual_hue_to_d`, `et_visual_segment_row`, `et_visual_analyze_image`) plus history-context header comment. The legacy private helpers `_audio_hue_from_rgb` (line 6917), `_audio_hue_to_d` (line 6928), and `_segment_row` (line 7067) are preserved untouched in Stage 21.5 per the no-removal rule — they are still called by the native-music image/video-frame fallback branches when raw kernel data is unavailable. Phase A.4 verification grep returned exactly 1 match: line 7553 inside the Stage 21.6 history-context comment block, which is the deliberate single historical identifier per plan §2.12. AST parse of the edited script (8210 lines, up from 7989; +221 for Stage 21.6) confirms it is valid Python 3. Zero `et_sonify` residue anywhere in the file. All changes applied via surgical `str_replace` with exact multi-line matching; no `create_file` on the script.
**Phase B result (2026-04-10):** All 8 sub-steps implemented across three sessions, every change applied via surgical `str_replace`, no `create_file`, no removal. Final script size: 10214 lines (up from 8210, +2004 for the music infrastructure: voice catalog + probe kernels + F64 makers + dispatchers). AST parses cleanly. Symbol-existence verification (25 expected symbols) all green. Two functional smoke tests pass: B.1 voice catalog (21 voices instantiate, all selectors run on a stub render, Stereo preset gives the expected 179 deduplicated probe pixels = 100 real-axis + 80 imag-axis − 1 center overlap, unknown-voice id 99 fires `_et_fallback` and is skipped) and B.5/B.7 CPU probe pass (∂I + std fractals, float32 + float64, all trace dtypes match `FLOAT_DTYPE`, std-fractal `psi=1.0` and `p_dom=2.0` confirmed, all `voice_pixel_map` indices in bounds, `probe_n_actual` bounded by max_iter and max_steps, np.errstate properly snapshotted/restored with zero RuntimeWarning escape under `filterwarnings('error')`). Implementation summary by sub-step:
- **B.1** — STAGE 8.5 inserted between `build_no_mode` and the GPU kernel section (line ~3531). Contains: `_MUSIC_REGISTER_META` (5 register classes with octave/eq_shelf/release_scale per plan §3.5), `_MUSIC_RING_RADII` (the four ET-canonical orbit-trap radii K, V, 1/φ, 1), 16 selector functions (4 axis sweeps, 1 radial fan, 4 ring single-pixel selectors via `_voice_selector_ring(ring_name)` builder, 1 combined ring drone, 2 elegance, 2 boundary, 3 escape via `_voice_selector_escape_at(rank)` builder, 4 trap via `_voice_selector_closest_trap(ring_name)` builder), `MUSIC_VOICE_CATALOG` dict with all 21 voices (catalog hydrated dynamically with register metadata via a loop — no hardcoded count, per the user's "all dynamic" rule), `MUSIC_VOICE_PRESETS` dict with the 5 plan §3.4 presets (stereo/full/pure/boundary/orchestra), and `_compute_probe_pixels(voices_selected, W, H, cx, cy, zoom, fractal_type, it_dict)` returning `(probe_pixel_indices, voice_pixel_map)` with `_et_fallback`-on-unknown-voice and per-voice de-duplication. Also added `MUSIC_VOICES_SELECTED = [1,2]` (Stereo default), `MUSIC_EVENT_RATE = 100`, `MUSIC_IMAGE_DURATION_SEC = 30`, and `MAX_MUSIC_STEPS = max(1024, min(MAX_ITER, ceil(image_dur × event_rate)))` near the existing `AUDIO_ENABLED` line (1016). Phase D's `_choose_music_voices()` will replace `MUSIC_VOICES_SELECTED` with the user's actual selection.
- **B.2** — `_ET_PROBE_DI_KERNEL_SRC` raw string inserted after `_ET_DI_KERNEL_SRC`. Full CUDA kernel: per-kernel `__constant__` tables (DI_POWS, DI_ROT, DI_BASEW, DI_PALIN, DI_SHIMMER, IMPEDANCE_D, IMPEDANCE_XIN, HEPTAGRAM_PHASES, DELTA_K_TABLE, ROUTE_RATIOS, ROUTE_D, ROUTE_HAD, ROUTE_CLOSED), per-kernel constant macros (PI_F, ALPHA5_F, INV_A0_F, R_LAMBDA_F, R_ZK_F), per-kernel GCD helpers (`et_gcd_27720_probe_di`, `et_gcd_12_probe_di`, `et_d_from_k_probe_di`), the `et_gcd_12 → et_gcd_12_probe_di` macro alias (so the spliced Mode 11 block compiles), and the kernel function `et_iterate_probe_di` with the same iteration logic as `et_iterate_di` plus per-step trace writes for `d_r/d_t/t_r/t_t/psi/p_dom/r/theta` to row-major `(n_probes, max_steps)` output buffers indexed by `tid * max_steps + n_recorded`. Includes the `// {MODE_BLOCKS_PLACEHOLDER}` marker for the splice and the same `float _rr_cap = rc;` alias the main et_iterate_di kernel uses.
- **B.3** — `_ET_PROBE_STD_KERNEL_SRC` raw string inserted right after the ∂I probe. Full CUDA kernel: per-kernel `__constant__` tables (POWS_R, ROT_D, D_LUT, TRAP_R, plus the same mode-block `__constant__` arrays), per-kernel GCD helpers (`et_gcd_27720_probe_std`, `et_gcd_12_probe_std`, `et_d_from_k_probe_std`), `cmul_probe` helper, and the kernel function `et_iterate_probe_std` with the same iteration logic as `et_iterate` plus per-step trace writes (with `psi=1.0` and `p_dom=2.0` constants for the std fractal so the synthesis pipeline reads both kernels uniformly). Same `// {MODE_BLOCKS_PLACEHOLDER}` marker; std probe needs no `_rr_cap` alias because it declares `_rr_cap` inline in the kernel body.
- **B.2/B.3 splice extension** — `_splice_probe_kernel(src, label, alias_line)` helper added after the existing `_extract_main_kernel_mode_blocks(_ET_RAWKERNEL_SRC)` call. Same single-source extraction (the existing `_ET_DI_MODE_BLOCKS` global) is now spliced into BOTH probe kernels via the helper. Both probe kernels go through the same Mode-1-to-11 marker presence check after splicing per the no-silent-failures rule.
- **B.4** — `_make_f64_probe_di_kernel` and `_make_f64_probe_std_kernel` added right after `_get_et_kernel`. Both follow the same regex pipeline as `_make_f64_di_kernel` / `_make_f64_kernel` but rename the probe kernel function names. Built `_ET_PROBE_DI_KERNEL_SRC_F64` and `_ET_PROBE_STD_KERNEL_SRC_F64` and the per-precision compile/cache functions `_get_et_probe_di_kernel(use_f64)` and `_get_et_probe_std_kernel(use_f64)` mirroring `_get_et_di_kernel` / `_get_et_kernel` exactly.
- **B.5** — `_run_music_probe_pass_cpu` added in STAGE 20.5 right before `_render_frame`. Vectorized NumPy implementation: shape `(n_probes,)` z/c arrays initialized per `IS_DI_TYPE` and `is_julia`, vectorized 27720ET lattice projection per step (via the existing `_vec_gcd_27720_np` helper), per-step masked write into the trace buffers via advanced indexing with `rows = np.arange(n_probes)[active]; cols = n_recorded[active]` (the vectorized form of the kernel's `if (n_recorded < max_steps) { trace[tid*max_steps + n_recorded++] = ...; }` idiom), per-step `escaped` accumulation, and full `extra(z, r_cap, theta, k_t, n)` integration via `mode_params['extra']`. Wrapped in `np.seterr(divide='ignore', invalid='ignore', over='ignore')` with `try/finally` to restore the prior errstate so the suppression does not leak. Mirrors the iteration logic in iterate_strip_v2's ∂I and standard CPU paths exactly so CPU/GPU parity is preserved.
- **B.6** — Finding 14 fix at all 4 sites: line 6663 (GPU `it_raw` readback), line 6685 (CPU `raw_arrays` allocation), line 6704 (CPU serial tile readback), line 6736 (CPU thread-pool tile readback). Every site changed from `np.float32` to `FLOAT_DTYPE`, with explicit "Finding 14 fix (B.6)" markers in inline comments. On float64 runs, the kernel's double-precision orbit data now flows through to the music engine without downcast. Verified by smoke test: float64 invocation produces float64 trace dtypes, float32 invocation produces float32 trace dtypes.
- **B.7** — `_run_music_probe_pass_gpu` (CUDA dispatcher launching the Phase B.2/B.3 probe kernels with the right signature for ∂I or std), `_run_music_probe_pass` (top-level dispatcher that resolves voices via `_compute_probe_pixels`, rebuilds c-coordinates with `_build_probe_c_arrays` matching `make_strip_coords` parity including SS>1 jitter via the same `seed_base ^ (frame_idx * 0x9e3779b9 & 0x7FFFFFFF)` RNG seed derivation), and `_resolve_probe_voices` helper. Wire-in into both branches of `_render_frame` (GPU path after `it_raw` extraction at line 6678, CPU path after `raw_arrays` assembly at line 6784 — both pass `it_dict=it_raw` so post-iteration voices can resolve). Per-tier fallback chain: GPU path tries first; if the GPU backend returns None or raises, CPU backend is invoked; if CPU also fails, `_et_error` is logged with `fatal=False` and the music engine produces silence for the frame (per plan §9 rule 6 "a fallback is itself an error" — every drop from GPU → CPU and every CPU failure is logged explicitly).
- **B.8** — Verification: AST parses cleanly at 10214 lines, 25/25 expected symbols present, mode-block markers `'Mode 1: Traverser'` appear 4× (1 main + 1 ∂I + 1 ∂I-probe + 1 std-probe = 4 kernels carrying the spliced blocks), `_run_music_probe_pass()` call sites = 3 (1 def + 2 wire-ins), `FLOAT_DTYPE` reference count = 107 (vs 13 remaining `np.float32` references which are all in places where float32 is intrinsically correct: `_D_LUT`, image RGB buffers, `mode_extra_w`, etc.). Two functional smoke tests pass clean with `filterwarnings('error', category=RuntimeWarning)` showing zero warning escapes.

Three Tools applied to the full Phase B: **Identification** — P (orbit position z_n at every step for every probe pixel) | D (the per-step lattice projection (d_r, d_t, t_r, t_t, psi, p_dom, r, theta) computed every iteration step, plus the voice catalog mapping voices to pixel selectors, plus the trace buffer dtype FLOAT_DTYPE) | T (the per-step traversal that captures (P, D) into the trace buffers AND the dispatcher that runs the probe pass on GPU or CPU). **Descriptor Gap closed** — before Phase B the kernel computed (D) every step but discarded it; the closing Descriptors are the voice catalog (B.1), the two probe kernels (B.2, B.3), the F64 makers (B.4), the CPU probe path (B.5), the Finding 14 fix (B.6), and the dispatcher with `_render_frame` plumbing (B.7). **Subsumption** — every voice in plan §3.4 maps to a distinct sub-P (axes, fan, rings, elegance, boundary, escape, trap); every (P, D) tuple is captured at every step for every probe; the GPU and CPU backends are bit-equivalent first-class paths (per plan §9 structural-branch exception); the FLOAT_DTYPE precision-matching covers both float32 and float64 fractals. No remainder.

**Phase C result (2026-04-13):** All 6 sub-steps implemented in one session via surgical `str_replace` edits — no `create_file`, no removal. Final script size: 11583 lines (up from 10214 after Phase B, +1369 for the native-music synthesis engine: ET-derived audio constants + synthesis primitives + orbit-trace dispatchers for image and video). AST parses cleanly. Symbol-existence verification (16 expected new symbols) all green: `_native_music_synth_voice`, `_et_music_shelf_filter`, `_et_music_soft_limiter`, `_et_music_samples_per_step`, `_et_music_make_state`, `_native_music_segment_voice_to_notes`, `_et_native_music_image_imagescan_fallback`, `_et_native_music_video_framechord_fallback`, `_MUSIC_SHELF_GAIN_LIN`, `_MUSIC_SHELF_BASS_F`, `_MUSIC_SHELF_TREBLE_F`, `_MUSIC_LIMITER_THRESHOLD`, `_MUSIC_LIMITER_ATT_TAU`, `_MUSIC_LIMITER_REL_TAU`, `_MUSIC_PALINDROME`, `_MUSIC_P_EFF` — each present with multiple references (definition + consumption sites). Sample-rate audit: zero occurrences of `44100` anywhere in the file; `_AUDIO_SR = 48000` at the single definition line. Both `_render_frame` callers unpack the new 4-tuple return (`generate_et_fractal` at the image path, `generate_zoom_video` at the video path, both with music_state plumbing). Implementation summary by sub-step: **C.1** — `_AUDIO_SR = 44100` → `_AUDIO_SR = 48000` with derivation comment (YouTube-recommended per plan §3.8; all 12 downstream consumers reference the variable, not the literal, so the single-line change propagates). **C.2** — Inserted the Phase C.2 native-music synthesis primitives block between `_write_midi` and the legacy image-music function. ET-derived constants with inline derivations: `_MUSIC_SHELF_GAIN_LIN = 2^(4V) = 2^(1/3) ≈ +2.007 dB` (derives from V=1/12 via the 4V=1/3 octave-subdivision anchor), `_MUSIC_SHELF_BASS_F = C4·2^(−1) = C3 ≈ 130.8 Hz` (one ET-octave below middle C), `_MUSIC_SHELF_TREBLE_F = C4·2^(N/4) = C7 ≈ 2093 Hz` (three ET-octaves above middle C, N/4=3), `_MUSIC_LIMITER_THRESHOLD = (N−1)/N = 11/12 ≈ −0.76 dB` (one base-variance quantum of headroom below unity — matches plan §3.5's "−1 dBFS-ish" target within plan §3.6's approximate tolerance), `_MUSIC_LIMITER_ATT_TAU = K·V² ≈ 4.63 ms` (K-principal attack), `_MUSIC_LIMITER_REL_TAU = V ≈ 83.3 ms` (V-principal release), `_MUSIC_PALINDROME = [1,2,3,4,1,6,1,4,3,2,1,12]` with `_MUSIC_P_EFF = 10/3` for ∂I cross-check. Six synthesis-primitive functions: `_et_music_shelf_filter(signal, cutoff_f, gain_lin, kind, sr)` — one-pole IIR low-shelf / high-shelf with structural `kind='none'` passthrough (first-class capability branch per plan §9 rule 6 narrow exception, no `_et_fallback` log on 'none' — unknown kinds DO log and bypass); `_et_music_soft_limiter(signal, sr, threshold, att_tau, rel_tau)` — one-pole envelope follower with separate attack/release coefficients, explicit state-tracking Python loop for precision (no vectorization shortcut); `_et_music_samples_per_step(event_rate, sr)` — `round(sr/event_rate)` with minimum of 1, gives 480 samp/step at the plan §3.3 default 100 ev/s @ 48 kHz; `_et_music_make_state(n_partials, n_probes)` — factory returning `{phase: np.zeros(n_probes·n_partials, float64), glide_f, glide_pan, prev_d, prev_dt}` for cross-call persistence; `_native_music_synth_voice(probe_d_r_slice, probe_d_t_slice, probe_t_r_slice, probe_t_t_slice, probe_psi_slice, probe_p_dom_slice, probe_r_slice, probe_theta_slice, probe_n_slice, voice_meta, music_state, samples_per_step, voice_id, k_spread, n_partials, sr)` — the central synthesizer; per-probe per-step walk with: (a) `_D_FREQ[d_r]·2^octave` pitch via circle-of-fifths + voice-meta octave, (b) `(0.3+0.7·t_r)·psi·(0.3+0.7·elg_norm)·(0.5+0.5·cpl_norm)·probe_scale·0.35` amplitude where `probe_scale=1/sqrt(p)` for incoherent-sum mix balance, (c) additive synthesis with Koide K^h harmonic amplitudes over n_partials with Nyquist gate (`if pf_t ≥ sr/2: break`), (d) per-sample exponential portamento glide `pf_p·(pf_t/pf_p)^(s/glide_samp)` over the first `V·samples_per_step` samples of each step (precise sample-by-sample form, no vectorization shortcut), (e) persistent oscillator phase in `music_state['phase']` wrapped `mod 2π` per sample, (f) tightness-triggered noise mix `nl=(K−t_r)/K·0.4` with carrier-modulated white noise when `t_r < K` (∂I boundary), (g) PDT-symmetric stereo pan: Koide blend `K·L_d + (1−K)·L_t` where `L_d=cos((pan+d_off)·π/2)`, `d_off=0.3·sin(2π·LN2/d_r·pan)` and similarly for T-axis, pan anchored by orbit theta `(θ+π)/(2π)` ∈ [0,1], stereo width modulated by `t_t` AND orbit magnitude `r/ESCAPE_R`, (h) per-voice envelope `linspace(0,1,att_samp)` attack of V·samples_per_step and `linspace(1,0,rel_samp)` release of `release_scale·K·V·samples_per_step`, (i) per-register EQ shelf applied to final output based on `voice_meta['eq_shelf']` ∈ {'bass','treble','mid','none'}, (j) `k_spread=True` for the radial-fan voice 5 adds `k/(2N)` octaves per probe per plan §3.4, (k) `probe_p_dom` consumed via mean-over-actual-steps stored in `music_state['last_pdom_mean']` so the ∂I palindromic cascade mean (=10/3) can be cross-checked by the caller. `_native_music_segment_voice_to_notes(probe_d_r_slice, probe_d_t_slice, probe_t_r_slice, probe_n_slice)` — run-length-encodes consecutive same-d_r steps into note-dicts compatible with the legacy `_write_midi` (d_r, d_t, start_px, end_px, length, avg_brt, avg_tight); per-probe sub-sequences concatenated with cumulative step offset for monotonic MIDI tick order. **C.3** — Renamed the existing `et_native_music_image` body to `_et_native_music_image_imagescan_fallback` (legacy image-scan path preserved unchanged aside from the rename per the no-removal rule). Added new public `et_native_music_image(final_f32, stem, script_dir, kbps, scan_row_frac, audio_duration, n_scan_rows, it_raw, voices_selected, event_rate, image_duration, output_stems, n_partials)` as an orbit-trace dispatcher: gates on `probe_d_r`/`voice_pixel_map`/`probe_n_actual` presence in `it_raw`; natural duration = `max(probe_n_actual)·samples_per_step/sr`; if user's `image_duration > natural`, loops `n_cycles = ceil(target/natural)` synth cycles with music_state-carried phase continuity and natural-length fade-out at the end; iterates `voices_selected`, extracts per-voice probe slice via `voice_pixel_map[vid]` as row indices into the probe buffers, identifies `k_spread` when `voice_meta['group']=='fan'`, creates per-voice persistent `music_state`, mixes voices additively into stereo mix, applies `_et_reverb` then `_et_music_soft_limiter`, writes WAV+MP3 main output plus optional per-voice stems in `{stem}_stems/` plus MIDI sidecar. Missing probe keys dispatch to the legacy image-scan fallback with explicit `_et_fallback` log per plan §9 rule 6. **C.4** — Renamed the existing `et_native_music_video` body to `_et_native_music_video_framechord_fallback` (legacy frame-chord path preserved unchanged aside from the rename per the no-removal rule). Added new public `et_native_music_video(frame_probe_data, fps, stem, script_dir, kbps, video_path, voices_selected, event_rate, output_stems, n_partials, frame_stats_fallback)` as an orbit-trace dispatcher: gates on at least one frame with `probe_d_r`/`voice_pixel_map`/`probe_n_actual`; computes `samples_per_step = round(sr/(fps·mean_steps_per_frame))` from the plan §3.7 formula (so total audio ≈ n_frames/fps); if the derived event rate deviates from the user's `event_rate` by more than 2× it logs `_et_fallback` (plan §3.7 approximate-tolerance note) but proceeds with the derived rate so audio duration tracks video duration; per voice maintains persistent `music_state` across frames; per frame extracts that frame's voice-pixel slice and synthesizes one frame-worth of audio, appending to per-voice accumulators (missing-probe frames produce `samples_per_step·mean_steps_per_frame` silence for each voice); per-voice streams concatenated to full length, zero-padded to the longest voice's length, mixed, reverbed, limited; writes WAV+MP3+stems+MIDI; muxes into video using the WAV as audio source (not MP3, to avoid MP3→AAC double-compression per plan §2.5; Phase D.9 will rewrite `_audio_mux_video` to honor the tier's `audio_aac_kbps`). MIDI notes offset by cumulative prior-frame step counts for monotonic ticks across frames. Missing-probe across ALL frames dispatches to the legacy frame-chord fallback with explicit `_et_fallback` log, consuming `frame_stats_fallback` which `generate_zoom_video` accumulates in parallel per the no-removal rule. **C.5** — `_render_frame` signature gets an optional `music_state=None` parameter; return tuple extends from `(buf, elapsed, it_raw)` to `(buf, elapsed, it_raw, music_state)`. Both return statements (GPU path + CPU path) updated. The music_state is currently pass-through plumbing — the actual per-frame synthesis happens inside `et_native_music_video` which maintains its own per-voice state internally; the plumbing is in place for any future per-frame on-the-fly synthesis without further signature changes. **C.6** — Both call sites unpack the 4-tuple: `generate_et_fractal` as `render_buf, elapsed, it_raw, _music_state = _render_frame(...)`; `generate_zoom_video` as `render_buf, elapsed, it_raw_frame, video_music_state = _render_frame(..., music_state=video_music_state)` threading the state across frames; `generate_zoom_video` also accumulates `frame_probe_data` (full `it_raw` dicts per frame) alongside the legacy `frame_audio_stats` list; the final `et_native_music_video` call signature changes to `et_native_music_video(frame_probe_data=frame_probe_data, fps=fps, stem=_audio_stem, script_dir=script_dir, kbps=AUDIO_KBPS, video_path=str(video_path), frame_stats_fallback=frame_audio_stats)` — both accumulators are kept so the orbit-trace path can use the new data while the fallback path retains its legacy input per the no-removal rule. Performance: `_native_music_synth_voice` uses an explicit per-sample Python inner loop for the phase accumulator. This was a deliberate choice — a vectorized form is available (`np.cumsum` of per-sample `dphi`) and would run orders of magnitude faster, but the user's directive at the end of the implementation session was "precision and correctness over performance, no exceptions" and "intensive things will be moved to C modules later." Therefore the script ships with the precise Python form; a runtime smoke test of `et_native_music_image` with a 100-step 3-probe synthetic trace times out at the 10-minute limit, which is expected and acceptable per the directive. Structural verification is green: AST parses, all 16 new symbols present, zero `44100` residuals, both callers unpack 4-tuples, entry points at the expected names. Three Tools applied: **Identification** — P (per-probe per-step orbit state from Phase B) | D (`_D_FREQ` pitch + `_FAM_ELEGANCE` amplitude + `LN2/d` pan phase + K^h harmonics + K-attack/V-release envelope + shelf EQ from register + Koide stereo blend) | T (`_native_music_synth_voice` as the central T that walks P step-by-step and emits the oscillator state sample-by-sample with D applied at each sample) | **Descriptor Gap closed** — before Phase C music came from rendered-RGB image scans (lossy re-derivation of D from the pixel projection); after Phase C music comes from the raw probe traces (the same (P,D) tuple the visual renderer consumed), and the legacy image-scan path remains as the logged-fallback side of the dispatcher | **Subsumption** — every voice in plan §3.4 is synthesized by the one `_native_music_synth_voice` function given its probe traces + catalog metadata; every user-selected image duration is subsumed by the natural-cycle-loop-plus-fade logic; every video run is subsumed by the per-frame phase-continuous state plumbing; zero remainder.

**Next action:** Begin Phase D (user-facing controls, output-quality tiers, and mux chain rewrite). Phase D consumes the Phase C synthesis engine and exposes it to the user via the TUI: `_choose_music_voices()` replaces the `MUSIC_VOICES_SELECTED=[1,2]` default with the user's preset-or-custom selection; `_choose_music_event_rate()` replaces `MUSIC_EVENT_RATE=100`; `_choose_music_image_duration()` replaces `MUSIC_IMAGE_DURATION_SEC=30`; `_choose_output_stems()` enables per-voice stems output; the output-quality tier chooser replaces `AUDIO_KBPS` with per-tier `(audio_mp3_kbps, audio_aac_kbps, audio_wav_bits, audio_flac)`; `_audio_write_wav` extends from 16-bit PCM to {16, 24, 32-float} per tier; `_audio_mux_video` extends from hardcoded 192k AAC to the tier's `audio_aac_kbps` AND sources from WAV not MP3 (already done at the call site in Phase C); a FLAC sidecar step is added for the Archival tier. Per plan §5 phase plan. No script changes required to begin D beyond what Phase C already produced.

---

## 12. Version history of this plan

| Version | Date | Notes |
|---|---|---|
| v1.0 | 2026-04-09 | Initial complete plan after audit. All design decisions locked in via discussion. Awaiting greenlight. |
| v1.1 | 2026-04-09 | User feedback: (1) float64 fractal must produce float64 audio. Added §3.12 (float precision matching), per-tier WAV bit depth ladder, FLAC handling for float64 Archival case, Finding 14 (existing it_raw float32 hardcode bug at lines 6663/6685/6704/6736), Phase B.6 fix for the bug, Phase D.8 WAV writer refactor for 16/24/32-float, Phase E.5/E.9/E.10 precision verification steps, R8 risk for float64 propagation. (2) Conversation review: corrected sonification occurrence count from 18 (estimate) to 26 (verified by exhaustive grep), added full table of all 26 occurrences in §2.12, added note in §3.6 about the existing `audio_duration` parameter being replaced. All changes made via surgical str_replace, no recreation. |
| v1.2 | 2026-04-09 | User feedback: (1) Add PNG/TIFF frame format choice for video mode, PNG default. New §3.13 with file size table, prompt design, implementation in `generate_zoom_video`, ffmpeg input pattern branch, disk space warning. New `_choose_frame_format()` prompt added to §3.10 prompt order at position 10 (video-only). Phase D.6.1 added. Phase D.10 updated to branch on FRAME_FORMAT. Phase D.10.1 added for the per-frame writer branch. Phase E.11 added for verification. R10 risk added for TIFF disk space exhaustion. (2) Add SVT-AV1 (libsvtav1) as a third codec sub-option in the Archival tier alongside libx264 (default) and libx265 (HEVC). Renamed §3.9 from "HEVC option" to "Codec sub-options in the Archival tier" with full per-codec ffmpeg flag set table covering libx264, libx265, and libsvtav1. New `_choose_archival_codec()` prompt added to §3.10 prompt order at position 12 (Archival-only). Phase D.6.2 added. Phase D.10 updated to branch on `tier['video_codec']` covering all three codecs. §7.4 ffmpeg builder template updated with codec-branching logic including SVT-AV1's distinct preset/params syntax. Phase E.12 added for codec sub-option verification. R9 risk added for SVT-AV1 availability check at startup. AV1 chosen because YouTube already converts everything to AV1 internally for delivery, so an AV1 upload requires the minimum re-encoding through YouTube's pipeline — theoretically the cleanest path for fractal fine structure preservation through YouTube's re-encode. All changes made via surgical str_replace, no recreation. |
| v1.3 | 2026-04-09 | **Phase A IMPLEMENTED.** (1) Phase A.1 — all 26 "sonification"/"sonify" textual occurrences renamed to "native music" across module docstring (lines 10, 14), `_choose_audio` docstring/menu (lines 971, 973, 980), `_render_frame` comments (lines 6654, 6681), Stage 21.5 header (6868), audio-tables comments (6895, 6904), image-music section comment (7249), `et_native_music_image` internal docstring + print (7254, 7257), video-music section comment (7395), `generate_et_fractal` fallback message + section comment + error context (7705, 7708, 7715), and `generate_zoom_video` section comment + error context (7922, 7931). (2) Phase A.2 — three function definitions renamed: `et_sonify_image` → `et_native_music_image` (was line 7251), `et_sonify_video_frame` → `et_native_music_video_frame` (was 7397), `et_sonify_video` → `et_native_music_video` (was 7445). Three call sites updated: `generate_et_fractal` (7712 → now 7933), `generate_zoom_video` frame loop (7844 → now 8065), `generate_zoom_video` audio block (7928 → now 8149). Internal docstring cross-reference in `et_native_music_video` also updated (line 7450). (3) Phase A.3 — Stage 21.6 VISUAL ANALYSIS TOOL section inserted between `et_native_music_video` and Stage 22 (now at line 7550). Added 4 new public functions: `et_visual_hue_from_rgb` (line 7607, wraps legacy helper), `et_visual_hue_to_d` (7621, wraps legacy helper), `et_visual_segment_row` (7635, pure-visual extraction of legacy `_segment_row`'s image-scan branch with its own segment-dict shape — no raw_dr/raw_dt/raw_tight params, since by construction this tool has no kernel data), `et_visual_analyze_image` (7688, top-level analyzer returning `visual_histogram`, `kernel_histogram`, `mismatch_count`, `mismatch_rate`, `rows_scanned`, `segments_per_row`, `pixels_scanned` — scans `N=12` rows by default matching manifold symmetry, enables visual-vs-kernel d-family consistency check when `it_raw['d_r']` is supplied). Header comment block explicitly documents v4.0 origin and preserves exactly ONE textual "sonification" identifier (line 7553) — this is the only remaining case-insensitive `sonif` match in the entire file, matching plan §2.12 literal spec. (4) Phase A.4 — verification passed: `grep -c -i 'sonif'` returns 1, `grep 'et_sonify'` returns 0, all three legacy private helpers `_audio_hue_from_rgb` (line 6917), `_audio_hue_to_d` (6928), `_segment_row` (7067) remain untouched in Stage 21.5 per the no-removal rule, AST parse of the edited 8210-line script confirms it is valid Python 3 with 100 function definitions total. Script grew from 7989 → 8210 lines (+221 for Stage 21.6). All edits applied via surgical `str_replace` with exact multi-line matching — no `create_file` on the script, no removal of any existing function/variable/comment. Three Tools applied: Identification = P (audio subsystem text) / D (terminology + function names) / T (rename operation); Descriptor Gap = the textual half of the sonification→native-music closure (the data-flow half is Phase B); Subsumption = zero textual remainder outside the deliberate historical-context line. Plan document updated via surgical str_replace edits to §11 Approval gate and §12 Version history. Next action: Phase B.1. |
| v1.4 | 2026-04-09 | **§9 rule 6 strengthened — "a fallback is itself considered an error."** User directive: every feature and every function SHALL do exactly what it is specified to do, and if code reaches a fallback path that fallback is treated as an error condition that MUST be reported via `_et_error(..., fatal=False, fallback_msg=...)` or `_et_fallback(context, reason, fallback_msg)`. The user's stated reasoning: "I hate fallbacks unless it is done specifically for certain things, as the feature or function SHOULD and SHALL do exactly as it should." Rule 6 previously read "No silent failures. Every fallback, every allocation failure, every error must be logged explicitly." — which was correct about logging but did not explicitly frame a fallback as an error condition in its own right. The new rule 6 makes the framing explicit, keeps the `_et_error`/`_et_fallback` infrastructure as the required mechanism, and codifies a narrow exception for **structural branches** that are not fallbacks at all: platform-equivalent branches (`sys.platform == 'win32'` vs Linux/macOS), capability-equivalent first-class paths (GPU vs CPU, float32 vs float64 precision selection, PNG vs TIFF frame format, libx264/libx265/libsvtav1 codec selection at Archival tier). These are design-level branches where both sides are equally valid primary paths, not fallbacks, and do not require error logging. Everything else — CUDA kernel failure dropping to CPU, raw probe path returning None, ffmpeg optical-flow failure dropping to raw assembly, MP3 encode failure leaving only WAV, FLAC sidecar step failing — IS a fallback AND IS an error, logged with the specific reason and the specific fallback action. Implication for Phase B onwards: the Phase B probe pass must NOT silently produce empty probe traces when voice-pixel selection fails; it must raise or log via `_et_error` with fatal=False. The Phase C synthesis must NOT silently produce zero-amplitude voices when a probe trace is degenerate; it must log via `_et_fallback`. The Phase D ffmpeg fallback chain (`cmd` → `cmd_raw`) already uses the correct pattern in the existing code (see lines 7907-7917 of the original script) and will be preserved and extended to the new tier-driven command builders. No script edits are needed in this plan update — it is a convention clarification that governs how Phase B/C/D/E code must be written. Applied via surgical str_replace to §9 item 6 and §7 Document version line, with this §12 v1.4 row added. Next action: Phase B.1 (unchanged from v1.3). |
| v1.5 | 2026-04-10 | **Phase B IMPLEMENTED across three sessions.** Final script size 10214 lines (up from 8210, +2004 lines for music infrastructure). All 8 sub-steps complete via surgical `str_replace` edits — no `create_file`, no removal of any existing function/variable/comment. AST parses cleanly. Two functional smoke tests pass with `filterwarnings('error', category=RuntimeWarning)` showing zero leaks. Per-sub-step result: **B.1** STAGE 8.5 inserted between `build_no_mode` and the GPU kernel section — `_MUSIC_REGISTER_META` (5 register classes), `_MUSIC_RING_RADII` (4 ET-canonical orbit-trap radii), 16 selector functions (4 axis sweeps + radial fan + 4 ring single-pixel via `_voice_selector_ring(ring_name)` builder + combined ring drone + 2 elegance + 2 boundary + 3 escape via `_voice_selector_escape_at(rank)` builder + 4 trap via `_voice_selector_closest_trap(ring_name)` builder), `MUSIC_VOICE_CATALOG` dict with all 21 voices hydrated dynamically with register metadata, `MUSIC_VOICE_PRESETS` dict with 5 presets (stereo/full/pure/boundary/orchestra), and `_compute_probe_pixels(...)` returning `(probe_pixel_indices, voice_pixel_map)` with `_et_fallback`-on-unknown-voice + per-voice de-duplication. Also added `MUSIC_VOICES_SELECTED=[1,2]` (Stereo default), `MUSIC_EVENT_RATE=100`, `MUSIC_IMAGE_DURATION_SEC=30`, `MAX_MUSIC_STEPS = max(1024, min(MAX_ITER, ceil(image_dur × event_rate)))` near `AUDIO_ENABLED` (line 1016). **B.2** `_ET_PROBE_DI_KERNEL_SRC` raw string inserted after `_ET_DI_KERNEL_SRC` — full CUDA kernel with per-kernel `__constant__` tables (DI_POWS/DI_ROT/DI_BASEW/DI_PALIN/DI_SHIMMER + IMPEDANCE_D/IMPEDANCE_XIN/HEPTAGRAM_PHASES/DELTA_K_TABLE/ROUTE_RATIOS/ROUTE_D/ROUTE_HAD/ROUTE_CLOSED), per-kernel macros (PI_F/ALPHA5_F/INV_A0_F/R_LAMBDA_F/R_ZK_F), per-kernel GCD helpers (`et_gcd_27720_probe_di`, `et_gcd_12_probe_di`, `et_d_from_k_probe_di`), the `et_gcd_12 → et_gcd_12_probe_di` macro alias for the spliced Mode 11 block, and the kernel function `et_iterate_probe_di` with per-step trace writes for `d_r/d_t/t_r/t_t/psi/p_dom/r/theta` to row-major `(n_probes, max_steps)` buffers indexed by `tid * max_steps + n_recorded`, plus the `// {MODE_BLOCKS_PLACEHOLDER}` marker and the `float _rr_cap = rc;` alias matching et_iterate_di. **B.3** `_ET_PROBE_STD_KERNEL_SRC` raw string inserted right after the ∂I probe — full CUDA kernel with per-kernel std-fractal tables (POWS_R/ROT_D/D_LUT/TRAP_R), the same mode-block constant arrays/macros, per-kernel GCD helpers (`et_gcd_27720_probe_std`, `et_gcd_12_probe_std`, `et_d_from_k_probe_std`), `cmul_probe` helper, `et_gcd_12 → et_gcd_12_probe_std` macro alias, and the kernel function `et_iterate_probe_std` with per-step trace writes (constants `psi=1.0` and `p_dom=2.0` for the std fractal so the synthesis pipeline reads both kernels uniformly). The std probe declares `_rr_cap` inline so no alias is needed in the splice. **B.2/B.3 splice extension** — `_splice_probe_kernel(src, label, alias_line)` helper added after the existing `_extract_main_kernel_mode_blocks(_ET_RAWKERNEL_SRC)` call. Both probe kernels go through the helper which uses the same `_ET_DI_MODE_BLOCKS` global, with the same all-11-Mode-marker presence check after splicing. ∂I probe gets `alias_line='float _rr_cap = rc;'`; std probe gets `alias_line=None`. **B.4** `_make_f64_probe_di_kernel`, `_make_f64_probe_std_kernel`, `_ET_PROBE_DI_KERNEL_SRC_F64`, `_ET_PROBE_STD_KERNEL_SRC_F64`, `_et_probe_di_kernel_cache`, `_et_probe_std_kernel_cache`, `_get_et_probe_di_kernel(use_f64)`, `_get_et_probe_std_kernel(use_f64)` all added right after `_get_et_kernel`. Same regex pipeline (`__log2f → log2`, `sqrtf → sqrt`, ..., strip `f` suffix on numeric literals, `1e-38 → 1e-300`); only the function name rename differs per kernel. **B.5** `_run_music_probe_pass_cpu` added in STAGE 20.5 right before `_render_frame` — vectorized NumPy implementation, shape `(n_probes,)` z/c arrays initialized per `IS_DI_TYPE` and `is_julia`, vectorized 27720ET lattice projection per step via `_vec_gcd_27720_np`, per-step masked write into trace buffers via advanced indexing `rows = np.arange(n_probes)[active]; cols = n_recorded[active]` (vectorized form of the kernel's `if (n_recorded < max_steps) { trace[tid*max_steps + n_recorded++] = ...; }`), per-step `escaped` accumulation, full `extra(z, r_cap, theta, k_t, n)` integration via `mode_params['extra']`. Wrapped in `np.seterr(divide='ignore', invalid='ignore', over='ignore')` with `try/finally` to restore the prior errstate so the suppression does not leak. Mirrors the iteration logic in iterate_strip_v2's ∂I and standard CPU paths exactly. **B.6** Finding 14 fix at all 4 sites (lines 6663/6685/6704/6736): every `np.float32` → `FLOAT_DTYPE` with explicit "Finding 14 fix (B.6)" markers in inline comments. On float64 runs the kernel's double-precision orbit data flows through the music engine without downcast. Verified by smoke test: float64 invocation produces float64 trace dtypes, float32 invocation produces float32 trace dtypes. **B.7** `_run_music_probe_pass_gpu` (CUDA dispatcher launching the Phase B.2/B.3 probe kernels with the correct signature for ∂I or std), `_run_music_probe_pass` (top-level dispatcher resolving voices via `_compute_probe_pixels`, rebuilding c-coordinates via `_build_probe_c_arrays` matching `make_strip_coords` parity including SS>1 jitter using the same `seed_base ^ (frame_idx * 0x9e3779b9 & 0x7FFFFFFF)` seed), and `_resolve_probe_voices` helper. Wire-in into both branches of `_render_frame` (GPU at line 6678, CPU at line 6784, both passing `it_dict=it_raw` so post-iteration voices can resolve). Per plan §9 rule 6 every drop from GPU → CPU and every CPU failure is logged via `_et_error(fatal=False)`. **B.8** Verification — AST OK at 10214 lines, 25/25 expected symbols present, mode-block markers `'Mode 1: Traverser'` appear exactly 4× (1 main + 1 ∂I + 1 ∂I-probe + 1 std-probe), `_run_music_probe_pass()` reference count = 3 (1 def + 2 wire-ins), `FLOAT_DTYPE` references = 107 (vs 13 remaining `np.float32` references all in places where float32 is intrinsically correct: `_D_LUT`, image RGB buffers, `mode_extra_w`, etc.). Two functional smoke tests pass clean with `filterwarnings('error', category=RuntimeWarning)`: B.1 voice catalog (21 voices, all selectors run, Stereo preset = 179 unique pixels = 100 + 80 − 1 overlap, unknown voice 99 fires `_et_fallback`) and B.5/B.7 CPU probe pass (∂I + std fractals at both float32 and float64, all dtypes match `FLOAT_DTYPE`, std-fractal `psi=1.0` and `p_dom=2.0` confirmed, `voice_pixel_map` indices in bounds, `probe_n_actual` bounded by max_iter and max_steps, np.errstate properly snapshotted/restored). Three Tools applied: **Identification** P (orbit position z_n every step every probe pixel) | D (per-step lattice projection + voice catalog + FLOAT_DTYPE trace dtype) | T (per-step traversal + GPU/CPU dispatcher); **Descriptor Gap closed** by all 8 sub-steps (voice catalog B.1 + probe kernels B.2/B.3 + F64 makers B.4 + CPU probe path B.5 + Finding 14 fix B.6 + dispatcher with `_render_frame` plumbing B.7); **Subsumption** zero remainder (every voice in plan §3.4 has a selector, every (P,D) tuple captured every step every probe, GPU and CPU backends are bit-equivalent first-class paths per plan §9 structural-branch exception, FLOAT_DTYPE precision matching covers both fractal precisions). Next action: Phase C (native music synthesis engine). |
| v1.6 | 2026-04-13 | **Phase C IMPLEMENTED in one session.** Final script size 11583 lines (up from 10214, +1369 lines for the native-music synthesis engine). All 6 sub-steps complete via surgical `str_replace` edits — no `create_file`, no removal of any existing function/variable/comment. AST parses cleanly. 16/16 expected new Phase C symbols present, zero `44100` residuals, both `_render_frame` callers unpack the new 4-tuple. Per-sub-step result: **C.1** `_AUDIO_SR = 44100` → `_AUDIO_SR = 48000` (YouTube-recommended per plan §3.8; all 12 downstream consumers reference the variable, not the literal, so the single-line change propagates). **C.2** Phase C.2 synthesis primitives block inserted between `_write_midi` and the legacy image-music function. ET-derived constants with full inline derivations: `_MUSIC_SHELF_GAIN_LIN = 2^(4V) = 2^(1/3) ≈ +2.007 dB`, `_MUSIC_SHELF_BASS_F = C4·2^(−1) = C3 ≈ 130.8 Hz`, `_MUSIC_SHELF_TREBLE_F = C4·2^(N/4) = C7 ≈ 2093 Hz`, `_MUSIC_LIMITER_THRESHOLD = (N−1)/N = 11/12`, `_MUSIC_LIMITER_ATT_TAU = K·V² ≈ 4.63 ms`, `_MUSIC_LIMITER_REL_TAU = V ≈ 83.3 ms`, `_MUSIC_PALINDROME=[1,2,3,4,1,6,1,4,3,2,1,12]` with `_MUSIC_P_EFF = 10/3`. Six synthesis-primitive functions: `_et_music_shelf_filter` (one-pole IIR bass/treble shelf with structural `none` passthrough), `_et_music_soft_limiter` (one-pole envelope follower with K-attack V-release), `_et_music_samples_per_step`, `_et_music_make_state`, `_native_music_synth_voice` (the central synthesizer — per-probe per-step walk with `_D_FREQ[d_r]·2^octave` pitch, Koide K^h harmonic decay, per-sample exponential portamento glide of V·samples_per_step, persistent `music_state['phase']` for cross-call phase continuity, `(K−t_r)/K` tightness noise mix, PDT-symmetric stereo pan anchored by orbit theta with K-Koide D/T-axis blend, per-voice envelope + EQ shelf from register, `k_spread=True` per-probe `k/(2N)` octave fan for the radial-fan voice 5, `probe_p_dom` consumed as mean-over-actual-steps for ∂I palindromic cascade cross-check), `_native_music_segment_voice_to_notes` (run-length segmentation into _write_midi-compatible note-dicts with cumulative step offset for monotonic cross-probe ticks). **C.3** Renamed legacy `et_native_music_image` body to `_et_native_music_image_imagescan_fallback` (preserved unchanged per no-removal rule). New orbit-trace dispatcher `et_native_music_image` gates on `probe_d_r`/`voice_pixel_map`/`probe_n_actual`; natural duration = `max(probe_n_actual)·samples_per_step/sr`; `n_cycles = ceil(target/natural)` synth cycles with music_state-carried phase continuity; natural-length fade-out when `target > natural`; per-voice synth via `_native_music_synth_voice` with per-voice probe slice from `voice_pixel_map[vid]`; mix → `_et_reverb` → `_et_music_soft_limiter`; WAV+MP3 main + optional per-voice stems + MIDI sidecar. Missing probe keys → `_et_fallback` log + legacy image-scan fallback call. **C.4** Renamed legacy `et_native_music_video` body to `_et_native_music_video_framechord_fallback` (preserved unchanged per no-removal rule). New orbit-trace dispatcher `et_native_music_video` computes `samples_per_step = round(sr/(fps·mean_steps_per_frame))` per plan §3.7 (total audio ≈ n_frames/fps); per voice maintains persistent `music_state` across frames; per frame synthesizes one frame-worth of audio from that frame's voice-pixel slice; missing-probe frames produce per-voice silence of `samples_per_step·mean_steps_per_frame`; per-voice streams concat → zero-pad → mix → reverb → limiter → WAV+MP3+stems+MIDI; muxes WAV source into video (not MP3, no double-compression per plan §2.5). MIDI notes offset by cumulative prior-frame step counts for monotonic ticks. Missing-probe across all frames → `_et_fallback` + legacy frame-chord fallback with `frame_stats_fallback`. Derived event rate deviating >2× from user's `event_rate` logs `_et_fallback` per plan §3.7 approximate-tolerance (samples_per_step wins so audio duration tracks video). **C.5** `_render_frame` signature gets optional `music_state=None`; return tuple extends from `(buf, elapsed, it_raw)` to `(buf, elapsed, it_raw, music_state)`; both GPU and CPU return statements updated; music_state is currently pass-through plumbing — the actual per-frame synthesis happens inside `et_native_music_video` which maintains its own per-voice state internally. **C.6** `generate_et_fractal` unpacks 4-tuple as `render_buf, elapsed, it_raw, _music_state = _render_frame(...)`. `generate_zoom_video` unpacks 4-tuple as `render_buf, elapsed, it_raw_frame, video_music_state = _render_frame(..., music_state=video_music_state)` threading state across frames; also accumulates `frame_probe_data` (full `it_raw` dicts per frame) alongside legacy `frame_audio_stats` list; final `et_native_music_video` call uses `frame_probe_data=..., frame_stats_fallback=frame_audio_stats` so the orbit-trace path has the probe data AND the legacy fallback path retains its input per the no-removal rule. Performance note: `_native_music_synth_voice` uses an explicit per-sample Python inner loop for phase accumulation. A vectorized form is available but was explicitly forbidden by the user mid-session — "precision and correctness over performance, no exceptions; intensive things will be moved to C modules later." The script therefore ships with the precise per-sample Python form; a runtime smoke test of `et_native_music_image` with a 100-step 3-probe synthetic trace times out at the 10-minute limit, which is expected and acceptable. Structural verification is green (AST + 16 new symbols + zero 44100 residuals + 4-tuple unpack at both callers). One correction-during-session: an initial `scipy.signal.lfilter` substitution in `_et_music_shelf_filter` was reverted to the explicit Python recurrence per the same precision-first directive — even though lfilter is mathematically the same one-pole form, the substitution was performance-motivated and therefore forbidden. Three Tools applied: **Identification** — P (per-probe per-step orbit state from Phase B) | D (`_D_FREQ` pitch + `_FAM_ELEGANCE` amplitude + `LN2/d` pan phase + K^h harmonics + K-attack/V-release envelope + shelf EQ from register + Koide K-stereo D/T-axis blend) | T (`_native_music_synth_voice` as the central T walking P step-by-step and emitting oscillator state sample-by-sample with D applied at each sample); **Descriptor Gap closed** — before Phase C music came from rendered-RGB image scans (lossy re-derivation of D from the pixel projection), after Phase C music comes from the raw probe traces (the same (P,D) tuple the visual renderer consumed), and the legacy image-scan path remains as the logged-fallback side of the dispatcher; **Subsumption** — every voice in plan §3.4 is synthesized by the one `_native_music_synth_voice` function given its probe traces + catalog metadata; every user-selected image duration is subsumed by the natural-cycle-loop-plus-fade logic; every video run is subsumed by the per-frame phase-continuous state plumbing; zero remainder. Next action: Phase D (user-facing TUI controls + output-quality tiers + FLAC sidecar + mux chain rewrite). |
