...
class LatticeWalkCompressor:
    ...
    def find_walk_archetypes(self, sym_stream: List[int],
        ...
        # Accumulate for database storage (side-channel — does not affect compression)
        self.discovered_archetypes.extend(archetypes)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ARCHETYPE DATABASE — Persistent Pattern Memory with Koide Disk Budget
#
...
#
# The Koide ceiling governs disk usage. When the database exceeds K × budget,
# the lowest-value archetypes (low hit count × low elegance) are pruned —
# natural selection on the lattice. T_WEIGHT = 1/3 of entries are pruned
# each cycle, matching the resource governance ratio.
#
# ET Derivation:
...
class ArchetypeDatabase:
    """
    Persistent archetype storage with Koide-governed disk budget.

    Stores archetypes discovered during compression in a SQLite database.
    Groups patterns by quantized R₀ for fast same-type lookups.
    Prunes low-value entries when disk budget is exceeded.
    ...
    # Default disk budget base: 100MB. Koide ceiling: K × 100MB = 66.7MB
    DEFAULT_BUDGET_BASE = 100 * 1024 * 1024
    ...
    def __init__(self, db_path: Optional[str] = None,
        ...
        if db_path is None:
            # Default: in user's temp directory (survives session, not machine)
            # For permanent storage, pass an explicit path
            db_dir = os.path.join(tempfile.gettempdir(), 'cdf_archetype_db')
            os.makedirs(db_dir, exist_ok=True)
            db_path = os.path.join(db_dir, 'archetypes.db')
        self.db_path = db_path

        # Koide disk budget: K × base
        base = disk_budget_bytes or self.DEFAULT_BUDGET_BASE
        self.disk_budget = int(base * K_KOIDE)

        self._init_db()
        self._log(f"ArchetypeDB: {self.db_path} "
                  f"(budget: {self.disk_budget / (1024 ** 2):.1f} MB)")
    ...
    def store(self, archetypes: list, source_r0: float):
        """
        Store newly discovered archetypes.
            ...
            for arch in archetypes:
                ...
                if existing:
                    conn.execute('''
                        ...
                        WHERE pattern_hash = ?
                    ''', (len(arch.occurrences), now, arch.hierarchy_elegance, ph))
                    n_updated += 1
                else:
                    conn.execute('''
    ...
    def lookup(self, source_r0: float,
        ...
        Returns patterns that were previously found in files with the
        same quantized R₀ (same file type / byte distribution). Each
        result includes the pattern tuple, its elegance, and hit count.
    ...
    def _check_budget(self):
        """Prune low-value archetypes if database exceeds Koide disk budget."""
        if not os.path.isfile(self.db_path):
            return
        db_size = os.path.getsize(self.db_path)
        if db_size <= self.disk_budget:
            return

        import sqlite3
        self._log(f"ArchetypeDB: {db_size / (1024 ** 2):.1f} MB exceeds budget "
                  f"{self.disk_budget / (1024 ** 2):.1f} MB — pruning...")

        with sqlite3.connect(self.db_path) as conn:
            total = conn.execute('SELECT COUNT(*) FROM archetypes').fetchone()[0]
            # Delete T_WEIGHT = 1/3 of entries (lowest value)
            n_delete = max(1, int(total * T_WEIGHT))
            conn.execute('''
                DELETE FROM archetypes WHERE pattern_hash IN (
                    SELECT pattern_hash FROM archetypes
                    ORDER BY hit_count * hierarchy_elegance ASC
                    LIMIT ?
                )
            ''', (n_delete,))
            conn.execute('VACUUM')
        ...
        self._log(f"ArchetypeDB: pruned {n_delete} entries, "
                  f"{new_size / (1024 ** 2):.1f} MB after vacuum")
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        if not os.path.isfile(self.db_path):
            return {'entries': 0, 'total_hits': 0, 'avg_elegance': 0.0,
                    'disk_mb': 0.0, 'budget_mb': self.disk_budget / (1024 ** 2),
                    'budget_pct': 0.0, 'r0_groups': 0}
        ...
        return {
            ...
            'budget_mb': self.disk_budget / (1024 ** 2),
            'budget_pct': (db_size / max(self.disk_budget, 1)) * 100,
            'r0_groups': r0_groups,
        }
    ...
    def summary(self) -> str:
        ...
        s = self.stats()
        return (f"ArchetypeDB: {s['entries']} patterns across {s['r0_groups']} R₀ groups, "
                f"{s['total_hits']} total hits, "
                f"avg elegance {s['avg_elegance']:.1f}, "
                f"{s['disk_mb']:.1f}/{s['budget_mb']:.1f} MB "
                f"({s['budget_pct']:.0f}%)")
    ...
    def compress_file(self, input_path: str, output_path: str) -> dict:
        ...
        try:
            ...
            if self.engine.discovered_archetypes:
                self.archetype_db.store(self.engine.discovered_archetypes, global_r0)
                ...
                if len(parts) >= 2 and parts[1].lower() == 'clear':
                    ...
                    try:
                        with sqlite3.connect(db.db_path) as conn:
                            ...
                            conn.execute('VACUUM')
                        self.log("  Archetype database cleared.")
                    except Exception as e:
                    ...
                    if s['entries'] > 0:
                        ...
                        self.log(f"  Disk: {s['disk_mb']:.2f}/{s['budget_mb']:.1f} MB "
                                 f"({s['budget_pct']:.0f}%)")
                return
