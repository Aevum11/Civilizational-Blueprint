...
# ── default-endpoint & shared-format control — the OS's own switches,
#    invoked programmatically (IPolicyConfig; vtable layout LIVE-PROVEN on
#    Mike's machine by the successful default switch) ─────────────────────────
def mm_id_of(instance_id):
    return instance_id.split("MMDEVAPI\\", 1)[1] if "MMDEVAPI\\" in instance_id else instance_id

def is_render_ep(instance_id):
    return "{0.0.0." in instance_id          # render flow flag; capture is {0.0.1.
...
def _policy_iface():
    ...
    class IPolicyConfig(IUnknown):
        ...
        # Slots 0-3 are the REAL format methods; 4-9 are vtable OFFSET PADDING
        # (required ABI structure, never called, complete by design); then the
        # two methods this tool uses. Layout live-proven by SetDefaultEndpoint.
        _methods_ = ([COMMETHOD([], HRESULT, 'GetMixFormat',
                                (['in'], ctypes.c_wchar_p, 'wszDeviceId'),
                                (['out'], ctypes.POINTER(PWFX), 'ppFormat')),
                      ...
                      COMMETHOD([], HRESULT, 'SetDeviceFormat',
                                (['in'], ctypes.c_wchar_p, 'wszDeviceId'),
                                (['in'], PWFX, 'pEndpointFormat'),
                                (['in'], PWFX, 'pMixFormat'))] +
                     [COMMETHOD([], HRESULT, '_vtable_pad%d' % i) for i in range(6)] +
    ...
    cls = GUID('{870af99c-171d-4f9e-af0d-e63df40c2bc9}')
    return comtypes.CoCreateInstance(cls, interface=IPolicyConfig)

def set_default_endpoint(mm_id):
    try:
        pc = _policy_iface()
        for role in (0, 1, 2):                # console, multimedia, communications
            pc.SetDefaultEndpoint(mm_id, role)
        return True
    except Exception as e:
        print(f"  (programmatic default-switch unavailable: {e})")
        return False

def set_shared_rate(mm_id, new_rate):
    """Set an endpoint's SHARED-mode format rate via the OS's own switch —
    the same act as Sound settings > Advanced, no samples touched. Returns
    the previous rate for exact revert, or None on refusal."""
    try:
        pc = _policy_iface()
        pw = pc.GetDeviceFormat(mm_id, 1)
        wf = pw.contents
        old = int(wf.nSamplesPerSec)
        wf.nSamplesPerSec = int(new_rate)
        wf.nAvgBytesPerSec = int(new_rate) * int(wf.nBlockAlign)
        pc.SetDeviceFormat(mm_id, pw, pw)
        return old
    except Exception as e:
        print(f"  (shared-format change unavailable: {e})")
        return None
...
def bridge(gaps):
    ...
    dst_pa = (devs[dev]["name"] if dev is not None
              else sd.query_devices(kind="output")["name"]).strip()
    if src_name in dst_pa.lower() or dst_pa.lower() in src_name:
        # AUTO-TOPOLOGY: the default output IS the destination. Move the
        # playback surface to another render endpoint (retrieved, not guessed),
        # bridge from it, restore the default on exit. Zero manual steps.
        eps = gaps.get("all_eps", [])
        dst_ep = next((e for e in eps if e["name"].lower() in dst_pa.lower()
                       or dst_pa.lower() in e["name"].lower()), None)
        alt_ep = next((e for e in eps if e["status"] == "OK" and is_render_ep(e["id"])
                       and "hands-free" not in e["name"].lower()
                       and (dst_ep is None or e["id"] != dst_ep["id"])
                       and e["name"].lower() not in dst_pa.lower()), None)
        if alt_ep is None:
            print("No alternate render endpoint exists to act as the playback")
            print("surface — audio already flows directly to the headphones, so a")
            print("bridge would only echo. Skipping (the honest no-op).")
            p.terminate(); return
        print(f"Auto-topology: Windows default output -> {alt_ep['name']}")
        if not set_default_endpoint(mm_id_of(alt_ep["id"])):
            print(f"Programmatic switch failed; set default output to {alt_ep['name']} and rerun.")
            p.terminate(); return
        if dst_ep is not None:
            restore_mm[0] = (mm_id_of(dst_ep["id"]), dst_ep["name"])
        time.sleep(1.2)
        p.terminate(); p = pyaudio.PyAudio()
        lb = p.get_default_wasapi_loopback()
        rate = int(lb["defaultSampleRate"])
        print(f"Playback surface now: {lb['name']} @ {rate} Hz")
        src2 = lb["name"].lower().replace(" [loopback]", "").strip()
        if src2 in dst_pa.lower() or dst_pa.lower() in src2:
            print("Switch did not take effect — aborting rather than echo-looping.")
            p.terminate(); return
    st = None; inp = None; fmt_restore = [None]
    dst_rate = int((devs[dev] if dev is not None
                    else sd.query_devices(kind="output"))["default_samplerate"])
    try:
        if rate != dst_rate:
            print(f"Rate split: surface {rate} Hz vs headphones link {dst_rate} Hz.")
            print("No sample is ever fabricated here — resolving by descriptor:")
            cand = None
            try:
                for d in p.get_loopback_device_info_generator():
                    if (int(d["defaultSampleRate"]) == dst_rate
                            and dst_pa.lower() not in d["name"].lower()):
                        cand = d; break
            except Exception:
                pass
            if cand is not None:
                lb = cand; rate = dst_rate
                print(f"  [OK] surface already at link rate found: {lb['name']}")
            else:
                surf_ep = next((e for e in gaps.get("all_eps", [])
                                if is_render_ep(e["id"]) and e["status"] == "OK"
                                and e["name"].lower().split(" (")[0]
                                    in lb["name"].lower()), None)
                if surf_ep is not None:
                    old = set_shared_rate(mm_id_of(surf_ep["id"]), dst_rate)
                    if old is not None:
                        fmt_restore[0] = (mm_id_of(surf_ep["id"]), old, surf_ep["name"])
                        print(f"  [OK] surface shared format set {old} -> {dst_rate} Hz"
                              " (the OS's own switch; reverted on exit)")
                        time.sleep(1.5)
                        p.terminate(); p = pyaudio.PyAudio()
                        lb = p.get_default_wasapi_loopback()
                        rate = int(lb["defaultSampleRate"])
                if rate != dst_rate:
                    print("  No lossless topology exists on this hardware right now:")
                    print("  every reachable surface runs a different rate than the")
                    print("  headphones' A2DP link, and this tool will not invent")
                    print("  samples. The engine's direct path is then equivalent —")
                    print("  standing down cleanly.")
                    return
        mode = None
        try:
            ...
            try:
                ...
                print(f"Shared open also refused ({e2}) — standing down cleanly.")
                return
        ...
        traceback.print_exc()
        print("Bridge closed on the error above — cleanup and restore follow.")
    finally:
        for closer in ((lambda: inp.close()) if inp else None,
                       (lambda: (st.stop(), st.close())) if st else None,
                       (lambda: p.terminate())):
            if closer:
                try: closer()
                except Exception: pass
        if fmt_restore[0] is not None:
            mm, old, nm = fmt_restore[0]
            if set_shared_rate(mm, old) is not None:
                print(f"Surface shared format reverted -> {old} Hz ({nm})")
            else:
                print(f"Revert the shared format of {nm} to {old} Hz manually if desired.")
        if restore_mm[0] is not None:
            mm, nm = restore_mm[0]
            if set_default_endpoint(mm):
                print(f"Default output restored -> {nm}")
            else:
                print(f"Could not restore automatically — set default output back to {nm}.")

# ── GLITCH BISECTOR — localize intermittent fragmentation to ENGINE side
#    (pre-radio, software-fixable) or RF side (post-antenna, physical).
#    Method: capture the headphones endpoint's own loopback (the exact bytes
#    the engine hands the radio), run exact INTEGER continuity analysis, and
#    let the ears mark fragmentation moments. Clean engine at the marks
#    proves the loss is in the air; dirty engine proves it is upstream. ─────
def analyze_block(prev_last, ints):
    """Exact integer continuity metrics for one block. Returns
    (zeros_run_max, junction_step, intra_step_max, level_sum)."""
    zr = zmax = 0
    for v in ints:
        zr = zr + 1 if v == 0 else 0
        if zr > zmax: zmax = zr
    junction = abs(int(ints[0]) - int(prev_last)) if prev_last is not None else 0
    intra = 0
    for a, b in zip(ints, ints[1:]):
        d = abs(int(a) - int(b))
        if d > intra: intra = d
    level = sum(abs(int(v)) for v in ints)
    return zmax, junction, intra, level

def bisector(gaps):
    print("\nGLITCH BISECTOR — listen normally; press ENTER the moment audio")
    print("fragments; Ctrl+C ends the session and renders the verdict.\n")
    pyaudio = ensure("pyaudiowpatch")
    p = pyaudio.PyAudio()
    lb = None
    targets = [e["name"].lower() for e in gaps.get("a2dp", [])]
    try:
        for d in p.get_loopback_device_info_generator():
            if any(t in d["name"].lower() or d["name"].lower() in t for t in targets):
                lb = d; break
    except Exception:
        pass
    if lb is None:
        try: lb = p.get_default_wasapi_loopback()
        except Exception:
            print("No loopback tap available — connect the headphones and rerun.")
            p.terminate(); return
    rate = int(lb["defaultSampleRate"])
    print(f"Tapping engine output: {lb['name']} @ {rate} Hz (capture-only; no re-render)")
    # session-scope timer hardening (documented winmm API; restored after)
    timer_set = False
    try:
        ctypes.windll.winmm.timeBeginPeriod(1); timer_set = True
    except Exception:
        pass
    try:
        import msvcrt
        ...
        kb = None
        print("(keyboard marking unavailable on this platform — logging metrics only)")
    inp = p.open(format=pyaudio.paInt16, channels=2, rate=rate, input=True,
                 input_device_index=lb["index"], frames_per_buffer=BLOCK)
    logpath = os.path.join(config_dir(), "etbt_glitch_log.csv")
    rows = []; marks = []
    prev_last = None
    expect_ns = BLOCK * 1_000_000_000 // rate
    t_prev = time.perf_counter_ns()
    late = 0
    print("LIVE — metrics every 64 blocks: zeros | junction | jitter-late | marks")
    try:
        n = 0
        while True:
            raw = inp.read(BLOCK, exception_on_overflow=False)
            ...
            if dt > 2 * expect_ns: late += 1
            ints = struct.unpack("<%dh" % (len(raw) // 2), raw)
            zmax, junc, intra, level = analyze_block(prev_last, ints)
            prev_last = ints[-1]
            rows.append((t_now, zmax, junc, intra, level, dt))
            if kb:
                while kb.kbhit():
                    kb.getch(); marks.append(t_now)
                    print(f"\n  [MARK #{len(marks)}] ear-flag recorded")
            n += 1
            if n % 64 == 0:
                print(f"  blocks {n:6d} | zmax {zmax:4d} | junc {junc:5d} | "
                      f"late {late:3d} | marks {len(marks)}", end="\r")
    except KeyboardInterrupt:
        print("\nSession ended — computing the verdict.")
    finally:
        try: inp.close()
        except Exception: pass
        p.terminate()
    ...
    try:
        with open(logpath, "w", encoding="utf-8") as f:
            f.write("t_ns,zeros_run,junction,intra_max,level,dt_ns\n")
            for r in rows: f.write(",".join(str(x) for x in r) + "\n")
        print(f"Full metric log: {logpath}")
    except Exception as e:
        print(f"(log write failed: {e})")
    if not marks:
        print("No ear-marks recorded — run again and press ENTER during a bad patch.")
        return
    # verdict: engine-dirty if, within ±2 s of a mark, the engine tap shows a
    # silence-gap run, an outsized junction step, or a late callback burst
    win = 2_000_000_000
    engine_dirty = 0
    for mts in marks:
        seg = [r for r in rows if abs(r[0] - mts) <= win]
        gap = any(r[1] > BLOCK // 8 for r in seg)
        step = any(r[2] > 8192 and r[3] < 2048 for r in seg)
        lateb = sum(1 for r in seg if r[5] > 2 * expect_ns) >= 3
        if gap or step or lateb:
            engine_dirty += 1
    clean = len(marks) - engine_dirty
    print(f"\nVERDICT over {len(marks)} ear-marked episodes:")
    print(f"  engine-side evidence at {engine_dirty} · engine CLEAN at {clean}")
    if engine_dirty > clean:
        print("  => ENGINE SIDE: the stream is damaged before the radio.")
        print("     Actions: update the Intel Bluetooth + chipset drivers; hunt the")
        print("     DPC source (GPU/network driver latency storms starve the audio")
        print("     graph); keep the timer hardening; retest after each change.")
    else:
        print("  => RF SIDE, proven: the engine handed the radio intact samples at")
        print("     the very moments your ears heard fragments — the loss is in the")
        print("     air, after the antenna. Actions, cheapest first: reposition the")
        print("     PC/antennas for line-of-sight (steel cases shadow rear-mounted")
        print("     antennas); move USB-3 devices (QuadCast included) and their")
        print("     cables away from the antenna path; a $15 BT dongle on a USB-2")
        print("     extension placed at desk level gives Bluetooth its own radio")
        print("     and its own vantage — the decisive physical fix.")
...
def selftest():
    ...
    rep("MMDevice id parser strips the SWD prefix",
        mm_id_of("SWD\\MMDEVAPI\\{0.0.0.00000000}.{CA34}") == "{0.0.0.00000000}.{CA34}")
    rep("render/capture flow classifier from the id flow flag",
        is_render_ep("{0.0.0.00000000}.{X}") and not is_render_ep("SWD\\MMDEVAPI\\{0.0.1.00000000}.{Y}"))
    z, j, i2, lv = analyze_block(100, [0] * 600 + [500, -500] * 100)
    rep("bisector detects a silence-gap run exactly (600 zeros)", z == 600)
    z, j, i2, lv = analyze_block(-12000, [12000, 12010, 12020, 12030])
    rep("bisector measures the junction discontinuity exactly (24000)",
        j == 24000 and i2 == 10)
...
def main():
    ...
    if ans in ("", "y", "yes"):
        bridge(gaps or {})
    ans = input("\nRun the GLITCH BISECTOR (engine vs RF, ears as marker)? [Y/n]: ").strip().lower()
    if ans in ("", "y", "yes"):
        bisector(gaps or {})
