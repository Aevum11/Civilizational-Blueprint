...
# ── default-endpoint control — the OS's own switch, invoked programmatically
#    (IPolicyConfig) so the user never opens a settings page ─────────────────
...
def set_default_endpoint(mm_id):
    try:
        ...
        class IPolicyConfig(IUnknown):
            ...
            # The first 10 entries are vtable OFFSET PADDING for this
            # undocumented interface — required ABI structure so that
            # SetDefaultEndpoint binds to its correct slot (#11). They are
            # never called and are NOT unimplemented features: remove them
            # and the call lands on the wrong function. Complete by design.
            _methods_ = ([COMMETHOD([], HRESULT, '_vtable_pad%d' % i) for i in range(10)] +
        ...
        pc = comtypes.CoCreateInstance(cls, interface=IPolicyConfig)
