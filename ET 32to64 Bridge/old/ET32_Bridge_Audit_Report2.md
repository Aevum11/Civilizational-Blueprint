...
| ISSUE-19 | **MEDIUM** | Multiple | — | Error capture incomplete | ✓ |
| ISSUE-20 | **LOW** | `build.bat`, `et_bridge32.c` | 27 / 170 | -lntdll inconsistency | ✓ |

**Total Issues Found: 21** (20 base + ISSUE-01B sub-issue)
**Critical: 6 · High: 5 · Medium: 7 · Low: 3**
...
| Family | d | Broker (et_host64.py) | C DLL (et_bridge32.c) | ETAPIGateway | Parity |
|--------|---|----------------------|----------------------|--------------|--------|
| MEMORY_BASIC | 1 | Complete | Complete (hooks present) | Partial | ✗ (injection broken) |
| MEMORY_MAP | 2 | Complete | Complete | Partial | ✗ (injection broken) |
| THREAD_OPS | 3 | Complete | Partial — `GetThreadContext` not in IAT table | None | ✗ |
| DLL_OPS | 4 | Complete | Complete | Partial | ✗ (injection broken) |
| PROCESS_OPS | 5 | Complete | Partial — `EnumProcesses`, `EnumProcessModules`, `WOW64FS` not in IAT table | Partial | ✗ |
| REGISTRY_OPS | 6 | Complete | Partial — `RegCreateKeyExW`, `RegDeleteKeyExW`, `RegDeleteValueW`, `RegCloseKey` defined in `ET32_*` wrappers but IAT table coverage unverified | Partial | ✗ |
| GRAPHICS_OPS | 7 | Partial — no real GPU/D3D calls; `GPU_SUBMIT`, `GPU_MAP_VRAM` unimplemented | Minimal | None | ✗ |
| FILE_OPS | 8 | Complete (all 16 ops) | Complete (all wrappers present) | None | ✗ (no gateway) |
| SYNC_OPS | 9 | Complete (all 9 ops) | Partial — `SYNC_RELEASE_SEM`, `SYNC_WAIT_MULTIPLE`, `SYNC_RESET_EVENT`, `SYNC_CLOSE` missing from IAT hooks | None | ✗ |
| NET_OPS | 10 | Complete (all 10 ops) | Partial — `NET_SELECT64`, `NET_SOCKOPT64` missing from IAT hooks | None | ✗ |
| PYTHON_OPS | 11 | Complete (all 8 ops) | Complete | Partial | ✗ (injection broken) |
| COMPOUND_OPS | 12 | **DEAD** (ISSUE-05) | Present in DLL | None | ✗ |

### Additional Parity Gaps

**Children — not fully equivalent.** The monitor detects children and bridges them (et_monitor.py:418–479). However, because the DLL injection is broken (ISSUE-02), children receive the same non-functional shellcode-stub injection as the parent. Children never get real 64-bit capabilities.

**Blacklist mode — children unverified.** In blacklist mode, `synthetic_config()` (et_config.py:462) generates a full-feature config for unknown processes. Children of bridged processes inherit the parent's config. This path has never been end-to-end tested since the injection itself is broken.

**SeLockMemoryPrivilege for AWE.** If the broker is not run as Administrator with SeLockMemoryPrivilege, `AllocateUserPhysicalPages` fails (et_awe.py:179). The AWE bookshelf — which provides genuine physical memory access beyond 4GB — silently degrades to standard `VirtualAlloc`. The 32-bit process cannot exceed 4GB of addressable memory in this case. The failure is logged (et_api.py:887) but no operator-facing error is shown.

**GPU/VRAM family effectively absent.** The GRAPHICS_OPS family (d=7) has command codes defined for D3D12, D3D11, D3D9, Vulkan, NvAPI, ADL, GPU alloc, and Heaven GPU calls. However, the broker handlers in `_handle_graphics_ops()` (et_host64.py) do not perform real D3D or VRAM operations — they have no DXGI enumeration, no actual GPU memory allocation, and no Heaven's Gate GPU calls. A 32-bit application relying on the bridge to access >4GB VRAM has no functional path.

### Conclusion

No 32-bit program in the current codebase receives any 64-bit capabilities — the injection chain is broken at three independent points (ISSUE-01, ISSUE-02, ISSUE-05). Once the injection chain is repaired, the bridge provides genuine 64-bit parity for MEMORY_BASIC, MEMORY_MAP, DLL_OPS, REGISTRY_OPS, FILE_OPS, and PYTHON_OPS. GRAPHICS_OPS, SYNC_OPS, NET_OPS, and COMPOUND_OPS require additional implementation work to reach parity.
...
13. **ISSUE-17**: Rename all d=1 command code defines in `et_bridge32.c` to match `et_math.py` exactly. Add `CMD_READ_MEM (0x0B)` and `CMD_WRITE_MEM (0x0C)` defines to C.
14. **ISSUE-18**: Add `CMD_AWE_INIT_SIGNAL (0x0D)` to both Python and C. Remove `is_awe_init` heuristic from `HEAP_ALLOC` handler.
15. **ISSUE-19**: Implement exit-code capture in `on_process_exit()` and optionally `OutputDebugString` forwarding.

### Tier 4 — Race Conditions

16. **ISSUE-13**: Hold `_pipe_lock` across the entire `create_pipe_for_pid` sequence (check → create → IOCP associate → register).
17. **ISSUE-14**: Use a `_pending_pids: Set[int]` inside `_lock` to prevent concurrent engagement of the same PID.

### Tier 5 — Bugs and Polish

18. **ISSUE-15**: Change broker pipe creation to `PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE` to match the DLL's intent, or remove the `SetNamedPipeHandleState` call from the C DLL.
19. **ISSUE-16**: Replace the O(n) loop in `project_address()` with `self._addr64_to_handle.get(addr64)`.
20. **ISSUE-20**: Remove `-lntdll` from the source comment to match `build.bat`.
