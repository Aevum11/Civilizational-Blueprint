...
class CurvatureAnalyzer:
    ...
    def compute_segmentation(self, dk_stream: List[int],
        ...
        # Map ddk-stream indices → byte indices: b_ddk → b_ddk + 1
        # Clamp to (0, n_bytes-1) so neither endpoint is degenerate.
        # Drop boundaries that, after clamping, would produce a segment
        # shorter than min_segment_length BYTES from either side.
        byte_boundaries: List[int] = []
        for b in boundaries_ddk:
            byte_idx = b + 1  # mapping: ddk index b ↔ byte split at byte b+1
...
class ArchetypeDatabase:
    ...
    def compact_to_cdf(self):
        """Compress the archetype database in-place via CDF VFS.

        Tier 7 deliverable (full implementation in §19.7 of the design
        doc). This stub raises NotImplementedError so that
        _maybe_compact_for_disk_pressure can detect Tier 7 absence and
        log a clear "no compaction available; no deletion either"
        message — preserving the §16.9 NO-REMOVAL contract.

        When implemented (Tier 7), this method will:
          1. Open a CDFDatabaseVFS handle on a temporary `.cdf` file.
          2. Stream every row from the live SQLite DB into the VFS.
          3. The VFS encodes each row through the lattice compressor,
             producing a generator-keyed compact representation.
          4. Atomically rename the `.cdf` over the live `.db`.
          5. Verify roundtrip: read every row back through the VFS,
             confirm every original row is reconstructible byte-exact.
          6. On any verification failure, abort and leave the original
             `.db` untouched (NEVER lose data).

        Until Tier 7: NotImplementedError is the explicit signal that
        the operation is not yet available. The caller MUST handle
        this without falling back to deletion.
        """
        raise NotImplementedError(
            "compact_to_cdf is a Tier 7 deliverable; the implementation "
            "lands with the CDFDatabaseVFS + GeneratorEvaluator work. "
            "Until then, the contract holds: NOTHING IS EVER DELETED. "
            "If disk pressure is real, free space externally."
        )
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        # Sense disk free space on the database volume
        db_dir = os.path.dirname(os.path.abspath(self.db_path))
