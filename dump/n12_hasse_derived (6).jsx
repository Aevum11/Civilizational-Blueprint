...
export default function N12Hasse() {
  ...
  // FULL EM→m TABLE — every harmonic family m=1..N. Shadow targets live at their
  // NATIVE lattice R = lcm(N, m); reaching them requires the carry κ that lands in
  // THEIR resolution, which for shadows is |κ|≥2. Search the FULL carry range at
  // each R — NO channel is closed; every family opens (the shadows ARE residues,
  // tracked by ε to their native homes). κ=0 present → D-arithmetic (green);
  // only κ≠0 reaches it → T-act (red).
  const resN = resOf(N);
  const fullTable = range1(N).map(m => {
    ...
    lift.forEach(r1 => lift.forEach(r2 => {
      for (let kap = -Math.floor(R / 2); kap <= Math.floor(R / 2); kap++) {
        if (dR((r1 + r2 + kap % R + R) % R) === m) {
          kset.add(kap); tot++;
          if (kap === 0) c0++;
        }
      }
    }));
    const nK = kset.size || 1;
    const Tnum = tot, Tden = lift.length * lift.length * nK;   // exact rational
    const g = gcd(Tnum, Tden);
    const Enum = (Tnum / g) * A0, Eden = (Tden / g) * xiDen(m);
    return { m, R, c0, kmin: Math.min(...[...kset].map(Math.abs)),
      kap: c0 > 0 ? "κ=0" : "κ≠0", eff: trunc3(Enum, Eden), open: true };
  });

  // DRAWN CHANNELS — EVERY family gets its arrow (green κ=0 / red κ≠0-only),
  // shadows included at their diagrammed positions. The self-channel m=N is the
  // one non-line: its direction vector is 0/0, and [0/0] IS the Traverser
  // (Sempaevum §2.4) — rendered as the red T-act ring, no curve.
  ...
  return (
    <div style={{
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, margin: "0 auto", display: "block" }}>
        ...
        <text x={418} y={1307} fill={t.legend} fontSize="6">κ≠0 (T-act only)</text>
